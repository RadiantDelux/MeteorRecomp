...
public class PpcLifterCoverageTests
{
    ...
    public void LiftsUpdateLoadFormsAndCacheNoOps()
    {
        ...
        var icbi = Instruction(0, "icbi", Gpr(4), Gpr(5));
        var icbiIr = Assert.Single(lifter.Lift(new[] { icbi })).Ir;
        Assert.Equal("add", Assert.IsType<IrBinary>(icbiIr[0]).Op);
        Assert.Equal("PPC_Icbi", Assert.IsType<IrCall>(icbiIr[1]).Target);

        foreach (var mnemonic in new[] { "dcbi" })
        {
    ...
    public void LiftsCacheHintsByteReverseAndRawXoFallbacks()
    {
        ...
        foreach (var mnemonic in new[] { "xo_470", "dcbt", "xo_278", "dcbtst", "xo_246", "dcba", "xo_758", "tlbie", "xo_306", "tlbsync", "xo_566" })
        {
