...
public sealed class TranslatedBuildShardEmitterTests
{
    ...
    public void EmitsDeterministicSmallTraitShardsWithoutCodeMap()
    {
        ...
        try
        {
            ...
            Assert.Contains("InvokeStaticTranslatedCpu<Target, Entry>(Context)", shardText);
            Assert.DoesNotContain("ApplyRuntimeCallOptions(Target, Context)", shardText);
    ...
    [Fact]
    public void GenericDolShardEmissionKeepsBaseTranslationForSuppressedLegacyOverride()
    {
        var root = Path.Combine(Path.GetTempPath(), $"translator-generic-native-{Guid.NewGuid():N}");
        var functions = Path.Combine(root, "functions");
        var native = Path.Combine(root, "runtime", "src");
        var output = Path.Combine(root, "out");
        Directory.CreateDirectory(functions);
        ...
        try
        {
            const string legacyTarget = """
                ...
                // RECOMP_REGISTRATION base 0x80003000 func_80003000 preserves=true fpr_mask=0x00000000
                """;
            const string titleTarget = """
                #include "abi_bridge.h"
                extern "C" void func_80004000(CpuContext*) {}
                // RECOMP_REGISTRATION base 0x80004000 func_80004000 preserves=true fpr_mask=0x00000000
                """;
            Write(Path.Combine(functions, "func_80003000.cpp"), legacyTarget);
            Write(Path.Combine(functions, "func_80004000.cpp"), titleTarget);
            Write(Path.Combine(native, "override.cpp"),
                "REGISTER_TRANSLATED_FUNCTION(0x80003000, LegacyOverride_80003000);\n" +
                "REGISTER_TITLE_NATIVE_FUNCTION(0x80004000, TitleOverride_80004000);\n");

            var metadataPath = Path.Combine(root, "base_output.json");
            BaseTranslationOutputMetadataFile.WriteIfChangedAtomic(metadataPath,
                BaseTranslationOutputMetadata.Create([
                    Metadata("func_80003000.cpp", 0x80003000u, legacyTarget),
                    Metadata("func_80004000.cpp", 0x80004000u, titleTarget)
                ], TranslationQualityMetadata.Clean, "identity"));
            ...
            Assert.Equal(2, result.BaseFunctionCount);
            Assert.Equal(1, result.SharedBaseFunctionCount);
            var baseDispatch = File.ReadAllText(
                Directory.GetFiles(Path.Combine(output, "base_dispatch"), "*.cpp").Single());
            Assert.Contains("{0x80003000u, &func_80003000, 0x00000000u, false}", baseDispatch);
            Assert.DoesNotContain("LegacyOverride_80003000", baseDispatch);
            Assert.DoesNotContain("0x80004000u", baseDispatch);
            Assert.DoesNotContain(
                "func_80004000",
                Directory.GetFiles(output, "*.cpp", SearchOption.AllDirectories)
                    .Select(File.ReadAllText)
