...
public class SdaAndGeneratorTests
{
    ...
    public void RuntimeConfigGeneratorEmitsConfiguredConstants()
    {
        ...
        RuntimeConfigGenerator.GenerateConfigHeader(
            0x8038F780u, 0x8038E9C8u, entry, output, gameId: "R3SP52");
        ...
        Assert.Contains("constexpr uint32_t ENTRY_POINT = 0x80006000u;", text);
        Assert.Contains("constexpr uint32_t GAME_CODE = 0x52335350u;", text);
        Assert.Contains("constexpr uint16_t MAKER_CODE = 0x3532u;", text);
        Assert.Contains("constexpr const char* GUEST_PLATFORM = \"wii\";", text);
        Assert.Contains("constexpr bool IS_WII = true;", text);
        Assert.Contains("constexpr bool IS_GAMECUBE = false;", text);
    }

    [Fact]
    public void RuntimeConfigGeneratorEmitsGameCubePlatformConstants()
    {
        const uint entry = 0x80003100;
        var tempDir = Path.Combine(Path.GetTempPath(), $"mkw_runtime_config_platform_{Guid.NewGuid():N}");
        Directory.CreateDirectory(tempDir);
        try
        {
            var output = Path.Combine(tempDir, "RuntimeConfig.generated.h");
            RuntimeConfigGenerator.GenerateConfigHeader(
                ...
                entry,
                output,
                projectName: "Synthetic GameCube Project",
                gameId: "GABC01",
                guestPlatform: "gamecube");

            var text = File.ReadAllText(output);
            Assert.Contains("constexpr const char* GUEST_PLATFORM = \"gamecube\";", text);
            Assert.Contains("constexpr bool IS_WII = false;", text);
            Assert.Contains("constexpr bool IS_GAMECUBE = true;", text);
        }
        finally
        {
    ...
    public void RuntimeConfigGeneratorDoesNotRewriteIdenticalContent()
    {
        ...
        RuntimeConfigGenerator.GenerateConfigHeader(0x8038F780u, 0x8038E9C8u, entry, output);
