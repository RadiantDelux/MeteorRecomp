...
public class CodeGenTests
{
    ...
    [Fact]
    public void CodeGenerator_EmitsRuntimeCheckpointOnlyOnCfgBackedge()
    {
        var function = new IrFunction(
            ...
            new[]
            {
                new IrBasicBlock("0x80001000", new IrInstruction[]
                {
                    ...
                    new IrJump("0x80001010")
                }),
                new IrBasicBlock("0x80001010", new IrInstruction[]
                {
                    new IrBinary("r3", IrValue.Register("r3"), IrValue.Imm(1), "add"),
                    new IrBranch("beq", "0x80001010", "0x80001020")
                }),
                new IrBasicBlock("0x80001020", new IrInstruction[]
                {
                    new IrReturn(null)
                })
            });
        ...
        var code = new CxxLinearCodeGenerator().Emit(
            ...
            new FunctionAbiClassification("interruptible_loop", ValueRepresentation.Void),
            types);

        Assert.Equal(1, code.Split("ApplyRuntimeLoopOptions(", StringSplitOptions.None).Length - 1);
        Assert.Contains("ApplyRuntimeLoopOptions(0x80001010u, ctx);", code, StringComparison.Ordinal);
    }
