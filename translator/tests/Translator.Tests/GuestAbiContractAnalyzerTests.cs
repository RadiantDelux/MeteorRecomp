...
public sealed class GuestAbiContractAnalyzerTests
{
    ...
    [Fact]
    public void BackedgeRequiresCompleteContextBecauseRuntimeCheckpointCanSwitchFibers()
    {
        var function = new IrFunction("interruptible_loop", "entry", new[]
        {
            new IrBasicBlock("entry", new IrInstruction[]
            {
                new IrJump("loop")
        ...
        Assert.True(contract.HasFullSynchronizationFence);
        Assert.NotEqual(GuestCallBoundaryFlags.None,
            contract.BoundaryFlags & GuestCallBoundaryFlags.CanSuspend);
        Assert.NotEqual(GuestCallBoundaryFlags.None,
            contract.BoundaryFlags & GuestCallBoundaryFlags.CanSwitchThreads);
    }
}
