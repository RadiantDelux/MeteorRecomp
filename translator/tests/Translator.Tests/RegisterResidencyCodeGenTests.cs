...
public class RegisterResidencyCodeGenTests
{
    ...
    public void BoundaryNarrowingSyncsConditionAndCountRegistersOnAnyIntersectingField()
    {
        ...
        var loopSafePoint = untouched.IndexOf("ApplyRuntimeLoopOptions(", call, StringComparison.Ordinal);
        // The exit flush and loop-backedge safe point still publish CR/CTR; only
        // the narrowed call boundary itself drops them. The safe point must
        // reload the resident values because a title hook may service an
        // asynchronous hardware interrupt and mutate the CpuContext.
        ...
        Assert.True(loopSafePoint > call);
        Assert.DoesNotContain("    cr = ctx->cr;", untouched[call..loopSafePoint], StringComparison.Ordinal);
        Assert.DoesNotContain("    ctr = ctx->ctr;", untouched[call..loopSafePoint], StringComparison.Ordinal);
        Assert.Contains("    cr = ctx->cr;", untouched[loopSafePoint..], StringComparison.Ordinal);
        Assert.Contains("    ctr = ctx->ctr;", untouched[loopSafePoint..], StringComparison.Ordinal);
    }
