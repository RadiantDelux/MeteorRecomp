...
public class LifterTests
{
    ...
    [Theory]
    [InlineData(0x41830024u, "bso")]
    [InlineData(0x40830024u, "bns")]
    public void LiftsSummaryOverflowBranches(uint word, string expectedCondition)
    {
        var instruction = PpcDecoder.Decode(0x8007AD38, word);
        var lifter = new PpcLifter();

        var branch = Assert.IsType<IrBranch>(lifter.Lift(new[] { instruction })[0].Ir.Single());

        Assert.Equal(expectedCondition, branch.Condition);
        Assert.Equal("cr0", branch.ConditionRegister);
        Assert.Equal("0x8007AD5C", branch.TrueLabel);
        Assert.Equal("0x8007AD3C", branch.FalseLabel);
    }
}
