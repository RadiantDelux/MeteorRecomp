...
internal sealed class TranslationProjectConfig
{
    ...
    public static TranslationProjectConfig Load(string path)
    {
        ...
        var identity = new ProjectIdentity(
            ...
            dto.Project?.GameId,
            dto.Project?.Region,
            ParsePlatform(dto.Project?.Platform),
        ...
        ProjectBinaryInput? sel = null;
        if (dto.Inputs?.Sel is { Path.Length: > 0 } selDto)
        {
            sel = ResolveInput(workspaceRoot, selDto, "inputs.sel");
        }
        ...
        var translation = new ProjectTranslation(
            ...
            dto.Translation?.EnableStateFreeAbi ?? true,
            dto.Translation?.DiscoverVTableFunctions ?? false,
            dto.Translation?.DiscoverStaticCallbacks ?? false);
        ...
        var runtime = new ProjectRuntime(
            ...
            dto.Runtime?.GenericDolBoot ?? false);
    ...
    private void ValidateInputs()
    {
        ...
        if (Inputs.Sel is not null)
        {
            ValidateInput(Inputs.Sel, "SEL");
        }
    ...
    private static string ParsePlatform(string? value)
    {
        var platform = string.IsNullOrWhiteSpace(value) ? "wii" : value.Trim().ToLowerInvariant();
        return platform is "wii" or "gamecube"
            ? platform
            : throw new InvalidDataException(
                $"Unsupported project.platform '{value}'. Expected 'wii' or 'gamecube'.");
    }
    ...
    private sealed class InputsDto
    {
        ...
        public BinaryInputDto? Sel { get; init; }
    }
    ...
    private sealed class TranslationDto
    {
        ...
        public bool? EnableStateFreeAbi { get; init; }
        public bool? DiscoverVTableFunctions { get; init; }
        public bool? DiscoverStaticCallbacks { get; init; }
    }
    ...
    private sealed class RuntimeDto
    {
        ...
        public bool? GenericDolBoot { get; init; }
    }
...
internal sealed record ProjectInputs(ProjectBinaryInput Dol, ProjectRelInput? Rel, ProjectBinaryInput? Sel);
internal sealed record ProjectTranslation(
    ...
    bool EnableStateFreeAbi,
    bool DiscoverVTableFunctions,
    bool DiscoverStaticCallbacks);
