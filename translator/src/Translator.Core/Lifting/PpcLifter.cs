...
public sealed partial class PpcLifter
{
        ...
        IReadOnlySet<uint>? knownFunctionEntryPoints)
    {
        ...
        try
        {
            ...
            case "stwcx." when operands.Count == 3:
                {
                    ...
                    return BuildIndexedAddress(Reg(1), Reg(2), addr, a => new[]
                    {
                        // Keep CR/XER explicit across the host helper boundary.  PPC_Stwcx used to
                        // update CR0 through the thread-local CpuContext, then generated code reloaded
                        // ctx->cr.  An optimizing C++ compiler is allowed to keep the caller's local CR
                        // cached because the helper has no CpuContext* argument, which can make a
                        // successful reservation look like a failed stwcx. forever.  Returning the new
                        // CR makes the architectural dependency visible to both IR and the host compiler.
                        new IrCall("cr", "PPC_Stwcx", new[]
                        {
                            IrValue.Register(a),
                            IrValue.Register(Reg(0)),
                            IrValue.Register("cr"),
                            IrValue.Register("xer")
                        })
                    });
                }
            ...
            case "icbi" when operands.Count == 2:
                {
                    var rA = operands[0];
                    var rB = operands[1];
                    var addrReg = $"{rB}_addr_icbi";
                    IrInstruction addrCalc = rA == "r0"
                        ? new IrAssign(addrReg, IrValue.Register(rB))
                        : new IrBinary(addrReg, IrValue.Register(rA), IrValue.Register(rB), "add");
                    return new IrInstruction[]
                    {
                        addrCalc,
                        new IrCall(string.Empty, "PPC_Icbi", new[] { IrValue.Register(addrReg) })
                    };
                }
            ...
            case "xo_86":
                {
                    // Host RAM is coherent, but consumers cache decoded GPU
                    // resources. A guest writeback must publish its cache line
                    // even when no title-specific DCStoreRange HLE was called.
                    var rA = operands.Count == 2 ? operands[0] : $"r{(ins.Word >> 16) & 31}";
                    var rB = operands.Count == 2 ? operands[1] : $"r{(ins.Word >> 11) & 31}";
                    var store = mnemonic is "dcbst" or "xo_54";
                    var addrReg = $"{rB}_addr_dcwriteback";
                    IrInstruction addrCalc = rA == "r0"
                        ? new IrAssign(addrReg, IrValue.Register(rB))
                        : new IrBinary(addrReg, IrValue.Register(rA), IrValue.Register(rB), "add");
                    return new IrInstruction[]
                    {
                        addrCalc,
                        new IrCall(string.Empty, store ? "PPC_Dcbst" : "PPC_Dcbf", new[] { IrValue.Register(addrReg) })
                    };
                }

            case "dcbi":
                // No separate host CPU data cache: invalidation is distinct
                // from writing dirty CPU data back to RAM. Deferred GPU reads
                // are materialized by the memory access path.
                return new[] { new IrComment($"{mnemonic} @ 0x{ins.Address:X8} (no-op)") };
        ...
        var functionContext = functionEntryPoint is { } entry
            ? $" [function 0x{entry:X8}]"
            : string.Empty;
        var message = $"UNIMPLEMENTED 0x{ins.Address:X8}: {ins.Mnemonic} {ins.OperandText} (0x{raw}){functionContext}".Trim();
        if (allowUnsupported)
        {
