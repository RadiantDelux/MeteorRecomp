...
public static class GuestAbiContractAnalyzer
{
        ...
        IReadOnlyDictionary<uint, GuestAbiContract>? calleeContracts = null)
    {
        ...
        for (var index = 0; index < blockCount; ++index)
        {
            ...
            // Normal codegen injects ApplyRuntimeLoopOptions on every CFG
            // backedge. That hook can service asynchronous hardware and, on
            // titles using the fiber scheduler, can reschedule to another guest
            // thread. The hook is injected after IR/ABI analysis, so without
            // accounting for it here a loop can advertise a narrow fence=0 ABI.
            // Callers then keep unrelated guest registers resident in C++ locals
            // while the checkpoint saves the entire CpuContext, persisting stale
            // register values into the guest fiber. Treat a backedge exactly like
            // the complete-context/suspending boundary that codegen will emit so
            // the requirement propagates transitively through direct callers.
            var successorCount = Successors(blockList[index], out var firstSuccessor, out var secondSuccessor);
            var hasBackedge =
                (successorCount >= 1 && ordinals.TryGetValue(firstSuccessor, out var firstOrdinal) && firstOrdinal <= index) ||
                (successorCount == 2 && ordinals.TryGetValue(secondSuccessor, out var secondOrdinal) && secondOrdinal <= index);
            if (hasBackedge)
            {
                boundaryFlags |= GuestCallBoundaryFlags.RequiresCompleteContext |
