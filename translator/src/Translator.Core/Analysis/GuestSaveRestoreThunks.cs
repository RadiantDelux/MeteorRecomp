...
public sealed class GuestSaveRestoreThunks
{
    ...
    /// </summary>
    private static readonly AsyncLocal<GuestSaveRestoreThunks?> ScopedCurrent = new();
    private static GuestSaveRestoreThunks processDefault = None;
    public static GuestSaveRestoreThunks Current
    {
        get => ScopedCurrent.Value ?? processDefault;
        set => processDefault = value ?? throw new ArgumentNullException(nameof(value));
    }

    /// <summary>
    /// Selects a project for this execution context, including parallel workers,
    /// without changing another concurrent translation's thunk addresses.
    /// Dispose scopes in reverse order on their owning execution context.
    /// </summary>
    public static IDisposable Use(GuestSaveRestoreThunks thunks)
    {
        ArgumentNullException.ThrowIfNull(thunks);
        var scope = new Scope(ScopedCurrent.Value);
        ScopedCurrent.Value = thunks;
        return scope;
    }

    private sealed class Scope(GuestSaveRestoreThunks? previous) : IDisposable
    {
