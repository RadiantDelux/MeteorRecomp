...
public static partial class GeneratedMarkers
{
    ...
    /// <summary>
    /// <c>REGISTER_NATIVE_FUNCTION(0x…, symbol)</c>, title-scoped
    /// <c>REGISTER_TITLE_NATIVE_FUNCTION</c>, and their <c>_AS</c> alias forms;
    /// the <c>as</c> group tells the alias forms apart.
    /// </summary>
    [GeneratedRegex(@"REGISTER_(?<title>TITLE_)?NATIVE_FUNCTION(?<as>_AS)?\s*\(\s*0x(?<address>[0-9A-Fa-f]{8})\s*,\s*(?<symbol>[A-Za-z_][A-Za-z0-9_]*)", RegexOptions.CultureInvariant)]
