...
public static partial class TranslatedBuildShardEmitter
{
    ...
    public static TranslatedBuildShardResult Emit(TranslatedBuildShardOptions options)
    {
        ...
        var nativeOverrides = ReadNativeOverrides(
            options.NativeSourceDirectory, options.NativeIndex, options.GenericDolBoot);
    ...
    private static NativeOverrideSets ReadNativeOverrides(
        ...
        bool genericDolBoot = false)
    {
        var registrations = (index ?? RuntimeNativeIndexBuilder.Build(directory, genericDolBoot)).Registrations;
        if (genericDolBoot)
            registrations = registrations.Where(static registration => registration.IsTitleScoped).ToArray();
        ...
        ShardBoundaryTable? frozenBoundaries = null)
    {
        ...
        Parallel.For(0, groups.Count, index =>
        {
            ...
            source.AppendLine("#define MKW_STATIC_TRANSLATED_CALL(Target, Entry, Context) do { InvokeStaticTranslatedCpu<Target, Entry>(Context); } while (false)");
