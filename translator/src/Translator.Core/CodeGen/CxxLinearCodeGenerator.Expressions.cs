...
public sealed partial class CxxLinearCodeGenerator
{
    ...
    {
        if (val.Kind == "dynamic_address" && val.Constant.HasValue)
        {
            return $"DynamicModule::RebaseCanonicalAddress(ctx, 0x{unchecked((uint)val.Constant.Value):X8}u)";
        }
    ...
    {
        if (remap is { IsDynamic: true })
        {
            // This value is being consumed as a guest memory address, so it is
            // semantically safe to map canonical module/BSS address segments at
            // the point of dereference. This is intentionally not an integer
            // value transform.
            return $"DynamicModule::RebaseCanonicalAddress(ctx, static_cast<uint32_t>({Address(addr, types)}))";
        }
    ...
    private static string RemapAddressValue(
        IrValue value,
        RepresentationEnvironment types,
        LinkedAddressRemap? remap)
    {
        var expression = ToExpression(value, types);
        return remap is { IsDynamic: true }
            ? $"DynamicModule::RebaseCanonicalAddress(ctx, static_cast<uint32_t>({expression}))"
            : expression;
    }
