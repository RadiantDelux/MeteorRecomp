...
public sealed partial class CxxLinearCodeGenerator
{
            ...
            case IrAssign assign:
                {
                    ...
                    var src = linkedAddressRemap is { IsDynamic: true } &&
                              GetRegisterBaseName(assign.Destination).Equals("lr", StringComparison.OrdinalIgnoreCase) &&
                              assign.Value.Kind == "const" && assign.Value.Constant.HasValue &&
                              unchecked((uint)assign.Value.Constant.Value) >= linkedAddressRemap.LinkBase &&
                              unchecked((uint)assign.Value.Constant.Value) <= checked(linkedAddressRemap.LinkBase + linkedAddressRemap.CodeSize)
                        ? $"DynamicModule::RuntimeAddressFromOffset(ctx, 0x{unchecked((uint)assign.Value.Constant.Value) - linkedAddressRemap.LinkBase:X8}u)"
                        : ToRegisterAssignmentExpression(assign.Destination, assign.Value, types);
                    ...
                    if (IsFloatRegister(assign.Destination))
                    {
                            ...
                            IsFloatRegister(assign.Value.RegisterName))
                        {
                            ...
                            paired = PairedState(localPaired, srcBase);
                            if (paired is null)
                            {
                                var sourceBit = ParseFloatRegisterIndex(srcBase);
                                var destinationBit = ParseFloatRegisterIndex(assign.Destination);
                                if (sourceBit != destinationBit)
                                    sb.AppendLine($"{pad}mkw_paired_state = (mkw_paired_state & ~0x{1u << destinationBit:X8}u) | (((mkw_paired_state >> {sourceBit}) & 1u) << {destinationBit});");
                            }
                        }
            ...
            case IrStore store:
                {
                    ...
                    {
                        srcExpr = ToScalarFloatExpression(store.Source, types, localPaired);
                    }
            ...
            case IrResolvedPsqLoad load:
                {
                    ...
                    var address = RemapAddressValue(load.OriginalAddress, types, linkedAddressRemap);
            ...
            case IrResolvedPsqStore store:
                {
                    ...
                    var address = RemapAddressValue(store.OriginalAddress, types, linkedAddressRemap);
            ...
            case IrResolvedStorePair pair:
                {
                    ...
                    string Source(IrValue source)
                    {
                        ...
                        if (pair.ElementSizeBytes == 4 && source.RegisterName is { } name && IsFloatRegister(name))
                            expression = ToScalarFloatExpression(source, types, localPaired);
                        return expression;
                    }
            ...
            case IrCall call:
                if (TryParseAddress(call.Target, out var addr))
                {
                    ...
                    if (usesStateFreeCall)
                    {
                        ...
                        if (linkedAddressRemap is { IsDynamic: true } && addr >= linkedAddressRemap.LinkBase && addr < checked(linkedAddressRemap.LinkBase + linkedAddressRemap.CodeSize))
                            sb.AppendLine($"{pad}    InvokeCurrentDynamicModuleFunction(0x{addr - linkedAddressRemap.LinkBase:X8}u, ctx);");
                        else
                    ...
                    if (resumesThroughLinkRegister)
                    {
                        ...
                        {
                            var lrCompare = linkedAddressRemap is { IsDynamic: true }
                                ? "DynamicModule::CanonicalLinkAddressFromRuntime(ctx, ctx->lr)"
                                : "ctx->lr";
                            sb.AppendLine($"{pad}if ({lrCompare} != 0x{localFallthroughLr.Value:X8}u) {{");
                            fallbackPad = IndentPad(indent + 1);
                        }

                        EmitLocalLrContinuationDispatch(sb, fallbackPad, labelNames, linkedAddressRemap);
                        sb.AppendLine($"{fallbackPad}if (CanDispatchGuestTarget(ctx->lr)) {{");
                ...
                else
                {
                    ...
                    if (TryEmitInlinePpc(call, sb, pad, types, localPaired, stackFacts, linkedAddressRemap))
                    {
                        ...
                        {
                            bool? paired = IsPairedProducerTarget(call.Target);
                            if (call.Target.Equals("PPC_PsMr", StringComparison.OrdinalIgnoreCase) &&
                                call.Arguments.Count == 1)
                            {
                                // The inline move preserves bits. Preserve their
                                // interpretation too, without rounding scalar doubles.
                                paired = IsPairedFloatValue(call.Arguments[0], localPaired);
                                if (paired is null)
                                {
                                    var sourceBit = ParseFloatRegisterIndex(call.Arguments[0].RegisterName!);
                                    var destinationBit = ParseFloatRegisterIndex(call.Destination);
                                    if (sourceBit != destinationBit)
                                        sb.AppendLine($"{pad}mkw_paired_state = (mkw_paired_state & ~0x{1u << destinationBit:X8}u) | (((mkw_paired_state >> {sourceBit}) & 1u) << {destinationBit});");
                                }
                            }
                            localPaired[GetRegisterBaseName(call.Destination)] = paired;
                        }
                        break;
                    }
                    ...
                    for (var argIndex = 0; argIndex < call.Arguments.Count; argIndex++)
                    {
                        ...
                        var isPsqMemoryAddress = argIndex == 0 &&
                            (call.Target.Equals("PPC_PsqL", StringComparison.OrdinalIgnoreCase) ||
                             call.Target.Equals("PPC_PsqSt", StringComparison.OrdinalIgnoreCase) ||
                        ...
                        var expr = isPsqMemoryAddress
                            ? RemapAddressValue(argVal, types, linkedAddressRemap)
                            : isPairedConsumer
        ...
        LinkedAddressRemap? linkedAddressRemap)
    {
        ...
        var selector = linkedAddressRemap is { IsDynamic: true }
            ? "DynamicModule::CanonicalLinkAddressFromRuntime(ctx, ctx->lr)"
            : "ctx->lr";
        sb.AppendLine($"{pad}switch ({selector}) {{");
        ...
        IGuestFunctionAbiProvider guestAbiProvider)
    {
        foreach (var arg in arguments)
        {
            ...
            var scalar = $"PPC_PsToScalarInline(ctx->fpr[{regIndex}].d)";
            if (isPaired is null)
                scalar = $"({RuntimePairedTest(baseName)} ? {scalar} : ctx->fpr[{regIndex}].d)";
            sb.AppendLine($"{pad}ctx->fpr[{regIndex}].d = {scalar};");
