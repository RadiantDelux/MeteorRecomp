...
public sealed partial class CxxLinearCodeGenerator
{
    ...
    {
        if (dynamicModuleTemplate is not null && emitModRegistration)
            throw new InvalidOperationException("Dynamic module templates cannot emit fixed mod registrations.");
        if (dynamicModuleTemplate is not null && emitStateFreeLeafVariant)
            throw new InvalidOperationException("Dynamic module templates do not support state-free ABI emission until instance state is explicit in that ABI.");
        ...
        var cxxName = dynamicModuleTemplate is null
            ? SanitizeIdentifier(signature.Name)
            : $"dynmod_{SanitizeIdentifier(dynamicModuleTemplate.TemplateId)}_{checked(entryPoint - dynamicModuleTemplate.CanonicalImageBase):X8}";
        var pairedFlow = ComputePairedFlowStates(func, cfg, _guestAbiProvider);
        var pairedIn = pairedFlow.In;
        var pairedOut = pairedFlow.Out;
        var hasMixedPairedFlow = pairedIn.Values.Any(states => states.Values.Any(state => state is null));
        ...
        var blockOrder = func.Blocks
            .Select((block, index) => (block.Label, index))
            .ToDictionary(static item => item.Label, static item => item.index, StringComparer.OrdinalIgnoreCase);
        ...
        var linkedAddressRemap = dynamicModuleTemplate is not null
            ? new LinkedAddressRemap(
                dynamicModuleTemplate.CanonicalImageBase,
                0,
                dynamicModuleTemplate.ImageSize,
                IsDynamic: true,
                dynamicModuleTemplate.CanonicalBssBase,
                dynamicModuleTemplate.BssSize)
            : moduleLinkBase.HasValue && moduleGuestBase.HasValue && moduleLinkedCodeSize != 0
        ...
        string EmitFunctionBody(
            RegisterResidency? residency,
            int bufferBaseLength,
            bool discardResult = false,
            bool emitRuntimeLoopCheckpoints = true)
        {
            ...
            {
                body.AppendLine("{");
                if (hasMixedPairedFlow)
                    body.AppendLine("    uint32_t mkw_paired_state = 0u;");
                ...
                bool IsBackedge(string fromLabel, string toLabel) =>
                    blockOrder.TryGetValue(fromLabel, out var fromIndex) &&
                    blockOrder.TryGetValue(toLabel, out var toIndex) &&
                    toIndex <= fromIndex;

                void EmitRuntimeLoopCheckpoint(
                    StringBuilder destination,
                    string pad,
                    string fromLabel,
                    string toLabel)
                {
                    if (!emitRuntimeLoopCheckpoints || !IsBackedge(fromLabel, toLabel))
                        return;

                    var guestPc = TryParseAddress(toLabel, out var parsedAddress)
                        ? parsedAddress
                        : entryPoint;
                    destination.AppendLine($"{pad}if (ShouldApplyRuntimeLoopOptions(0x{guestPc:X8}u)) {{");
                    AppendFlush(destination, pad + "    ");
                    destination.AppendLine($"{pad}    ApplyRuntimeLoopOptions(0x{guestPc:X8}u, ctx);");
                    AppendReload(destination, pad + "    ");
                    destination.AppendLine($"{pad}}}");
                }
                ...
                foreach (var block in func.Blocks)
                {
                    ...
                    {
                        foreach (var (reg, state) in pairedAtEntry)
                        {
                            localPaired[reg] = state;
                        }
                    }
                    ...
                    switch (term)
                    {
                        ...
                        case IrJump jump:
                            EmitRuntimeLoopCheckpoint(body, "    ", block.Label, jump.TargetLabel);
                        ...
                        case IrBranch br:
                            ...
                            EmitRuntimeLoopCheckpoint(falseEdge, "        ", block.Label, br.FalseLabel);
                            ...
                            EmitRuntimeLoopCheckpoint(body, "        ", block.Label, br.TrueLabel);
                        ...
                        case IrJumpTable table:
                            ...
                            var jumpSelector = linkedAddressRemap is { IsDynamic: true }
                                ? $"DynamicModule::CanonicalAddressFromRuntime(ctx, static_cast<uint32_t>({selectorExpr}))"
                                : $"static_cast<uint32_t>({selectorExpr})";
                            body.AppendLine($"    switch ({jumpSelector}) {{");
                            foreach (var jt in table.Cases)
                            {
                                ...
                                EmitRuntimeLoopCheckpoint(body, "        ", block.Label, jt.TargetLabel);
        ...
        RegisterResidency? residency = null;
        {
            ...
            EmitFunctionBody(
                recorder,
                bodyBufferBaseLength,
                discardResult: true,
                emitRuntimeLoopCheckpoints: !emitStateFreeLeafVariant);
        ...
        sb.AppendLine(EmitFunctionBody(
            residencyOwnsPrimaryBody ? residency : null,
            bodyBufferBaseLength,
            emitRuntimeLoopCheckpoints: !emitStateFreeLeafVariant));

        var aliasName = $"func_{entryPoint:X8}";
        if (dynamicModuleTemplate is null && !emitModRegistration && !cxxName.Equals(aliasName, StringComparison.OrdinalIgnoreCase))
        {
            sb.AppendLine();
        ...
        CxxDynamicDispatchFacts? dynamicDispatch = null;
        if (dynamicModuleTemplate is not null)
        {
            // Dynamic template functions are emitted into a separate
            // (templateId, offset) namespace. Canonical addresses are never
            // inserted into base/mod registration markers or fixed dispatch.
            dynamicDispatch = new CxxDynamicDispatchFacts(
                cxxName,
                PreserveNonvolatileGprs: false,
                nonvolatileFprWriteMask);
        }
        else if (emitModRegistration)
        {
    ...
    private sealed record LinkedAddressRemap(
        uint LinkBase,
        uint GuestBase,
        uint CodeSize,
        bool IsDynamic = false,
        uint CanonicalBssBase = 0,
        uint BssSize = 0);
    ...
    private static void EmitNormalizePairedStateOnEdge(
        ...
        string fromLabel,
        string toLabel,
        IReadOnlyDictionary<string, Dictionary<string, bool?>> pairedOut,
        IReadOnlyDictionary<string, Dictionary<string, bool?>> pairedIn,
        IReadOnlyDictionary<string, HashSet<string>> liveIn)
    {
        ...
        fromSet ??= new Dictionary<string, bool?>(StringComparer.OrdinalIgnoreCase);
        toSet ??= new Dictionary<string, bool?>(StringComparer.OrdinalIgnoreCase);

        var candidateRegs = new HashSet<string>(fromSet.Keys, StringComparer.OrdinalIgnoreCase);
        candidateRegs.UnionWith(toSet.Keys);
        ...
        {
            var fromPaired = PairedState(fromSet, reg);
            var toPaired = PairedState(toSet, reg);
            ...
            if (toPaired is null)
            {
                // Preserve the payload verbatim at a mixed join. Record its
                // representation instead of losing a lane or double precision.
                sb.AppendLine(fromPaired == true
                    ? $"{pad}mkw_paired_state |= 0x{1u << regIndex:X8}u;"
                    : $"{pad}mkw_paired_state &= ~0x{1u << regIndex:X8}u;");
                continue;
            }
            ...
            var converted = toPaired == true
                ? $"PPC_PsFromScalarInline({regExpr})"
                : $"PPC_PsToScalarInline({regExpr})";
            if (fromPaired is null)
                converted = toPaired == true
                    ? $"({RuntimePairedTest(reg)} ? {regExpr} : {converted})"
                    : $"({RuntimePairedTest(reg)} ? {converted} : {regExpr})";
            sb.AppendLine($"{pad}{regExpr} = {converted};");
        }
    }
