...
public sealed partial class CxxLinearCodeGenerator
{
        ...
        IReadOnlyDictionary<string, bool?> localPaired)
    {
        ...
        return PairedState(localPaired, register);
    }
        ...
        IReadOnlyDictionary<string, bool?> localPaired)
    {
        ...
        return IsPairedFloatValue(value, localPaired) switch
        {
            true => $"PPC_PsToScalarInline({expression})",
            false => expression,
            null => $"({RuntimePairedTest(value.RegisterName!)} ? PPC_PsToScalarInline({expression}) : {expression})"
        };
    }
        ...
        bool fpscrNiDisabled = false)
    {
        ...
        return IsPairedFloatValue(value, localPaired) switch
        {
            true => expression,
            false => converted,
            null => $"({RuntimePairedTest(value.RegisterName!)} ? {expression} : {converted})"
        };
    }
