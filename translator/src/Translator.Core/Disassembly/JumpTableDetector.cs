...
internal static class JumpTableDetector
{
    ...
    // A function can hoist a stable nonvolatile table-base register into its
    // prologue and reuse it much later.  Keep the broader search confined to
    // resolving that absolute base; selector/bound searches remain deliberately
    // short so stale compares are not mistaken for the active switch bound.
    private const int MaxTableBaseBacktrackInstructions = 512;
    // Retail CodeWarrior sometimes emits very large dense state-machine
    // switches.  RDSPAF's Bluetooth/HID path has a 1160-entry table; the old
    // 512-entry guard rejected it even though every decoded target is still
    // range-checked below.  Keep a finite defensive cap, but large enough for
    // those real SDK/game state machines.
    ...
    private static bool TryResolveLisAddiConstant(IReadOnlyList<PpcInstruction> ordered, int startIndex, string register, out uint value)
    {
        ...
        for (var i = startIndex - 1; i >= 0 && startIndex - i <= MaxTableBaseBacktrackInstructions; i--)
        {
            var ins = ordered[i];
            if (ins.Operands.Count == 0 || ins.Operands[0] is not PpcRegisterOperand dest)
            {
    ...
    {
        // lwzx normally consumes a byte offset, not the logical switch selector:
        //
        //   cmplwi r4, 14
        //   ...
        //   rlwinm r0, r4, 2, 0, 29   ; r0 = selector * 4
        //   lwzx   r3, table, r0
        //
        // Looking backwards for a compare on r0 first is unsafe because volatile
        // temporaries are routinely reused.  A stale earlier `cmpwi r0, 1` made
        // the detector truncate a real 15-entry switch table to two entries.
        // Trace the immediate index-producing shift and prefer the compare on its
        // source selector before considering the legacy heuristics below.
        if (TryTraceScaledIndexSource(ordered, startIndex, register, out var selectorRegister, out var definitionIndex) &&
            TryFindRegisterUpperBound(ordered, definitionIndex, selectorRegister, out upperBound))
        {
            return true;
        }

        // Preferred: compare directly on the same register used for lwzx indexing.
        if (TryFindRegisterUpperBound(ordered, startIndex, register, out upperBound))
        {
            return true;
        }
    ...
    private static bool TryFindRegisterUpperBound(
        IReadOnlyList<PpcInstruction> ordered,
        int startIndex,
        string register,
        out int upperBound)
    {
        for (var i = startIndex - 1; i >= 0 && startIndex - i <= MaxBacktrackInstructions; i--)
        {
            ...
            if (ins.Operands.Count < 2 ||
                ins.Operands[0] is not PpcRegisterOperand reg ||
                !RegistersEqual(reg.Name, register) ||
    ...
    private static bool TryTraceScaledIndexSource(
        IReadOnlyList<PpcInstruction> ordered,
        int startIndex,
        string indexRegister,
        out string selectorRegister,
        out int definitionIndex)
    {
        for (var i = startIndex - 1; i >= 0 && startIndex - i <= MaxBacktrackInstructions; i--)
        {
            ...
            if (ins.Operands.Count == 0 ||
                ins.Operands[0] is not PpcRegisterOperand dest ||
                !RegistersEqual(dest.Name, indexRegister))
            {
                continue;
            }

            // The decoder emits rlwinm for PPC shift aliases; synthetic tests
            // and imported maps may use the canonical slwi spelling.
            if (ins.Mnemonic is "rlwinm" or "rlwinm." or "slwi" &&
                ins.Operands.Count >= 2 &&
                ins.Operands[1] is PpcRegisterOperand source)
            {
                selectorRegister = source.Name;
                definitionIndex = i;
                return true;
            }

            // This is the reaching definition of the lwzx index, but it is not
            // a scale transform we can reason about.  Do not walk through an
            // older lifetime of the same volatile register.
            break;
        }

        selectorRegister = string.Empty;
        definitionIndex = -1;
        return false;
    }
