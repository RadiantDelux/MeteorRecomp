...
TextureHandle new_dynamic_texture_2d_impl(uint32_t width, uint32_t height, uint32_t mips, u32 gxFormat,
                                          const char* label, StaticTextureTiming* timing) noexcept {
  ZoneScopedS(3);
...
TextureHandle new_dynamic_texture_2d(uint32_t width, uint32_t height, uint32_t mips, u32 gxFormat,
  ...
  return new_dynamic_texture_2d_impl(width, height, mips, gxFormat, label, nullptr);
}
...
void write_tlut_texture(TextureRef& ref, ArrayRef<uint8_t> data) noexcept {
  ZoneScoped;
  CHECK(ref.size.height == 1, "write_tlut_texture: expected height 1, got {}", ref.size.height);
  CHECK(ref.mipCount == 1, "write_tlut_texture: expected mipCount 1, got {}", ref.mipCount);

  auto converted = convert_tlut(ref.gxFormat, ref.size.width, data);
  if (converted.data.empty()) {
    return;
  }
  data = converted.data;
  ...
  if (dataSize > data.size()) {
    Log.warn("write_tlut_texture: expected at least {} bytes, got {}", dataSize, data.size());
    return;
  }
  ...
  if constexpr (UseTextureBuffer) {
    const auto range = push_texture_data(data.data(), dataSize, bytesPerRow, heightBlocks);
    const wgpu::TexelCopyBufferLayout dataLayout{
        .offset = range.offset,
        .bytesPerRow = bytesPerRow,
        .rowsPerImage = heightBlocks,
    };
    g_textureUploads.emplace_back(dataLayout, dstView, physicalSize);
  } else {
    const wgpu::TexelCopyBufferLayout dataLayout{
        .bytesPerRow = bytesPerRow,
        .rowsPerImage = heightBlocks,
    };
    g_queue.WriteTexture(&dstView, data.data(), dataSize, &dataLayout, &physicalSize);
  }
}
} // namespace aurora::gfx
