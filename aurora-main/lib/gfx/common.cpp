...
constexpr uint32_t DefaultBindGroupCacheRetainFrames = 32;
constexpr uint32_t DefaultBindGroupCacheSweepPeriod = 16;

} // namespace

static absl::flat_hash_map<BindGroupRef, CachedBindGroup> g_cachedBindGroups;
static uint32_t g_bindGroupCacheRetainFrames = DefaultBindGroupCacheRetainFrames;
static uint32_t g_bindGroupCacheSweepPeriod = DefaultBindGroupCacheSweepPeriod;
...
bool can_push_verts(size_t size) noexcept {
  return size <= VertexBufferSize && g_verts.size() <= VertexBufferSize - size;
}
...
void expire_bind_group_cache(uint32_t logicalFrame) noexcept {
  if (g_cachedBindGroups.empty() || g_frameIndex == UINT32_MAX ||
      g_frameIndex % g_bindGroupCacheSweepPeriod != 0) {
    return;
  }
...
void clear_completed_bind_group_cache() noexcept {
  g_cachedBindGroups.clear();
}

void set_bind_group_cache_policy(uint32_t retainFrames, uint32_t sweepPeriod) noexcept {
  // One frame is the minimum lifetime with the asynchronous frame worker: the
  // immediately previous sealed frame may still reference these handles while
  // the producer records the next. Zero-period modulo would also be invalid.
  g_bindGroupCacheRetainFrames = std::max(retainFrames, 1u);
  g_bindGroupCacheSweepPeriod = std::max(sweepPeriod, 1u);
}
...
void render(wgpu::CommandEncoder& cmd, int32_t interpolatedFrame, bool finalize) {
  ...
  if (finalize) {
    ...
    expire_bind_group_cache(logicalFrame);
  }
}

void after_submit(uint32_t logicalFrame) noexcept {
...
size_t vertex_bytes_used() noexcept { return g_verts.size(); }
Range push_indices(const uint8_t* data, size_t length) { return push(g_indices, data, length, 0); }
size_t index_bytes_used() noexcept { return g_indices.size(); }
...
size_t storage_bytes_used() noexcept { return g_storage.size(); }
size_t storage_allocation_size(size_t length) noexcept {
  const size_t alignment = static_cast<size_t>(g_cachedLimits.minStorageBufferOffsetAlignment);
  if (length == 0) {
    return alignment;
  }
  if (alignment == 0) {
