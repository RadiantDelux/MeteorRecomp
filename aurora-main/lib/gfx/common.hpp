...
private:
  ...
  void resize(size_t size, bool zeroed) {
    ...
    if (size > m_capacity) {
      if (!m_owned) {
        std::fprintf(stderr,
                     "[aurora][gfx] fixed staging buffer overflow: current=%zu capacity=%zu requested=%zu\n",
...
void expire_bind_group_cache(uint32_t logicalFrame = UINT32_MAX) noexcept;
...
void after_submit(uint32_t logicalFrame = UINT32_MAX) noexcept;
...
// The active staging slices are fixed-size mapped ranges. Callers that can
// produce an arbitrarily long logical GX frame must rotate the recording batch
// before appending past these capacities; ByteBuffer deliberately aborts rather
// than reallocating a borrowed WebGPU mapping.
size_t vertex_bytes_used() noexcept;
constexpr size_t vertex_capacity() noexcept { return static_cast<size_t>(VertexBufferSize); }
template <typename T>
...
size_t index_bytes_used() noexcept;
constexpr size_t index_capacity() noexcept { return static_cast<size_t>(IndexBufferSize); }
template <typename T>
...
// Bytes currently consumed by indexed-vertex array snapshots in the active
// staging batch. The command processor uses these to rotate the staging batch
// before a draw would exceed the fixed WebGPU storage slice.
size_t storage_bytes_used() noexcept;
size_t storage_allocation_size(size_t length) noexcept;
constexpr size_t storage_capacity() noexcept { return static_cast<size_t>(StorageBufferSize); }
template <typename T>
