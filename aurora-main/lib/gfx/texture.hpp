...
struct GXTexObj_ {
  ...
  // A texture configured through raw BP registers refers to its palette by
  // physical TMEM location, not by the SDK's logical GX_TLUTn object index.
  // UINT16_MAX means this texture came through the ordinary SDK metadata path.
