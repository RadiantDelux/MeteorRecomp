...
// The guest drives its own pacing, so never require a blocking FIFO present mode: coupling
// Present() to the monitor can slow the whole machine when guest cadence exceeds host refresh.
// Prefer Mailbox whenever the backend exposes it. It remains non-blocking for the producer while
// synchronizing scanout to the host compositor/vblank and dropping superseded images instead of
// tearing them. This matters in particular when a correctly paced 50 Hz VI is displayed on a
// 60 Hz desktop: Immediate adds an avoidable unsynchronised scanout artifact on top of the
// unavoidable 50->60 cadence conversion.
...
bool initialize(AuroraBackend auroraBackend) {
  ...
  uint32_t maxTextureDimension2D = 0;
  {
    ...
    bool depthClipControlSupported = false;
    ...
    for (size_t i = 0; i < supportedFeatures.featureCount; ++i) {
      ...
      if (feature == wgpu::FeatureName::DepthClipControl) {
        depthClipControlSupported = true;
      }
    }
    Log.info("DepthClipControl adapter support: {} (diagnostic only; not requested/enabled)",
             depthClipControlSupported);
