...
// Reuse retired copy targets to avoid repeated large D3D12 allocations.  The
// guest destination is logical identity only; it does not change the physical
// texture descriptor.  Pool by the exact physical descriptor so unrelated RAM
// destinations can reuse the same 8x allocation once no recorded draw owns it.
...
// Three allocations per descriptor are enough for the producer/worker overlap.
// A global byte budget prevents high-resolution scenes from retaining gigabytes
// of idle targets. Active references are never evicted.
constexpr size_t kCopyTexturePoolPerDescriptor = 3;
constexpr uint64_t kCopyTexturePoolByteBudget = 512ull * 1024ull * 1024ull;
uint64_t g_copyTexturePoolSerial = 0;

uint64_t copy_texture_bytes(const aurora::gfx::TextureHandle& handle) {
  if (!handle) {
    return 0;
  }
  return aurora::gfx::calc_texture_size(handle->format, handle->size.width, handle->size.height, 1);
}

void trim_copy_texture_pool(uint64_t incomingBytes) {
  auto pooledBytes = [] {
    uint64_t total = 0;
  ...
  while (!g_copyTexturePool.empty() && pooledBytes + incomingBytes > kCopyTexturePoolByteBudget) {
    auto victim = g_copyTexturePool.end();
    for (auto it = g_copyTexturePool.begin(); it != g_copyTexturePool.end(); ++it) {
      if (!it->handle || it->handle.use_count() != 1) {
        continue;
      }
      if (victim == g_copyTexturePool.end() || it->lastUseSerial < victim->lastUseSerial) {
        victim = it;
      }
    }
    if (victim == g_copyTexturePool.end()) {
      break;
    }
    pooledBytes = victim->bytes <= pooledBytes ? pooledBytes - victim->bytes : 0;
    g_copyTexturePool.erase(victim);
  }
}
...
aurora::gfx::TextureHandle acquire_copy_texture(const aurora::gx::GXState::CopyTextureKey& key, u32 width, u32 height,
  ...
  for (auto& entry : g_copyTexturePool) {
    if (entry.scaledWidth != width || entry.scaledHeight != height || entry.format != texCopyFmt) {
      continue;
    }
    ++sameDescriptor;
    if (entry.handle && entry.handle.use_count() == 1) {
      aurora::gx::invalidate_dynamic_palette_source(entry.handle.get());
      entry.lastUseSerial = ++g_copyTexturePoolSerial;
      return entry.handle;
    }
  }

  auto handle = create_copy_texture(width, height, texCopyFmt);
  if (sameDescriptor < kCopyTexturePoolPerDescriptor) {
    const uint64_t bytes = copy_texture_bytes(handle);
    trim_copy_texture_pool(bytes);
    g_copyTexturePool.push_back({width, height, texCopyFmt, handle, ++g_copyTexturePoolSerial, bytes});
  }
  return handle;
}
...
void prune_copy_texture_pool(const void* dest) noexcept {
  if (dest == nullptr) {
    ...
    g_copyTexturePoolSerial = 0;
  }
  // Logical guest destination eviction does not invalidate a retired physical
  // allocation. It can be overwritten and recycled for a later copy.
}
} // namespace aurora::gx
...
void GXSetTexCopyDst(u16 wd, u16 ht, GXTexFmt fmt, GXBool mipmap) {
  // The SDK API receives the logical GXTexFmt/GX_CTF_* value. The BP bit
  // permutation is only a wire-format concern and is decoded in handle_bp().
...
void GXCopyTex(void* dest, GXBool clear) {
  ...
  static u32 loggedCopyDestinations = 0;
  if (loggedCopyDestinations++ < 16) {
    std::fprintf(stderr,
                 "[gx] EFB copy destination=%p logical=%ux%u scaled=%ux%u format=0x%X "
                 "tile_row_pitch=%u bytes=%u clear=%u\n",
                 dest, logicalDstWidth, logicalDstHeight, scaledDstWidth, scaledDstHeight,
                 static_cast<u32>(texCopyFmt), (g_gxState.bpRegCache[0x4D] & 0x3ffu) << 5,
                 GXGetTexBufferSize(static_cast<u16>(logicalDstWidth), static_cast<u16>(logicalDstHeight),
                                    texCopyFmt, GX_FALSE, 0), static_cast<u32>(clear));
  }
