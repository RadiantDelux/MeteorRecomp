...
void GXSetChanAmbColor(GXChannelID id, GXColor color) {
  ...
  const u32 incoming = (static_cast<u32>(color.r) << 24) | (static_cast<u32>(color.g) << 16) |
                       (static_cast<u32>(color.b) << 8) | static_cast<u32>(color.a);
  const u32 chan = (id == GX_COLOR0 || id == GX_ALPHA0) ? 0u : 1u;
  const bool alphaOnly = (id == GX_ALPHA0 || id == GX_ALPHA1);
  const u32 old = __gx->ambColor[chan];
  // COLORn and ALPHAn address different components of the same XF RGBA
  // register. A COLOR write must preserve the current alpha; an ALPHA write
  // must preserve the current RGB. COLORnAn reaches here as two component
  // writes above, producing the complete requested RGBA value.
  const u32 packed = alphaOnly ? ((old & 0xFFFFFF00u) | (incoming & 0x000000FFu))
                               : ((incoming & 0xFFFFFF00u) | (old & 0x000000FFu));
  __gx->ambColor[chan] = packed;
  GX_WRITE_XF_REG(0xA + chan, packed);
  __gx->bpSent = 0;
}
...
void GXSetChanMatColor(GXChannelID id, GXColor color) {
  ...
  const u32 chan = (id == GX_COLOR0 || id == GX_ALPHA0) ? 0u : 1u;
  const bool alphaOnly = (id == GX_ALPHA0 || id == GX_ALPHA1);
  const u32 old = __gx->matColor[chan];
  const u32 packed = alphaOnly ? ((old & 0xFFFFFF00u) | (incoming & 0x000000FFu))
                               : ((incoming & 0xFFFFFF00u) | (old & 0x000000FFu));
  __gx->matColor[chan] = packed;
  GX_WRITE_XF_REG(0xC + chan, packed);
  __gx->bpSent = 0;
}
