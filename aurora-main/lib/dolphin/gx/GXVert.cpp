...
void GXAbortPendingPrimitive() {
  // Do not call GXEnd here: an abort is specifically allowed to terminate an
  // incomplete primitive, and GXEnd's byte-count validation assumes the whole
  // declared vertex payload arrived.  Reset only the writer bookkeeping; the
  // FIFO owner drops the corresponding unsent bytes separately.
  sBeginNVerts = 0;
  sBeginFifoSize = 0;
  sBeginPrimitive = GX_TRIANGLES;
  sBeginVtxFmt = GX_VTXFMT0;
  sInBegin = false;
  sWriteAttr = GX_VA_NULL;
  sWriteAttrComps = 0;
}
