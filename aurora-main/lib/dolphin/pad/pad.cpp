...
u32 PADRead(PADStatus* status) {
  ...
  for (uint32_t i = 0; i < PAD_CHANMAX; ++i) {
    ...
    if (g_keyboardBindings[i].m_mappingsSet) {
      std::ranges::for_each(
          g_keyboardBindings[i].m_buttonMapping, [&kbState, &numKeys, &i, &status](const PADKeyButtonBinding& mapping) {
            if (mapping.scancode > PAD_KEY_INVALID && mapping.scancode < numKeys && kbState != nullptr &&
                kbState[mapping.scancode]) {
    ...
    if (controller) {
      ...
      // A keyboard profile and a physical controller may intentionally share a
      // port. Buttons have always been merged above; keep the analog sources
      // additive too instead of silently replacing keyboard axes here.
      status[i].stickX = static_cast<int8_t>(
          std::clamp(static_cast<int>(status[i].stickX) + static_cast<int>(xl), -127, 127));
      status[i].stickY = static_cast<int8_t>(
          std::clamp(static_cast<int>(status[i].stickY) + static_cast<int>(yl), -127, 127));
      ...
      status[i].substickX = static_cast<int8_t>(
          std::clamp(static_cast<int>(status[i].substickX) + static_cast<int>(xr), -127, 127));
      status[i].substickY = static_cast<int8_t>(
          std::clamp(static_cast<int>(status[i].substickY) + static_cast<int>(yr), -127, 127));
      ...
      status[i].triggerLeft = static_cast<u8>(
          std::clamp(static_cast<int>(status[i].triggerLeft) + static_cast<int>(tl), 0, 255));
      status[i].triggerRight = static_cast<u8>(
          std::clamp(static_cast<int>(status[i].triggerRight) + static_cast<int>(tr), 0, 255));
