...
struct DrawData {
  ...
  uint32_t dstAlpha;
  // Draw-opcode state is not represented by GXState::stateDirty. Keep it
  // beside the batch so merging cannot reuse another VAT or line shader.
  GXVtxFmt vertexFormat = GX_VTXFMT0;
  bool allowsTriangleMerge = false;
};
...
inline bool valid_pipeline_config(const PipelineConfig& config) noexcept {
  ...
  // GX draw pipelines always consume position in the vertex shader.  A cached
  // recipe with GX_VA_POS disabled can otherwise pass the enum-only checks and
  // reach build_shader(), where vtx_attr() correctly rejects the unmapped
  // mandatory position attribute with a fatal error during background prewarm.
  const bool hasPosition = config.shaderConfig.attrs[GX_VA_POS].attrType != GX_NONE;
