...
static uint32_t s_maxDrawsPerBatch = 0;

void set_max_draws_per_batch(uint32_t limit) {
  s_maxDrawsPerBatch = limit;
}
...
static bool copy_xf_data(u32 addr, const u8* data, u32 len, bool bigEndian) {
  constexpr u32 kPostexWords = MaxPostexMtx * 12;
  if (addr < kPostexWords) {
    // XF position/texture memory is word-addressable. A transfer can replace
    // one row, a 2x4 matrix in the position bank, or span adjacent matrices.
    // The SDK's usual 8/12-word packets are not a hardware restriction.
    XF_REQUIRE(len != 0 && len <= kPostexWords - addr,
               "XF: position/texture copy oob; addr={}, len={}", addr, len);
    for (u32 i = 0; i < len; i++) {
      const u32 word = addr + i;
      auto& mtx = word < 0x78 ? g_gxState.pnMtx[word / 12].pos
                             : g_gxState.texMtxs[(word - 0x78) / 12];
      f32* flat = reinterpret_cast<f32*>(&mtx);
      flat[word % 12] = read_f32(data + i * 4, bigEndian);
    }
    g_gxState.stateDirty = true;
    return true;
    ...
    // XF memory is word-addressable. Indexed normal-matrix loads commonly
    // update one 3-float row at a time (offsets 0/3/6) rather than all nine
    // words in one packet, and hardware preserves the words not covered by the
    // transfer. Accept any contiguous write that stays within this 3x3 matrix.
    XF_REQUIRE(len != 0 && startOffset + len <= 9,
               "XF: NrmMtx copy crosses matrix: offs={}, len={}", startOffset, len);
    ...
    XF_REQUIRE(len != 0 && len <= 0x5F0 - addr,
               "XF: post-transform copy oob; addr={}, len={}", addr, len);
    for (u32 i = 0; i < len; i++) {
      const u32 word = addr - 0x500 + i;
      auto& mtx = g_gxState.ptTexMtxs[word / 12];
      f32* flat = reinterpret_cast<f32*>(&mtx);
      flat[word % 12] = read_f32(data + i * 4, bigEndian);
    }
    g_gxState.stateDirty = true;
...
static bool handle_draw(u8 cmd, const u8* data, u32& pos, u32 size, bool bigEndian,
                        std::unique_lock<std::recursive_mutex>& gpuLock);
  ...
  while (pos < size) {
    ...
    case CP_CMD_INVAL_VTX: {
      ...
      for (int i = GX_VA_POS; i <= GX_VA_TEX7; ++i) {
        ...
        g_gxState.arrays[i].cachedSourceGeneration = kGuestWriteUntracked;
      }
      // A merged draw reuses the preceding draw's uniform array offsets. Once
      // the vertex cache is invalidated those offsets refer to old CPU data,
      // so the next draw must allocate fresh uploads even if bindings match.
      g_gxState.stateDirty = true;
      break;
    }
    ...
    default:
      ...
      if (is_draw_cmd(cmd)) {
        if (!handle_draw(cmd, data, pos, size, bigEndian, gpuLock)) {
          return;
        }
      } else {
...
static void handle_bp(u32 value, bool bigEndian) {
  ...
  if (regId == 0xFE) {
    ...
    // Copy triggers and texture invalidation are actions, not passive state.
    // A repeated trigger must latch a newly programmed source rectangle, and
    // a repeated invalidation must observe texels written since the last draw.
    if (g_gxState.bpRegCache[regId] == value && regId != 0x52 && regId != 0x65 && regId != 0x66) return;
  ...
  case 0x64:
    break;
  case 0x65: {
    const auto tmem = static_cast<u16>(bp_get(value, 10, 0));
    // LOADTLUT1 stores GXTlutSize units, where one unit is 16 16-bit
    // palette entries.  The field is 11 bits wide so GX_TLUT_16K (1024)
    // is representable.
    const auto sizeUnits = static_cast<u16>(bp_get(value, 11, 10));
    auto& raw = g_gxState.rawTluts[tmem];
    // BP 0x65 executes a RAM->TMEM transfer, but Aurora does not receive the
    // source RAM bytes until the embedding runtime calls GXSetDirectTlutData.
    // Stage the transaction metadata without mutating the committed TLUT so a
    // repeated load cannot transiently discard valid palette bytes or force a
    // cache-version bump before the transfer actually commits.
    raw.pendingLoadTlut0 = g_gxState.bpRegCache[0x64];
    raw.pendingNumEntries = static_cast<u16>(sizeUnits * 16u);
    raw.loadPending = true;
    break;
  }
  case 0x66:
    // Raw GXInvalidateTexAll/GXInvalidateTexRegion writes TX_INVALIDATE.
    // The source cache revalidates bytes after this revision, retaining uploads
    // whose content is unchanged. Do not depend on the SDK/HLE entry being used.
  ...
  case 0x4D: {
    // COPY_DST stores the XFB row pitch in 32-byte units.  An XFB pixel is
    // YUYV 16 bpp, so each hardware unit is exactly 16 pixels.  GXSetDispCopyDst
    // encodes floor(width / 16); do not add a rounding bias when decoding it or
    // the common 640-pixel pitch (0x28) becomes 655 pixels.
    g_gxState.dispCopyDstWidth = static_cast<u16>(bp_get(value, 10, 0) << 4);
    break;
  }
  case 0x4E: {
  ...
  case 0x52: {
    ...
    const bool copyToXfb = bp_get(value, 1, 14) != 0;
    const bool halfScale = bp_get(value, 1, 9) != 0;
    if (!copyToXfb) {
      g_gxState.texCopyFmt = decode_efb_copy_format(bp_get(value, 4, 3), bp_get(value, 1, 15) != 0,
                                                    bp_get(value, 1, 16) != 0);
      // 0x49/0x4A are shared hardware EFB-copy source registers. The SDK keeps
      // separate software shadows for display and texture copies, but a retail
      // FIFO trigger has no preceding host GXSetTexCopySrc/Dst call. Latch the
      // raw rectangle here, and derive the produced image dimensions from the
      // source and half-scale bit (0x4D is tile-row pitch, not image width).
      const u32 sourceXY = g_gxState.bpRegCache[0x49];
      const u32 sourceWH = g_gxState.bpRegCache[0x4A];
      const u32 width = bp_get(sourceWH, 10, 0) + 1u;
      const u32 height = bp_get(sourceWH, 10, 10) + 1u;
      g_gxState.texCopySrc = {static_cast<s32>(bp_get(sourceXY, 10, 0)),
                              static_cast<s32>(bp_get(sourceXY, 10, 10)),
                              static_cast<s32>(width), static_cast<s32>(height)};
      g_gxState.texCopySrcRenderSpace = false;
      g_gxState.texCopyDstWidth = static_cast<u16>(std::max(width >> halfScale, 1u));
      g_gxState.texCopyDstHeight = static_cast<u16>(std::max(height >> halfScale, 1u));
      static u32 loggedCopySetups = 0;
      if (loggedCopySetups++ < 12) {
        Log.warn("Raw EFB texture copy: physical=0x{:X} src=({},{} {}x{}) dst={}x{} fmt=0x{:X} stride={} half={}",
                 (g_gxState.bpRegCache[0x4B] & 0x00ffffffu) << 5,
                 g_gxState.texCopySrc.x, g_gxState.texCopySrc.y, width, height,
                 g_gxState.texCopyDstWidth, g_gxState.texCopyDstHeight,
                 static_cast<u32>(g_gxState.texCopyFmt), (g_gxState.bpRegCache[0x4D] & 0x3ffu) << 5,
                 halfScale);
      }
    }
    g_gxState.dispCopyGamma = static_cast<GXGamma>(bp_get(value, 2, 7));
    g_gxState.texCopyHalfScale = halfScale;
  ...
  default:
    if (const auto mapping = decode_tex_bp_reg(regId); mapping.has_value()) {
      ...
      case TexBpRegMapping::Kind::Tlut:
        // TX_SETTLUT names a palette by its hardware TMEM offset and format.
        // Do not reinterpret this as the SDK's logical GX_TLUTn index: games
        // may install custom TLUT regions and emit BP commands directly.
        changed = slot.rawTlutTmem != bp_get(value, 10, 0) ||
                  slot.rawTlutFormat != static_cast<GXTlutFmt>(bp_get(value, 2, 10)) ||
                  slot.texObjId != 0 || slot.texDataVersion != 0;
        if (changed) {
          slot.rawTlutTmem = static_cast<u16>(bp_get(value, 10, 0));
          slot.rawTlutFormat = static_cast<GXTlutFmt>(bp_get(value, 2, 10));
          slot.texObjId = 0;
          slot.texDataVersion = 0;
          slot.set_no_cache(false);
        }
        break;
...
extern "C" void GXApplyBPReg(u8 reg, u32 value) {
  // This is an immediate raw-GX ingress, unlike the queued host GX setters.
  // Finish earlier host state first: otherwise a subsequent draw's drain can
  // apply an older blend/TEV/texture load over the newer raw register (and can
  // even consume a single-use BP mask in the wrong order).
  if (__gx->dirtyState != 0) {
    __GXSetDirtyState();
  }
  const u32 pendingBytes = get_buffer_size();
  drain();
  std::lock_guard gpuLock(aurora::renderer_gpu_mutex());
  static u32 loggedOrderedDrains = 0;
  if (pendingBytes != 0 && loggedOrderedDrains < 12) {
    ++loggedOrderedDrains;
    Log.warn("Ordered raw BP ingress: reg=0x{:02X} preceding_fifo_bytes={}", reg, pendingBytes);
  }
  handle_bp((static_cast<u32>(reg) << 24) | (value & 0x00FFFFFFu), true);
}

extern "C" void GXSetDirectTextureData(u8 texMapId, const void* data) {
  if (texMapId >= MaxTextures) {
    return;
  }

  auto& slot = g_gxState.loadedTextures[texMapId];
  const bool changed = slot.data != data || slot.texObjId != 0 || slot.texDataVersion != 0 || slot.no_cache();
  slot.data = data;

  // A raw BP load has no GXTexObj object/version identity. Do not let a prior
  // GXLoadTexObj identity make this direct hardware state hit an object cache.
  // Clearing the IDs is sufficient: raw textures can still use the source cache
  // keyed by bytes/format/dimensions and validated after guest invalidation.
  // Marking every raw binding no_cache needlessly re-decodes CMPR and creates a
  // GPU texture again each time two materials alternate the same texture slot.
  slot.texObjId = 0;
  slot.texDataVersion = 0;
  slot.set_no_cache(false);
  if (changed) {
...
extern "C" void GXSetDirectTlutData(u16 tmemOffset, const void* data, u32 byteSize) {
  const auto it = g_gxState.rawTluts.find(static_cast<u16>(tmemOffset & 0x3ffu));
  if (it == g_gxState.rawTluts.end()) {
    return;
  }

  auto& raw = it->second;
  const u32 committedLoadTlut0 = raw.loadPending ? raw.pendingLoadTlut0 : raw.loadTlut0;
  const u16 committedNumEntries = raw.loadPending ? raw.pendingNumEntries : raw.numEntries;
  const size_t expectedBytes = static_cast<size_t>(committedNumEntries) * sizeof(u16);
  const auto commitMetadata = [&]() noexcept {
    if (!raw.loadPending) {
      return;
    }
    raw.loadTlut0 = committedLoadTlut0;
    raw.numEntries = committedNumEntries;
    raw.loadPending = false;
  };
  if (data == nullptr || expectedBytes == 0 || byteSize < expectedBytes) {
    commitMetadata();
    raw.data.clear();
    ++raw.dataVersion;
    g_gxState.stateDirty = true;
  ...
  // A repeated LOADTLUT that writes the same bytes leaves TMEM in exactly the
  // same hardware-visible state.  Raw GameCube titles commonly re-issue the
  // load before many small draws (for example one glyph at a time).  Bumping
  // dataVersion unconditionally turns those harmless repeats into artificial
  // texture-cache invalidations and can force a palette reconvert/upload per
  // draw.  Preserve the transaction semantics while keeping the renderer's
  // version stable when the resulting palette contents are unchanged.
  if (raw.data.size() == expectedBytes &&
      std::memcmp(raw.data.data(), begin, expectedBytes) == 0) {
    commitMetadata();
    return;
  }
  commitMetadata();
  raw.data.assign(begin, begin + expectedBytes);
  ++raw.dataVersion;
  g_gxState.stateDirty = true;
}
  ...
  if (copy_xf_data(addr, xfData, count, bigEndian)) {
    ...
    for (u32 i = 0; i < count; i++) {
      ...
      case 0x00:
        ...
        break;
      case 0x05:
        // GXSetClipMode writes XF 0x1005.  Record the raw word without changing
        // host rasterization yet: this build is a bounded proof of the retail
        // state transition, not the clip-disable implementation itself.
        g_gxState.xfClipMode = val;
        break;
      case 0x08:
        // XF vertex specs (numColors, numNormals, numTexCoords) - informational
        break;
      case 0x09:
        // numChans participates in ShaderConfig, not just the uniform state.
      ...
      case 0x18: {
        ...
        // CP and XF feed the same decoded selection. A cached CP word cannot
        // suppress its next write after this bank changes that selection.
        s_cpRegisterCacheValid[0x30] = false;
      ...
      case 0x19: {
        ...
        s_cpRegisterCacheValid[0x40] = false;
        for (u32 i = 0; i < 4 && (i + 4) < MaxTexCoord; i++) {
...
// Temporary, bounded CPU-side reference probe for both raw-direct and DL draws.
// Sampling per layout avoids spending the entire budget on the first building.
static FILE* s_geometryCaptureState = nullptr;
static u32 s_geometryCaptureDraw = 0;
// Optional material detail for the existing one-frame geometry capture. Enable
// with request text "materials". Bounded RAM snapshots only; no GX state writes,
// texture resolves, deferred readbacks, or guest-visible side effects.
struct MaterialCaptureTexture { const void* data; u32 width, height, format, id; };
static FILE* s_geometryMaterials = nullptr;
static FILE* s_geometryTextures = nullptr;
static std::vector<MaterialCaptureTexture> s_materialCaptureTextures;
static size_t s_materialCaptureBytes = 0;
static u32 s_materialCaptureDraws = 0;
static bool s_materialCaptureQuadsOnly = false;
static u32 s_materialCaptureStartDraw = 0;
static bool s_materialCaptureRange = false;
static u32 s_materialCaptureEndDraw = UINT32_MAX;
static bool s_materialCaptureWorld = false;
static absl::flat_hash_map<HashType, u32> s_materialCaptureFamilies;

static bool material_capture_source_attr(GXAttr attr) noexcept {
  return attr == GX_VA_POS || attr == GX_VA_CLR0 || attr == GX_VA_CLR1 ||
         (attr >= GX_VA_TEX0 && attr <= GX_VA_TEX7);
}

// The inline vertex stream contains only an index for indexed attributes.  A
// material capture must preserve the referenced array bytes as well, otherwise
// TEX coordinates cannot be reconstructed after the process exits.  Keep this
// textual and per-reference so the existing texture/RAM budget is unchanged.
static void capture_material_indexed_sources(FILE* out, GXVtxFmt fmt, const u8* vertices,
                                             u16 count, u32 stride, bool bigEndian) {
  const auto& vtxFmt = g_gxState.vtxFmts[fmt];
  u32 attrOffset = 0;
  for (int i = GX_VA_PNMTXIDX; i <= GX_VA_TEX7; ++i) {
    const auto attr = static_cast<GXAttr>(i);
    const auto desc = g_gxState.vtxDesc[i];
    const auto& a = vtxFmt.attrs[i];
    u32 inlineSize = 0;
    if (desc == GX_DIRECT) {
      inlineSize = comp_type_size(attr, a.type) * comp_cnt_count(attr, a.cnt);
    } else if (desc == GX_INDEX8) {
      inlineSize = (attr == GX_VA_NRM && a.cnt == GX_NRM_NBT3) ? 3u : 1u;
    } else if (desc == GX_INDEX16) {
      inlineSize = (attr == GX_VA_NRM && a.cnt == GX_NRM_NBT3) ? 6u : 2u;
    }

    if (material_capture_source_attr(attr) && (desc == GX_INDEX8 || desc == GX_INDEX16)) {
      const auto& array = g_gxState.arrays[i];
      const u32 indexBytes = desc == GX_INDEX16 ? 2u : 1u;
      const u32 payloadBytes = comp_type_size(attr, a.type) * comp_cnt_count(attr, a.cnt);
      std::fprintf(out, " arrayref attr=%u le=%u stride=%u payload=%u", u32(attr), array.le,
                   array.stride, payloadBytes);
      for (u32 vertex = 0; vertex < count; ++vertex) {
        if (attrOffset + indexBytes > stride) {
          std::fprintf(out, " v%u=bad-inline", vertex);
          continue;
        }
        const u8* inlineAttr = vertices + size_t(vertex) * stride + attrOffset;
        const u32 index = desc == GX_INDEX16 ? read_u16(inlineAttr, bigEndian) : inlineAttr[0];
        const u64 arrayOffset = static_cast<u64>(index) * array.stride;
        if (array.data == nullptr || arrayOffset > array.size ||
            payloadBytes > static_cast<u64>(array.size) - arrayOffset) {
          std::fprintf(out, " v%u=i%u:bad-array", vertex, index);
          continue;
        }
        const auto* source = static_cast<const u8*>(array.data) + arrayOffset;
        std::fprintf(out, " v%u=i%u:", vertex, index);
        for (u32 b = 0; b < payloadBytes; ++b) std::fprintf(out, "%02X", source[b]);
      }
      std::fprintf(out, "\n");
    }
    attrOffset += inlineSize;
  }
}

static void capture_material_draw(GXPrimitive prim, GXVtxFmt fmt, const u8* vertices,
                                  u16 count, u32 stride, bool bigEndian, bool rawBridge) {
  if (s_geometryMaterials == nullptr || (!s_materialCaptureWorld && count > 32) ||
      s_materialCaptureDraws >= 4096) return;
  if (s_materialCaptureWorld) {
    // Keep the offscreen minimap and thousands of identical facade quads from
    // exhausting the budget before the main-screen particle passes arrive.
    if (std::abs(g_gxState.xfViewport[0]) < 250.f ||
        std::abs(g_gxState.xfViewport[1]) < 150.f) return;
    const auto family = xxh3_hash(g_gxState.bpRegCache,
        xxh3_hash(g_gxState.vtxFmts[fmt], xxh3_hash(g_gxState.vtxDesc, u32(prim))));
    if (s_materialCaptureFamilies[family]++ >= 4) return;
  }
  if (s_geometryCaptureDraw < s_materialCaptureStartDraw) return;
  if (s_materialCaptureRange && s_geometryCaptureDraw >= s_materialCaptureEndDraw) return;
  if (!s_materialCaptureRange && g_gxState.vtxFmts[fmt].attrs[GX_VA_POS].type != GX_F32) return;
  if (s_materialCaptureQuadsOnly && count != 4 && count != 6) return;
  ++s_materialCaptureDraws;
  FILE* out = s_geometryMaterials;
  std::fprintf(out, "draw=%u prim=%u fmt=%u count=%u stride=%u be=%u bridge=%u texgens=%u tev=%u pn=%u proj=%u\n",
      s_geometryCaptureDraw, u32(prim), u32(fmt), count, stride, bigEndian, rawBridge,
      g_gxState.numTexGens, g_gxState.numTevStages, g_gxState.currentPnMtx, u32(g_gxState.projType));
  std::fprintf(out, " viewport_xf %.9g %.9g %.9g %.9g %.9g %.9g\n",
      double(g_gxState.xfViewport[0]), double(g_gxState.xfViewport[1]),
      double(g_gxState.xfViewport[2]), double(g_gxState.xfViewport[3]),
      double(g_gxState.xfViewport[4]), double(g_gxState.xfViewport[5]));
  const u16 sampleCount = s_materialCaptureWorld ? std::min<u16>(count, 32) : count;
  if (s_materialCaptureWorld) std::fprintf(out, " sample_count=%u\n", sampleCount);
  std::fprintf(out, " bp");
  for (u32 value : g_gxState.bpRegCache) std::fprintf(out, " %08X", value);
  // TEV and K colors share the same BP addresses but are separate banks.
  // The last raw BP word alone cannot reconstruct both current values.
  for (u32 i = 0; i < g_gxState.colorRegs.size(); ++i) {
    std::fprintf(out, "\n tevreg=%u", i);
    for (u32 c = 0; c < 4; ++c) std::fprintf(out, " %.9g", double(g_gxState.colorRegs[i][c]));
  }
  for (u32 i = 0; i < g_gxState.kcolors.size(); ++i) {
    std::fprintf(out, "\n kcolor=%u", i);
    for (u32 c = 0; c < 4; ++c) std::fprintf(out, " %.9g", double(g_gxState.kcolors[i][c]));
  }
  for (u32 i = 0; i < g_gxState.colorChannelConfig.size(); ++i) {
    const auto& cfg = g_gxState.colorChannelConfig[i];
    const auto& state = g_gxState.colorChannelState[i];
    std::fprintf(out,
                 "\n channel=%u matSrc=%u ambSrc=%u light=%u diff=%u attn=%u mat=[%.9g,%.9g,%.9g,%.9g] amb=[%.9g,%.9g,%.9g,%.9g]",
                 i, u32(cfg.matSrc), u32(cfg.ambSrc), cfg.lightingEnabled, u32(cfg.diffFn), u32(cfg.attnFn),
                 double(state.matColor[0]), double(state.matColor[1]), double(state.matColor[2]), double(state.matColor[3]),
                 double(state.ambColor[0]), double(state.ambColor[1]), double(state.ambColor[2]), double(state.ambColor[3]));
  }
  std::fprintf(out, "\n attrs");
  for (u32 i = 0; i < MaxVtxAttr; ++i) {
    const auto& a = g_gxState.vtxFmts[fmt].attrs[i];
    const auto& array = g_gxState.arrays[i];
    std::fprintf(out, " %u:%u/%u/%u/%u/%p/%u/%u", i, u32(g_gxState.vtxDesc[i]),
        u32(a.cnt), u32(a.type), a.frac, array.data, array.size, array.stride);
  }
  std::fprintf(out, "\n vertices");
  const size_t allVertexBytes = size_t(sampleCount) * stride;
  const size_t vertexBytes = s_materialCaptureRange ? allVertexBytes : std::min<size_t>(allVertexBytes, 2048);
  for (size_t i = 0; i < vertexBytes; ++i) std::fprintf(out, "%s%02X", i % stride == 0 ? " " : "", vertices[i]);
  std::fprintf(out, "\n");
  if (s_materialCaptureRange) capture_material_indexed_sources(out, fmt, vertices, sampleCount, stride, bigEndian);
  if (s_materialCaptureWorld || s_materialCaptureRange) {
    for (u32 i = 0; i < g_gxState.pnMtx.size(); ++i) {
      const auto* m = reinterpret_cast<const float*>(&g_gxState.pnMtx[i].pos);
      std::fprintf(out, " pnmtx=%u", i);
      for (u32 j = 0; j < 12; ++j) std::fprintf(out, " %.9g", m[j]);
      std::fprintf(out, "\n");
    }
    for (u32 i = 0; i < g_gxState.ptTexMtxs.size(); ++i) {
      const auto* m = reinterpret_cast<const float*>(&g_gxState.ptTexMtxs[i]);
      std::fprintf(out, " postmtx=%u", i);
      for (u32 j = 0; j < 12; ++j) std::fprintf(out, " %.9g", m[j]);
      std::fprintf(out, "\n");
    }
  }
  for (u32 i = 0; i < MaxTexCoord; ++i) {
    ...
    std::fprintf(out, " tcg=%u type=%u src=%u mtx=%u post=%u norm=%u ab11=%u\n", i,
        u32(tcg.type), u32(tcg.src), u32(tcg.mtx), u32(tcg.postMtx), tcg.normalize, tcg.inputFormAB11);
  }
  for (u32 i = 0; i < g_gxState.texMtxs.size(); ++i) {
    const auto* m = reinterpret_cast<const float*>(&g_gxState.texMtxs[i]);
    std::fprintf(out, " texmtx=%u", i);
    for (u32 j = 0; j < 12; ++j) std::fprintf(out, " %.9g", m[j]);
    std::fprintf(out, "\n");
  }
  for (u32 i = 0; i < MaxTextures; ++i) {
    const auto& tex = g_gxState.loadedTextures[i];
    const bool copy = g_gxState.copyTextures.find(tex.data) != g_gxState.copyTextures.end();
    u32 id = UINT32_MAX;
    for (const auto& prior : s_materialCaptureTextures) {
      if (prior.data == tex.data && prior.width == tex.width() && prior.height == tex.height() && prior.format == tex.format()) {
        id = prior.id; break;
      }
    }
    const bool nativeFormat = tex.format() <= GX_TF_RGBA8 || tex.format() == GX_TF_CMPR;
    if (id == UINT32_MAX && tex.data != nullptr && !copy && nativeFormat &&
        tex.width() <= 1024 && tex.height() <= 1024 && s_geometryTextures != nullptr &&
        s_materialCaptureTextures.size() < 256) {
      const u32 bytes = GXGetTexBufferSize(tex.width(), tex.height(), GXTexFmt(tex.format()), GX_FALSE, 0);
      if (bytes != 0 && s_materialCaptureBytes + bytes <= 32u * 1024u * 1024u) {
        id = static_cast<u32>(s_materialCaptureTextures.size());
        const u32 header[] = {0x31545847u, id, tex.width(), tex.height(), tex.format(), bytes};
        std::fwrite(header, sizeof(header), 1, s_geometryTextures);
        std::fwrite(tex.data, 1, bytes, s_geometryTextures);
        s_materialCaptureBytes += bytes;
        s_materialCaptureTextures.push_back({tex.data, tex.width(), tex.height(), tex.format(), id});
      }
    }
    std::fprintf(out, " texture=%u id=%u ptr=%p width=%u height=%u format=%u image3=%08X copy=%u\n",
        i, id, tex.data, tex.width(), tex.height(), tex.format(), tex.image3, copy);
  }
}

static FILE* geometry_capture_for_frame(GXVtxFmt fmt) {
  static u32 previousFrame = UINT32_MAX;
  static FILE* output = nullptr;
  static bool waitForWorld = false;
  static std::optional<u32> worldTargetFrame;
  const u32 frame = gfx::current_frame();
  const auto& position = g_gxState.vtxFmts[fmt].attrs[GX_VA_POS];
  if (waitForWorld && !worldTargetFrame && g_gxState.projType == GX_PERSPECTIVE &&
      std::abs(g_gxState.xfViewport[0]) >= 250.f &&
      std::abs(g_gxState.xfViewport[1]) >= 150.f &&
      g_gxState.vtxDesc[GX_VA_POS] == GX_DIRECT &&
      position.cnt == GX_POS_XY && position.type == GX_U8) {
    worldTargetFrame = frame + 120;
    Log.warn("World capture armed at frame {}; waiting 120 frames for gameplay", frame);
  }
  if (frame != previousFrame) {
    if (output != nullptr) {
      std::fclose(output);
      output = nullptr;
      if (s_geometryCaptureState != nullptr) std::fclose(s_geometryCaptureState);
      s_geometryCaptureState = nullptr;
      if (s_geometryMaterials) std::fclose(s_geometryMaterials);
      if (s_geometryTextures) std::fclose(s_geometryTextures);
      s_geometryMaterials = nullptr;
      s_geometryTextures = nullptr;
      Log.warn("CPU geometry capture completed for frame {}", previousFrame);
    }
    previousFrame = frame;
    const bool worldReady = worldTargetFrame && frame >= *worldTargetFrame;
    FILE* request = worldReady ? nullptr : std::fopen("gx_geometry_capture.request", "rb");
    if (request || worldReady) {
      char options[32]{};
      const bool hasOptions = request && std::fscanf(request, "%31s", options) == 1;
      if (hasOptions && std::strcmp(options, "materials-world") == 0) {
        std::fclose(request);
        std::remove("gx_geometry_capture.request");
        waitForWorld = true;
        worldTargetFrame.reset();
        Log.warn("World capture queued; menus will not consume the request");
        return nullptr;
      }
      s_materialCaptureWorld = worldReady;
      if (worldReady) {
        waitForWorld = false;
        worldTargetFrame.reset();
      }
      s_materialCaptureQuadsOnly = hasOptions && std::strcmp(options, "materials-quads") == 0;
      s_materialCaptureRange = worldReady || (hasOptions && std::strcmp(options, "materials-range") == 0);
      const bool materials = worldReady || (hasOptions &&
          (std::strcmp(options, "materials") == 0 || s_materialCaptureQuadsOnly || s_materialCaptureRange));
      unsigned startDraw = 0;
      unsigned rangeCount = 0;
      if (request && materials) std::fscanf(request, "%u", &startDraw);
      s_materialCaptureStartDraw = startDraw;
      if (request && s_materialCaptureRange) std::fscanf(request, "%u", &rangeCount);
      s_materialCaptureEndDraw = worldReady || rangeCount > UINT32_MAX - startDraw ? UINT32_MAX : startDraw + rangeCount;
      if (request) {
        std::fclose(request);
        std::remove("gx_geometry_capture.request");
      }
      output = std::fopen("gx_geometry_capture.bin", "wb");
      if (output != nullptr) {
        s_geometryCaptureState = std::fopen("gx_geometry_capture.states.txt", "w");
        s_geometryCaptureDraw = 0;
        s_materialCaptureDraws = 0;
        s_materialCaptureBytes = 0;
        s_materialCaptureTextures.clear();
        s_materialCaptureFamilies.clear();
        if (materials) {
          s_geometryMaterials = std::fopen("gx_geometry_capture.materials.txt", "w");
          s_geometryTextures = std::fopen("gx_geometry_capture.textures.bin", "wb");
        }
        const u32 header[] = {0x31475043u, frame};
        std::fwrite(header, sizeof(header), 1, output);
        Log.warn("CPU geometry capture started for frame {}", frame);
      }
    }
  }
  return output;
...
static void probe_geometry(GXPrimitive prim, GXVtxFmt fmt, const u8* vertices, u16 vtxCount, u32 vtxSize,
                           bool bigEndian, bool rawBridge) {
  FILE* capture = geometry_capture_for_frame(fmt);
  // Geometry/material inspection is an explicit one-frame diagnostic.  Keep it
  // completely out of normal draws: the old perspective fallback still ran
  // key classification/counters (and periodic matrix logging) for every 3D
  // draw even when no capture was requested, which is especially expensive in
  // Meteor battle frames with thousands of draws.
  if (capture == nullptr || vtxCount == 0 || vtxSize == 0) return;
  const auto desc = g_gxState.vtxDesc[GX_VA_POS];
  const auto& pf = g_gxState.vtxFmts[fmt].attrs[GX_VA_POS];
  const bool indexedPn = g_gxState.vtxDesc[GX_VA_PNMTXIDX] == GX_DIRECT;
  const u32 key = (((static_cast<u32>(fmt) * 4 + static_cast<u32>(desc)) * 8 +
                    static_cast<u32>(pf.type)) * 2) + indexedPn;
  if (key >= 512 || desc == GX_NONE) return;

  const auto& arr = g_gxState.arrays[GX_VA_POS];
  const u32 prefix = matrix_index_prefix_size(fmt);
  if (prefix >= vtxSize) return;
  const u32 compSize = comp_type_size(GX_VA_POS, pf.type);
  const u32 compCount = pf.cnt == GX_POS_XY ? 2u : 3u;
  std::vector<f32> captured;
  captured.resize(static_cast<size_t>(vtxCount) * 10, std::numeric_limits<f32>::quiet_NaN());
  for (u32 sample = 0; sample < vtxCount; ++sample) {
    const u8* vertex = vertices + sample * vtxSize;
    const u32 pnRaw = indexedPn ? vertex[0] : g_gxState.currentPnMtx * 3;
    const u32 pnSlot = pnRaw / 3;
    const u8* src = vertex + prefix;
    u32 avail = vtxSize - prefix;
    bool srcBigEndian = bigEndian;
    u32 index = 0;
    if (desc == GX_INDEX8 || desc == GX_INDEX16) {
      if (desc == GX_INDEX16 && avail < 2) continue;
      index = desc == GX_INDEX8 ? src[0] : read_u16(src, bigEndian);
      const u64 offset = static_cast<u64>(index) * arr.stride;
      if (arr.data == nullptr || offset >= arr.size) continue;
      src = static_cast<const u8*>(arr.data) + offset;
      avail = arr.size - static_cast<u32>(offset);
      srcBigEndian = !arr.le;
    }
    if (avail < compCount * compSize) continue;
    f32 p[3]{};
    for (u32 c = 0; c < compCount; ++c) {
      const u8* s = src + c * compSize;
      switch (pf.type) {
      case GX_U8: p[c] = *s; break;
      case GX_S8: p[c] = static_cast<s8>(*s); break;
      case GX_U16: p[c] = read_u16(s, srcBigEndian); break;
      case GX_S16: p[c] = static_cast<s16>(read_u16(s, srcBigEndian)); break;
      case GX_F32: p[c] = read_f32(s, srcBigEndian); break;
      default: break;
      }
      if (pf.type != GX_F32) p[c] = std::ldexp(p[c], -static_cast<int>(pf.frac));
    }
    const Mat3x4<f32> empty{};
    const auto& matrix = pnSlot < MaxPnMtx ? g_gxState.pnMtx[pnSlot].pos :
                         (pnSlot < MaxPostexMtx ? g_gxState.texMtxs[pnSlot - MaxPnMtx] : empty);
    const f32* m = reinterpret_cast<const f32*>(&matrix);
    f32 mv[3]{};
    for (u32 r = 0; r < 3; ++r) mv[r] = p[0]*m[r*4] + p[1]*m[r*4+1] + p[2]*m[r*4+2] + m[r*4+3];
    f32* dst = captured.data() + sample * 10;
    std::memcpy(dst, p, sizeof(p));
    std::memcpy(dst + 3, mv, sizeof(mv));
    const f32* projection = reinterpret_cast<const f32*>(&g_gxState.proj);
    for (u32 r = 0; r < 4; ++r) dst[6+r] = mv[0]*projection[r*4] + mv[1]*projection[r*4+1] +
                                                 mv[2]*projection[r*4+2] + projection[r*4+3];
  }
  {
    const u32 header[] = {0x574F5344u, static_cast<u32>(prim), static_cast<u32>(fmt), vtxCount, vtxSize,
                         static_cast<u32>(desc), key | (static_cast<u32>(g_gxState.projType) << 16), static_cast<u32>(g_gxState.cullMode),
                         static_cast<u32>(g_gxState.depthCompare), static_cast<u32>(g_gxState.depthFunc),
                         static_cast<u32>(g_gxState.depthUpdate), g_gxState.currentPnMtx};
    std::fwrite(header, sizeof(header), 1, capture);
    std::fwrite(captured.data(), sizeof(f32), captured.size(), capture);
    if (s_geometryCaptureState != nullptr) {
      const auto& vp = g_gxState.xfViewport;
      const auto& pr = g_gxState.xfProjection;
      const auto& texture = g_gxState.loadedTextures[0];
      std::fprintf(s_geometryCaptureState,
          "draw=%u proj=%u count=%u xfvp=[%g,%g,%g,%g,%g,%g] proj6=[%g,%g,%g,%g,%g,%g] "
          "scissor=%08X/%08X/%08X clipmode=%08X posfmt=%u/%u/%u posarray=%p/%u/%u tex0=%p/%u/%u/%u\n",
          s_geometryCaptureDraw, static_cast<u32>(g_gxState.projType), vtxCount,
          vp[0],vp[1],vp[2],vp[3],vp[4],vp[5],pr[0],pr[1],pr[2],pr[3],pr[4],pr[5],
          g_gxState.bpRegCache[0x20],g_gxState.bpRegCache[0x21],g_gxState.bpRegCache[0x59],
          g_gxState.xfClipMode,
          static_cast<u32>(pf.cnt),static_cast<u32>(pf.type),pf.frac,arr.data,arr.size,arr.stride,
          texture.data,texture.width(),texture.height(),texture.format());
    }
    capture_material_draw(prim, fmt, vertices, vtxCount, vtxSize, bigEndian, rawBridge);
    ++s_geometryCaptureDraw;
  }
}

static void refresh_indexed_array_sources() {
  for (int attr = GX_VA_POS; attr <= GX_VA_TEX7; ++attr) {
    if (g_gxState.vtxDesc[attr] != GX_INDEX8 && g_gxState.vtxDesc[attr] != GX_INDEX16) continue;
    auto& array = g_gxState.arrays[attr];
    if (array.cachedRange.size == 0) continue;
    const u64 generation = guest_write_generation(array.data, array.size);
    if (!guest_write_generation_matches(array.cachedSourceGeneration, generation)) {
      // The whole-array host upload is a snapshot, not a live guest RAM view.
      // DC publication occurs outside the FIFO, so this must also veto merging
      // with a draw whose uniform offsets refer to the preceding snapshot.
      array.cachedRange = {};
      array.cachedSourceGeneration = kGuestWriteUntracked;
      g_gxState.stateDirty = true;
    }
  }
}

static bool ensure_draw_capacity(GXPrimitive prim, u16 vtxCount, u32 totalVtxBytes,
                                 std::unique_lock<std::recursive_mutex>& gpuLock) {
  // Every draw expands its primitive topology into the fixed index slice. This
  // is needed before any staging write so a mid-frame split cannot leave the
  // draw's vertex and index ranges in different mapped buffers.
  const auto& pressureIndexTemplate = cached_index_template(prim, vtxCount);
  const size_t indexRequired = pressureIndexTemplate.indices.size() * sizeof(u16);

  // Indexed vertex arrays are snapshotted into Aurora's fixed 8 MiB storage
  // slice. Large retail scenes can legitimately reuse enough distinct array
  // snapshots in one logical frame to fill that slice before GXCopyDisp. Split
  // the GPU batch before uploading this draw so all ranges for the draw belong
  // to the newly mapped staging buffer. end_batch() clears cachedRange for us.
  refresh_indexed_array_sources();
  ...
  for (int attr = GX_VA_POS; attr <= GX_VA_TEX7; ++attr) {
    ...
    if (array.cachedRange.size == 0) {
      storageRequired += gfx::storage_allocation_size(array.size);
    }
  }
  if (totalVtxBytes > gfx::vertex_capacity()) UNLIKELY {
    Log.error("Single GX draw needs {} vertex bytes, exceeding batch capacity {}", totalVtxBytes,
              gfx::vertex_capacity());
    return false;
  }
  if (indexRequired > gfx::index_capacity()) UNLIKELY {
    Log.error("Single GX draw needs {} index bytes, exceeding batch capacity {}", indexRequired,
              gfx::index_capacity());
    return false;
  }
  if (storageRequired > gfx::storage_capacity()) UNLIKELY {
    Log.error("Single GX draw needs {} storage bytes, exceeding batch capacity {}", storageRequired,
              gfx::storage_capacity());
    return false;
  }

  const bool vertexPressure =
      gfx::vertex_bytes_used() > gfx::vertex_capacity() - static_cast<size_t>(totalVtxBytes);
  const bool indexPressure = gfx::index_bytes_used() > gfx::index_capacity() - indexRequired;
  const bool storagePressure = gfx::storage_bytes_used() > gfx::storage_capacity() - storageRequired;
  const bool drawPressure = s_maxDrawsPerBatch != 0 && gfx::g_drawCallCount >= s_maxDrawsPerBatch;
  if (vertexPressure || indexPressure || storagePressure || drawPressure) UNLIKELY {
    if (drawPressure && !vertexPressure && !indexPressure && !storagePressure) {
      static uint32_t drawPressureSplitCount = 0;
      if (drawPressureSplitCount < 32 || (drawPressureSplitCount & 255u) == 0) {
        Log.info("Splitting GX batch for draw pressure: frame={} draws={}/{} split={}",
                 gfx::current_frame(), gfx::g_drawCallCount, s_maxDrawsPerBatch,
                 drawPressureSplitCount + 1);
      }
      ++drawPressureSplitCount;
    }
    static uint32_t stagingPressureSplitCount = 0;
    if ((vertexPressure || indexPressure || storagePressure) &&
        (stagingPressureSplitCount < 64 || (stagingPressureSplitCount & 255u) == 0)) {
      Log.info("Splitting GX batch for staging pressure: vertex={}+{}/{} index={}+{}/{} storage={}+{}/{} split={}",
               gfx::vertex_bytes_used(), totalVtxBytes, gfx::vertex_capacity(), gfx::index_bytes_used(),
               indexRequired, gfx::index_capacity(), gfx::storage_bytes_used(), storageRequired,
               gfx::storage_capacity(), stagingPressureSplitCount + 1);
    }
    if (storagePressure && stagingPressureSplitCount < 4) {
      for (int attr = GX_VA_POS; attr <= GX_VA_TEX7; ++attr) {
        if (g_gxState.vtxDesc[attr] != GX_INDEX8 && g_gxState.vtxDesc[attr] != GX_INDEX16) {
          continue;
        }
        const auto& array = g_gxState.arrays[attr];
        Log.info("  indexed array attr={} data=0x{:X} size={} stride={} cached={} sourceGen=0x{:X}",
                 attr, reinterpret_cast<uintptr_t>(array.data), array.size, array.stride,
                 array.cachedRange.size, array.cachedSourceGeneration);
      }
    }
    if (vertexPressure || indexPressure || storagePressure) {
      ++stagingPressureSplitCount;
    }
    gpuLock.unlock();
    const bool resumed = aurora::split_recording_batch();
    gpuLock.lock();
    if (!resumed) {
      Log.error("Unable to resume GX frame after staging-pressure batch split");
      return false;
    }

    // Every cached storage range belongs to the old staging batch and was
    // cleared by end_batch(). Recompute the full requirement after the split.
    storageRequired = 0;
    for (int attr = GX_VA_POS; attr <= GX_VA_TEX7; ++attr) {
      if (g_gxState.vtxDesc[attr] != GX_INDEX8 && g_gxState.vtxDesc[attr] != GX_INDEX16) {
        continue;
    ...
    if (storageRequired > gfx::storage_capacity()) UNLIKELY {
      Log.error("Single GX draw needs {} storage bytes after batch rotation, exceeding capacity {}",
                storageRequired, gfx::storage_capacity());
      return false;
    }
  }
...
bool submit_raw_draw(GXPrimitive prim, GXVtxFmt fmt, const uint8_t* vertices, uint16_t vtxCount,
  ...
  // Feed raw-bridge draws through the existing bounded geometry/material
  // capture probe as well.  The probe is normally inert; it only opens files
  // when gx_geometry_capture.request is present.  Keeping this hook here makes
  // the request mechanism cover the same HLE raw-direct path used by Meteor.
  probe_geometry(prim, fmt, vertices, vtxCount, vtxSize, false, true);

  // This entry point bypasses process(), so it owns the renderer lock itself.
  std::unique_lock gpuLock(aurora::renderer_gpu_mutex());
  if (!ensure_draw_capacity(prim, vtxCount, vertexBytes, gpuLock)) return false;
...
static bool handle_draw(u8 cmd, const u8* data, u32& pos, u32 size, bool bigEndian,
  ...
  if (!ensure_draw_capacity(prim, vtxCount, totalVtxBytes, gpuLock)) return false;

  // Push raw vertex data to buffer
  const uint8_t* vertices = data + pos;

  // The material-capture machinery was previously disconnected from normal
  // FIFO/DL draws.  Hook it at the decoded vertex stream so a one-frame request
  // can compare visible and missing Meteor materials without altering GX state.
  probe_geometry(prim, fmt, vertices, vtxCount, vtxSize, bigEndian, false);
  ...
  if (!g_gxState.stateDirty) LIKELY {
    ...
    if (lastDraw != nullptr && prim != GX_LINES && prim != GX_LINESTRIP && prim != GX_POINTS &&
        lastDraw->instanceCount == 1 && lastDraw->allowsTriangleMerge &&
        lastDraw->vertexFormat == fmt &&
        lastDraw->vtxCount <= std::numeric_limits<u16>::max() &&
        lastDraw->vtxCount + u32(vtxCount) <= u32(std::numeric_limits<u16>::max()) + 1u) LIKELY {
...
static void handle_draw_unmerged(GXPrimitive prim, GXVtxFmt fmt, u16 vtxCount,
  ...
  for (int i = GX_VA_POS; i <= GX_VA_TEX7; ++i) {
    ...
    if (array.cachedRange.size > 0) {
      ...
      // Sample before copying: a publication during the copy cannot stamp
      // older bytes with the newer source generation.
      const u64 generation = guest_write_generation(array.data, array.size);
      ...
      array.cachedSourceGeneration = generation;
    }
  }
  ...
  gfx::push_draw_command(DrawData{
      ...
      .vertexFormat = fmt,
      .allowsTriangleMerge = prim != GX_LINES && prim != GX_LINESTRIP && prim != GX_POINTS,
  });
...
bool handle_aurora(const u8* data, u32& pos, u32 size, bool bigEndian) {
  ...
  if (subCmd == GX_LOAD_AURORA_VIEWPORT_RENDER) {
    ...
    next.rawTlutTmem = 0xffffu;
    next.rawTlutFormat = GX_TL_IA8;
    pos += 4;
    if (data[pos] != 0) {
      next.flags |= 1u;
    } else {
    ...
    const bool changed = slot.data != next.data || slot.mWidth != next.mWidth || slot.mHeight != next.mHeight ||
                         slot.mFormat != next.mFormat || slot.tlut != next.tlut ||
                         slot.rawTlutTmem != next.rawTlutTmem || slot.rawTlutFormat != next.rawTlutFormat ||
                         slot.flags != next.flags ||
