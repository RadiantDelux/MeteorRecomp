...
struct TextureResolveIdentity {
  ...
  u16 rawTlutTmem = 0xffffu;
  GXTlutFmt rawTlutFormat = GX_TL_IA8;
  u32 rawTlutDataVersion = 0;
  u32 texObjId = 0;
  ...
  friend H AbslHashValue(H h, const TextureResolveIdentity& identity) {
    return H::combine(std::move(h), identity.data, identity.mode0, identity.mode1, identity.image0,
                      ...
                      identity.rawTlutTmem, identity.rawTlutFormat, identity.rawTlutDataVersion,
...
static TextureResolveIdentity make_texture_resolve_identity(const GXTexObj_& obj) noexcept {
  u32 rawTlutDataVersion = 0;
  if (obj.rawTlutTmem != 0xffffu) {
    if (const auto it = g_gxState.rawTluts.find(obj.rawTlutTmem); it != g_gxState.rawTluts.end()) {
      rawTlutDataVersion = it->second.dataVersion;
    }
  }
  return {
      .data = obj.data,
      ...
      .format = obj.format(),
      .tlut = obj.tlut,
      .rawTlutTmem = obj.rawTlutTmem,
      .rawTlutFormat = obj.rawTlutFormat,
      .rawTlutDataVersion = rawTlutDataVersion,
...
struct StaticPaletteTextureKey {
  ...
  // A raw TMEM slot alternates between palettes. Its load version cannot
  // identify reusable conversions: A/B/A must reuse A's immutable texture.
  u64 rawTlutDigest = 0;
  ...
  friend H AbslHashValue(H h, const StaticPaletteTextureKey& key) {
    return H::combine(std::move(h), key.texture, key.tlutData, key.tlutFormat, key.tlutEntries,
                      key.tlutDataVersion, key.rawTlutDigest);
  }
};
...
struct StaticTextureSourceEntry {
  ...
  // A source that changes after its draw was recorded must keep the old GPU
  // image alive until that frame is submitted.  Once the previous frame is
  // complete, the same allocation can be refreshed in place instead of
  // creating another texture object for every video frame.
  u32 lastUsedFrame = UINT32_MAX;
};
...
struct TlutTexturePoolEntry {
  u16 entries = 0;
  GXTexFmt format = GX_TF_IA8;
  gfx::TextureHandle handle;
};

std::vector<TlutTexturePoolEntry> s_tlutTexturePool;
constexpr size_t kTlutTexturePoolPerDescriptor = 4;
...
static void mark_static_source_used(u32 texObjId) noexcept {
  if (texObjId == 0) {
    return;
  }
  if (const auto keyIt = s_texObjSourceKeys.find(texObjId); keyIt != s_texObjSourceKeys.end()) {
    if (auto sourceIt = s_staticTextureSourceCache.find(keyIt->second);
        sourceIt != s_staticTextureSourceCache.end()) {
      sourceIt->second.lastUsedFrame = gfx::current_frame();
    }
  }
}
...
static StaticTextureSourceEntry make_static_texture_source_entry(const StaticTextureKey& key,
  ...
  return {
      ...
      .lastUsedFrame = gfx::current_frame(),
  };
}
...
StaticPaletteTextureKey make_static_palette_texture_key(const GXTexObj_& obj, const GXTlutObj_& tlut) {
  const bool rawTlut = obj.rawTlutTmem != 0xffffu;
  return {
      .texture = make_static_texture_key(obj),
      .tlutData = tlut.data,
      .tlutFormat = static_cast<u32>(tlut.format),
      .tlutEntries = tlut.numEntries,
      .tlutDataVersion = rawTlut ? 0u : tlut.tlutDataVersion,
      .rawTlutDigest = rawTlut
          ? digest_source_bytes(tlut.data, static_cast<size_t>(tlut.numEntries) * sizeof(u16)) : 0u,
  };
}
...
gfx::TextureHandle get_tlut_texture(const GXTlutObj_& tlut) {
  ...
  const GXTexFmt physicalFormat = gfx::tlut_texture_format(tlut.format);
  gfx::TextureHandle handle;
  size_t sameDescriptor = 0;
  for (auto& pooled : s_tlutTexturePool) {
    if (pooled.entries != tlut.numEntries || pooled.format != physicalFormat) {
      continue;
    }
    ++sameDescriptor;
    // The pool itself owns one reference. Reuse only after all recorded draws
    // and palette-conversion jobs released the previous contents.
    if (pooled.handle && pooled.handle.use_count() == 1) {
      gfx::write_tlut_texture(*pooled.handle, {static_cast<const u8*>(tlut.data), tlutSize});
      handle = pooled.handle;
      break;
    }
  }
  if (!handle) {
    handle = gfx::new_static_texture_2d(tlut.numEntries, 1, 1, physicalFormat,
                                        {static_cast<const u8*>(tlut.data), tlutSize}, true, "Loaded TLUT");
    if (sameDescriptor < kTlutTexturePoolPerDescriptor) {
      s_tlutTexturePool.push_back({tlut.numEntries, physicalFormat, handle});
    }
  }
  if (tlut.tlutObjId != 0) {
    auto& cache = s_tlutObjectCaches[tlut.tlutObjId];
...
gfx::TextureHandle resolve_static_texture(const GXTexObj_& obj) {
  ...
  if (canCacheUpload && obj.texObjId != 0) {
    if (const auto it = s_textureObjectCaches.find(obj.texObjId); it != s_textureObjectCaches.end()) {
      ...
      if (entry.handle && entry.texDataVersion == obj.texDataVersion && entry.tlutObjId == 0 &&
          entry.validationRevision == s_staticTextureCacheValidationRevision) {
        mark_static_source_used(obj.texObjId);
        return entry.handle;
      }
    }
  }
  ...
  if (canUseSourceCache) {
    if (auto handle = lookup_static_source_front_cache(sourceKey, sourceHash)) {
      if (auto it = s_staticTextureSourceCache.find(sourceKey); it != s_staticTextureSourceCache.end()) {
        it->second.lastUsedFrame = gfx::current_frame();
      }
      if (obj.texObjId != 0) {
    ...
    if (auto it = s_staticTextureSourceCache.find(sourceKey); it != s_staticTextureSourceCache.end()) {
      if (!validate_static_texture_source_entry(sourceKey, it->second)) {
        // A source changed after GXInvalidateTexAll. If the previous frame has
        // already been handed to the renderer, refresh its allocation in place
        // and keep the bind-group identity stable. During the same frame an
        // earlier draw may still reference the old bytes, so preserve the old
        // behavior and allocate a fresh image in that case.
        const u32 dataSize = static_texture_data_size(sourceKey);
        const u32 frame = gfx::current_frame();
        // With the asynchronous frame worker the producer can already be
        // recording frame N+1 while frame N is still being encoded.  Require
        // one complete intervening frame before mutating a handle, which is
        // the earliest point at which the worker has joined the old frame.
        const bool previousFrameComplete =
            frame != UINT32_MAX && it->second.lastUsedFrame != UINT32_MAX && frame > it->second.lastUsedFrame + 1u;
        if (it->second.handle && dataSize == it->second.sourceSize && previousFrameComplete) {
          gfx::write_texture(*it->second.handle,
                             {static_cast<const u8*>(sourceKey.data), dataSize});
          it->second.digest = digest_source_bytes(sourceKey.data, dataSize);
          it->second.sourceGeneration = guest_write_generation(sourceKey.data, dataSize);
          it->second.validationRevision = s_staticTextureCacheValidationRevision;
          it->second.lastUsedFrame = frame;
          store_static_source_front_cache(sourceKey, sourceHash, it->second.handle);
          if (obj.texObjId != 0) {
            store_cached_texture(obj, it->second.handle);
            record_tex_obj_source_key(obj.texObjId, sourceKey);
          }
          return it->second.handle;
        }

        // Cached bindings can bypass source lookup across frames. A changed
        // source that is still live in this frame gets a new handle so already
        // recorded draws keep their bytes.
  ...
  if (const auto replacement = gfx::texture_replacement::find_replacement(obj); replacement.has_value()) {
    ...
    static u64 staticTextureUploadMissCount = 0;
    ++staticTextureUploadMissCount;
    if (staticTextureUploadMissCount <= 64 || (staticTextureUploadMissCount & 255u) == 1u) {
      ...
      Log.info("Static texture upload miss #{}: data=0x{:X} {}x{} mips={} fmt=0x{:X} bytes={} texObj={} ver={} "
               "flags=0x{:X} noCache={} sourceCache={} objectCache={} validationRev={} copyRev={}",
               staticTextureUploadMissCount, reinterpret_cast<uintptr_t>(obj.data), obj.width(), obj.height(),
               obj.mip_count(), obj.format(), dataSize, obj.texObjId, obj.texDataVersion, obj.flags,
               obj.no_cache(), s_staticTextureSourceCache.size(), s_textureObjectCaches.size(),
               s_staticTextureCacheValidationRevision, s_copyTextureStateRevision);
    }
#if DEBUG
...
void invalidate_dynamic_palette_source(const void* sourceIdentity) noexcept {
  if (sourceIdentity == nullptr) {
    return;
  }
  for (auto& [_, cache] : s_tlutObjectCaches) {
    for (auto it = cache.dynamicPaletteTextures.begin(); it != cache.dynamicPaletteTextures.end();) {
      if (it->first.sourceIdentity == sourceIdentity) {
        cache.dynamicPaletteTextures.erase(it++);
      } else {
        ++it;
      }
...
void resolve_sampled_textures(const ShaderInfo& info) noexcept {
  ...
  for (u32 i = 0; i < MaxTextures; ++i) {
    ...
    if (is_palette_format(obj.format())) {
      ...
      GXTlutObj_ rawTlut{};
      if (obj.rawTlutTmem != 0xffffu) {
        if (const auto it = g_gxState.rawTluts.find(obj.rawTlutTmem);
            it != g_gxState.rawTluts.end() && !it->second.data.empty()) {
          rawTlut.data = it->second.data.data();
          rawTlut.format = obj.rawTlutFormat;
          rawTlut.numEntries = it->second.numEntries;
          rawTlut.loadTlut0 = it->second.loadTlut0;
          rawTlut.tlutDataVersion = it->second.dataVersion;
          tlut = &rawTlut;
        }
      } else {
      ...
      if (tlut != nullptr && tlut->data != nullptr) {
        if (copyRef != nullptr) {
...
static u64 s_bindGroupFrontCacheGeneration = 1;

void invalidate_bind_group_front_cache() noexcept {
  ++s_bindGroupFrontCacheGeneration;
  if (s_bindGroupFrontCacheGeneration == 0) {
    s_bindGroupFrontCacheGeneration = 1;
  }
}
...
GXBindGroups build_bind_groups(const ShaderInfo& info) noexcept {
  ...
  if (entry.valid && entry.frame == frame && entry.generation == s_bindGroupFrontCacheGeneration &&
  ...
  entry.valid = true;
  entry.frame = frame;
  entry.generation = s_bindGroupFrontCacheGeneration;
  entry.hash = hash;
  entry.key = key;
...
void shutdown() noexcept {
  ...
  s_textureObjectCaches.clear();
  s_tlutObjectCaches.clear();
  // The physical TLUT reuse pool owns TextureHandles independently from the
  // logical TLUT/object caches above. Drop them before the WebGPU device is
  // torn down so a later initialize cannot reuse handles from the old device.
  s_tlutTexturePool.clear();
  s_staticTextureSourceCache.clear();
  s_staticPaletteTextureSourceCache.clear();
  s_texObjSourceKeys.clear();
  s_texObjPaletteSourceKeys.clear();
