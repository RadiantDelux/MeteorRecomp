...
UniformRanges build_uniform(const ShaderInfo& info, u32 vtxStart, const BindGroupRanges& ranges,
  ...
  // WebGPU requires minDepth <= maxDepth, while GX permits either viewport
  // depth order. Keep the projection matrix byte-for-byte and tell the vertex
  // shader which post-multiply GX->WebGPU mapping preserves the guest order.
  // Reuse the first padding word so the uniform layout and offsets stay fixed.
  stage_u32(g_gxState.renderViewport.znear <= g_gxState.renderViewport.zfar ? 1u : 0u);
  stage_u32(0u);
