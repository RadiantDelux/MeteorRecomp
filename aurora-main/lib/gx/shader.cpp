...
wgpu::ShaderModule build_shader(const ShaderConfig& config) noexcept {
  ...
  // GX projection produces clip depth in [-W, 0]. WebGPU sorts the host
  // viewport to minDepth <= maxDepth, so preserve the original GX viewport
  // order here after the matrix multiply: normal GX depth maps z -> z + w,
  // while a reversed GX viewport maps z -> -z. Doing this post-multiply avoids
  // the coefficient-fold rounding that can move an exact clip-plane vertex by
  // an ULP. ubuf.pad.x is 1 for znear <= zfar and 0 for reversed depth order.
  vtxXfrAttrsPre +=
      "\n    var converted_z = select(-out.pos.z, out.pos.z + out.pos.w, ubuf.pad.x != 0u);"
      "\n    let clip_slop = max(out.pos.w, 0.0) * 0.00000025;"
      "\n    let repair_low_clip = converted_z < 0.0 && -converted_z <= clip_slop;"
      "\n    converted_z = select(converted_z, 0.0, repair_low_clip);"
      "\n    let repair_high_clip = converted_z > out.pos.w && converted_z - out.pos.w <= clip_slop;"
      "\n    out.pos.z = select(converted_z, out.pos.w, repair_high_clip);";
