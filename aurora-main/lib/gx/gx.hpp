...
// XF position/texture matrix memory also contains the SDK's GX_IDENTITY slot
// immediately after GX_TEXMTX9. Keep that physical slot modeled instead of
// aliasing raw selector 60 to GX_TEXMTX9.
constexpr u32 MaxTexMtx = (GX_IDENTITY - GX_TEXMTX0) / 3 + 1;
...
// Position and texture matrices share one shader array (`ubuf.postex_mtx`),
// mirroring XF matrix memory: rows 0..29 (slots 0..9) are position matrices,
// rows 30..59 (slots 10..19) are GX_TEXMTX0..9, and rows 60..62 (slot 20)
// hold GX_IDENTITY.
...
struct AttrArray {
  ...
  u64 cachedSourceGeneration = kGuestWriteUntracked;
};
...
struct GXState {
  struct RawTlutState {
    u32 loadTlut0 = 0;
    u16 numEntries = 0;
    u32 pendingLoadTlut0 = 0;
    u16 pendingNumEntries = 0;
  ...
  // LOADTLUT works in TMEM addresses.  Keep raw display-list/BP state separate
  // from the SDK's GX_TLUT0..GX_BIGTLUT3 object slots so custom TLUT regions
  // retain hardware semantics.
  absl::flat_hash_map<u16, RawTlutState> rawTluts;
  ...
  u32 xfError = 0;
  // Raw XF 0x1005 clip-control word.  Kept as diagnostic state for now;
  // rasterization semantics are intentionally unchanged until the Meteor
  // receiver proof establishes the exact disabled-clip path needed.
  u32 xfClipMode = 0;
  u32 dualTex = 0;
  float zScale = 1.f;
  float zOffset = 0.f;
...
// Drops all retired physical GX copy targets. A non-null logical destination is
// intentionally ignored: free physical targets are descriptor-pooled and may be
// safely recycled for another guest destination after their source identity is invalidated.
void prune_copy_texture_pool(const void* dest) noexcept;
// A recycled physical copy texture keeps the same host pointer. Dynamic palette
// conversions cache that pointer as source identity, so forget those conversions
// before publishing new pixels through the recycled allocation.
...
// BP 0x52 stores the four-bit EFB copy encoder mode with its MSB rotated into the
// low bit. Decode that hardware representation before exposing it as an SDK-style
// texture format. Intensity conversion is active only when both control bits are set.
static inline GXTexFmt decode_efb_copy_format(u32 rawFormat, bool intensityFormat,
                                               bool automaticColorConversion) noexcept {
  const u32 format = ((rawFormat & 0xFu) >> 1) | ((rawFormat & 1u) << 3);
  const bool intensity = intensityFormat && automaticColorConversion;
  switch (format) {
  case 0x0:
    return intensity ? GX_TF_I4 : GX_CTF_R4;
  case 0x1:
    return intensity ? GX_TF_I8 : GX_CTF_R8;
  case 0x2:
    return intensity ? GX_TF_IA4 : GX_CTF_RA4;
  case 0x3:
    return intensity ? GX_TF_IA8 : GX_CTF_RA8;
  case 0x4:
    return GX_TF_RGB565;
  case 0x5:
    return GX_TF_RGB5A3;
  ...
  case 0xC:
    return GX_CTF_GB8;
  default:
    return static_cast<GXTexFmt>(format);
  }
}
...
static inline bool copy_texture_format_compatible(GXTexFmt copyFmt, u32 sampleFmt) noexcept {
  ...
  case GX_CTF_B8:
  case GX_CTF_A8:
    // These copy formats all produce one 8-bit channel per texel. Besides
    // direct I8/A8 sampling, retail games also reinterpret that byte as a C8
    // palette index. The dynamic-palette path applies the currently selected
    // TLUT after sampling the copied byte, matching GX TMEM semantics.
    return sampleFmt == GX_TF_I8 || sampleFmt == GX_TF_A8 || sampleFmt == GX_TF_C8;
  case GX_CTF_RA4:
