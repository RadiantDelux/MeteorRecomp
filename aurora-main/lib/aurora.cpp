...
// Hardware-style VI scanout decouples producer cadence from display cadence:
// GX seals/latches immutable images, while VI retraces present the newest one.
std::atomic<bool> g_viScanoutMode{false};
// When true, normal producer presentation remains unchanged but the final
// immutable image is also latched so VI can repeat it across producer gaps.
std::atomic<bool> g_viScanoutRepeatMode{false};
...
struct FrameCostTelemetry {
  uint32_t createTextureCount = 0;
  uint32_t createViewCount = 0;
  uint32_t conversionCount = 0;
  uint32_t writeCount = 0;
  uint32_t reclaimedBindGroups = 0;
  uint32_t finishCount = 0;
  uint32_t submitCount = 0;
  uint32_t processEventsCount = 0;
  uint64_t sourceBytes = 0;
  uint64_t uploadBytes = 0;
  std::chrono::nanoseconds createTexture{};
  std::chrono::nanoseconds createView{};
  std::chrono::nanoseconds conversion{};
  std::chrono::nanoseconds writeTexture{};
  std::chrono::nanoseconds bindGroupReclaim{};
  std::chrono::nanoseconds finish{};
  std::chrono::nanoseconds submit{};
  std::chrono::nanoseconds processEvents{};
};

std::mutex g_frameCostTelemetryMutex;
std::unordered_map<uint32_t, FrameCostTelemetry> g_frameCostTelemetry;

FrameCostTelemetry take_frame_cost_telemetry(uint32_t logicalFrame) {
  std::lock_guard lock(g_frameCostTelemetryMutex);
  FrameCostTelemetry result;
  if (const auto it = g_frameCostTelemetry.find(logicalFrame); it != g_frameCostTelemetry.end()) {
    result = it->second;
    g_frameCostTelemetry.erase(it);
  }
  if (g_frameCostTelemetry.size() > 256) {
    for (auto it = g_frameCostTelemetry.begin(); it != g_frameCostTelemetry.end();) {
      if (it->first + 128u < logicalFrame) {
        it = g_frameCostTelemetry.erase(it);
      } else {
        ++it;
      }
    }
...
void add_frame_cost(FrameCostTelemetry& dst, const FrameCostTelemetry& src) {
  dst.createTextureCount += src.createTextureCount;
  dst.createViewCount += src.createViewCount;
  dst.conversionCount += src.conversionCount;
  dst.writeCount += src.writeCount;
  dst.reclaimedBindGroups += src.reclaimedBindGroups;
  dst.finishCount += src.finishCount;
  dst.submitCount += src.submitCount;
  dst.processEventsCount += src.processEventsCount;
  dst.sourceBytes += src.sourceBytes;
  dst.uploadBytes += src.uploadBytes;
  dst.createTexture += src.createTexture;
  dst.createView += src.createView;
  dst.conversion += src.conversion;
  dst.writeTexture += src.writeTexture;
  dst.bindGroupReclaim += src.bindGroupReclaim;
  dst.finish += src.finish;
  dst.submit += src.submit;
  dst.processEvents += src.processEvents;
}

struct MotionCostWindow {
  PresentClock::time_point startedAt{};
  uint32_t motionFrames = 0;
  uint32_t gapSamples = 0;
  uint32_t hitches50ms = 0;
  std::chrono::nanoseconds totalGap{};
  std::chrono::nanoseconds maxGap{};
  uint32_t maxGapFrame = 0;
  FrameCostTelemetry totals{};
  FrameCostTelemetry maxGapCosts{};
};

MotionCostWindow g_motionCostWindow;
std::optional<PresentClock::time_point> g_lastMotionPresent;

double to_ms(std::chrono::nanoseconds value) {
  return std::chrono::duration<double, std::milli>(value).count();
}

AuroraPresentTiming snapshot_present_timing() noexcept;

void record_motion_cost(uint32_t logicalFrame, PresentClock::time_point now, const FrameCostTelemetry& costs) {
  auto& window = g_motionCostWindow;
  if (window.startedAt == PresentClock::time_point{}) {
    window.startedAt = now;
  }
  ++window.motionFrames;
  add_frame_cost(window.totals, costs);

  if (g_lastMotionPresent) {
    const auto gap = std::chrono::duration_cast<std::chrono::nanoseconds>(now - *g_lastMotionPresent);
    ++window.gapSamples;
    window.totalGap += gap;
    if (gap >= std::chrono::milliseconds(50)) {
      ++window.hitches50ms;
    }
    if (gap > window.maxGap) {
      window.maxGap = gap;
      window.maxGapFrame = logicalFrame;
      window.maxGapCosts = costs;
    }
  }
  g_lastMotionPresent = now;

  if (now - window.startedAt < std::chrono::seconds(1)) {
    return;
  }

  const double avgGapMs = window.gapSamples != 0 ? to_ms(window.totalGap) / window.gapSamples : 0.0;
  const double uploadMiB = static_cast<double>(window.totals.uploadBytes) / (1024.0 * 1024.0);
  const AuroraPresentTiming presentTiming = snapshot_present_timing();
  Log.info(
      "Frame causal telemetry: motion={} presentFps={:.2f} effectiveFps={:.2f} presentSamples={} "
      "avgGap={:.2f}ms maxGap={:.2f}ms@{} hitches50={} "
      "createTex={} ({:.3f}ms) createView={} ({:.3f}ms) convert={} ({:.3f}ms) writes={} ({:.3f}ms) "
      "upload={:.2f}MiB "
      "reclaim={} ({:.3f}ms) finish={} ({:.3f}ms) submit={} ({:.3f}ms) process={} ({:.3f}ms); "
      "maxGapCosts create={:.3f} view={:.3f} convert={:.3f} write={:.3f} reclaim={:.3f} finish={:.3f} "
      "submit={:.3f} process={:.3f}ms",
      window.motionFrames, presentTiming.framesPerSecond, presentTiming.effectiveFramesPerSecond,
      presentTiming.sampleCount, avgGapMs, to_ms(window.maxGap), window.maxGapFrame, window.hitches50ms,
      window.totals.createTextureCount, to_ms(window.totals.createTexture), window.totals.createViewCount,
      to_ms(window.totals.createView), window.totals.conversionCount, to_ms(window.totals.conversion),
      window.totals.writeCount, to_ms(window.totals.writeTexture), uploadMiB,
      window.totals.reclaimedBindGroups, to_ms(window.totals.bindGroupReclaim), window.totals.finishCount,
      to_ms(window.totals.finish), window.totals.submitCount, to_ms(window.totals.submit),
      window.totals.processEventsCount, to_ms(window.totals.processEvents), to_ms(window.maxGapCosts.createTexture),
      to_ms(window.maxGapCosts.createView), to_ms(window.maxGapCosts.conversion), to_ms(window.maxGapCosts.writeTexture),
      to_ms(window.maxGapCosts.bindGroupReclaim), to_ms(window.maxGapCosts.finish), to_ms(window.maxGapCosts.submit),
      to_ms(window.maxGapCosts.processEvents));
  window = MotionCostWindow{.startedAt = now};
}
...
void record_successful_present(bool, uint32_t logicalFrame, bool duplicated) noexcept {
...
struct ViScanoutState {
  ...
  uint64_t lastScannedSerial = 0;
};

ViScanoutState g_viScanout;

void latch_vi_scanout_image(const PresentationJob& job) {
  if (!job.image) {
    return;
  }
  std::lock_guard lock(g_viScanout.mutex);
  g_viScanout.image = job.image;
  g_viScanout.logicalFrame = job.logicalFrame;
  ++g_viScanout.imageSerial;
}
...
bool present_presentation_job(PresentationJob job) {
  ...
  std::chrono::nanoseconds submitMutexWaitDuration{};
  std::chrono::nanoseconds submitApiDuration{};
  std::chrono::nanoseconds scheduleWaitDuration{};
  std::chrono::nanoseconds presentDuration{};
  std::chrono::nanoseconds presentApiDuration{};
  ...
  }
  {
    ...
    if (!surfaceSizeChanged && window::is_presentable()) {
      ...
      if (acquired) {
        ...
        {
          std::unique_lock submitLock(g_queueSubmitMutex);
          const auto submitLocked = PresentClock::now();
          submitMutexWaitDuration = std::chrono::duration_cast<std::chrono::nanoseconds>(
              submitLocked - submitStarted);
          g_queue.Submit(1, &buffer);
          submitApiDuration = std::chrono::duration_cast<std::chrono::nanoseconds>(
              PresentClock::now() - submitLocked);
        }
        ...
        if (!g_surfaceReconfigurePending.load(std::memory_order_acquire) &&
          ...
          const wgpu::Status presentStatus = g_surface.Present();
          presentApiDuration = std::chrono::duration_cast<std::chrono::nanoseconds>(
              PresentClock::now() - presentStarted);
          presentDuration = std::chrono::duration_cast<std::chrono::nanoseconds>(
              PresentClock::now() - presentStarted);
          if (presentStatus == wgpu::Status::Success) {
            presented = true;
            record_successful_present(job.interpolated, job.logicalFrame, job.duplicated);
          } else {
  ...
  if (totalDuration >= kSurfaceStallThreshold) {
    ...
    Log.warn("Presentation job took {:.1f} ms (surface lock {:.1f}, acquire {:.1f}, encode {:.1f}, "
             "finish {:.1f}, submit {:.1f} [mutex {:.1f}, api {:.1f}], schedule wait {:.1f}, "
             "present {:.1f} [api {:.1f}]){}",
             ...
             std::chrono::duration<double, std::milli>(submitMutexWaitDuration).count(),
             std::chrono::duration<double, std::milli>(submitApiDuration).count(),
             ...
             std::chrono::duration<double, std::milli>(presentApiDuration).count(),
...
void stop_presenter() noexcept {
  ...
  // PresentationImage owns GPU resources and the VI latch deliberately pins one
  // image outside the pool. Release it before the pools/device are destroyed.
  {
    std::lock_guard lock(g_viScanout.mutex);
    g_viScanout.image.reset();
    g_viScanout.logicalFrame = 0;
    g_viScanout.imageSerial = 0;
    g_viScanout.lastScannedSerial = 0;
  }
}
...
void seal_frame_locked(gfx::SealedFrame& sealedFrame, SealedFrameContext& ctx) {
  ...
  gfx::expire_bind_group_cache(ctx.logicalFrame);
}
...
std::vector<PresentationJob> encode_sealed_frame(gfx::SealedFrame& sealedFrame, SealedFrameContext& ctx) {
  ...
  const auto submitEncodedSlot = [&](wgpu::CommandEncoder& target) {
    ...
    const auto submitLocked = PresentClock::now();
    g_queue.Submit(1, &buffer);
    const auto submitFinished = PresentClock::now();
    const auto submitDuration =
        std::chrono::duration_cast<std::chrono::nanoseconds>(submitFinished - submitStarted);
    const auto submitMutexWait =
        std::chrono::duration_cast<std::chrono::nanoseconds>(submitLocked - submitStarted);
    const auto submitApi =
        std::chrono::duration_cast<std::chrono::nanoseconds>(submitFinished - submitLocked);
    if (submitDuration >= std::chrono::milliseconds(50)) {
      Log.warn("Scene Queue.Submit took {:.1f} ms (mutex {:.1f}, api {:.1f}) frame={}",
               std::chrono::duration<double, std::milli>(submitDuration).count(),
               std::chrono::duration<double, std::milli>(submitMutexWait).count(),
               std::chrono::duration<double, std::milli>(submitApi).count(), ctx.logicalFrame);
    }
    record_scene_submit_telemetry(ctx.logicalFrame, finishDuration, submitDuration);
  };
  ...
  // In VI-owned scanout mode the native frame becomes the new immutable XFB
  // snapshot, but sealing GX work must not itself refresh the host surface.
  ...
  gfx::after_submit(ctx.logicalFrame);
...
void record_static_texture_upload_telemetry(uint32_t logicalFrame,
  ...
  std::lock_guard lock(g_frameCostTelemetryMutex);
  auto& frame = g_frameCostTelemetry[logicalFrame];
  ++frame.createTextureCount;
  ++frame.createViewCount;
  frame.conversionCount += conversionAttempted ? 1u : 0u;
  frame.writeCount += writeCount;
  frame.sourceBytes += sourceBytes;
  frame.uploadBytes += uploadBytes;
  frame.createTexture += createTexture;
  frame.createView += createView;
  frame.conversion += conversion;
  frame.writeTexture += writeTexture;
}
...
void record_bind_group_reclaim_telemetry(uint32_t logicalFrame, uint32_t reclaimed,
  ...
  std::lock_guard lock(g_frameCostTelemetryMutex);
  auto& frame = g_frameCostTelemetry[logicalFrame];
  frame.reclaimedBindGroups += reclaimed;
  frame.bindGroupReclaim += duration;
}
...
void record_process_events_telemetry(uint32_t logicalFrame, std::chrono::nanoseconds duration) noexcept {
  ...
  std::lock_guard lock(g_frameCostTelemetryMutex);
  auto& frame = g_frameCostTelemetry[logicalFrame];
  ++frame.processEventsCount;
  frame.processEvents += duration;
}

bool split_recording_batch() noexcept {
#ifdef AURORA_ENABLE_GX
  // The previous sealed frame may still be encoding without the renderer
  // mutex. Join it before rendering a prefix of the producer's current frame;
  // otherwise both encoders can touch the same mutable EFB resources.
  wait_for_frame_worker();
  std::lock_guard gpuLock(g_rendererGpuMutex);
  gx::mark_frame_interpolation_replay_unsafe();

  const wgpu::CommandEncoderDescriptor encoderDescriptor{
      .label = "GX staging-pressure batch split encoder",
  };
  auto encoder = webgpu::g_device.CreateCommandEncoder(&encoderDescriptor);
  gfx::end_batch(encoder);
  gfx::render(encoder);
  const wgpu::CommandBufferDescriptor commandDescriptor{
      .label = "GX staging-pressure batch split command buffer",
  };
  const auto commandBuffer = encoder.Finish(&commandDescriptor);
  {
    ...
    const auto submitFinished = PresentClock::now();
    const auto total = std::chrono::duration_cast<std::chrono::nanoseconds>(submitFinished - submitStarted);
    if (total >= std::chrono::milliseconds(50)) {
      Log.warn("Split-batch Queue.Submit took {:.1f} ms (mutex {:.1f}, api {:.1f})",
               std::chrono::duration<double, std::milli>(total).count(),
               std::chrono::duration<double, std::milli>(submitLocked - submitStarted).count(),
               std::chrono::duration<double, std::milli>(submitFinished - submitLocked).count());
    }
  }
  // A staging-pressure split is synchronous producer backpressure, not a frame
  // boundary. If we immediately resume recording, a pathological retail frame
  // can enqueue dozens of full GX command buffers faster than the GPU retires
  // them; Dawn/driver then keeps all referenced resources alive and host memory
  // grows by many GiB before the guest reaches its next GXCopyDisp. Drain this
  // prefix before reusing the fixed staging/GPU buffers for the next batch.
  auto workStatus = std::make_shared<std::atomic<wgpu::QueueWorkDoneStatus>>(
      wgpu::QueueWorkDoneStatus::CallbackCancelled);
  const auto workFuture = webgpu::g_queue.OnSubmittedWorkDone(
      wgpu::CallbackMode::WaitAnyOnly,
      [workStatus](wgpu::QueueWorkDoneStatus status, wgpu::StringView) { workStatus->store(status); });
  const auto waitStatus = webgpu::g_instance.WaitAny(workFuture, 5000000000);
  if (waitStatus != wgpu::WaitStatus::Success || workStatus->load() != wgpu::QueueWorkDoneStatus::Success) {
    Log.error("GX staging-pressure batch drain failed wait={} work={}", magic_enum::enum_name(waitStatus),
              magic_enum::enum_name(workStatus->load()));
    gfx::after_submit();
    return false;
  }
  // The completed prefix has already been encoded and its GPU work is retired,
  // so no queued command can reference these bind groups anymore. Releasing
  // them here bounds host memory for retail frames with thousands of distinct
  // draws while preserving the logical frame and EFB contents.
  gfx::clear_completed_bind_group_cache();
  gx::invalidate_bind_group_front_cache();
  gfx::after_submit();
  return gfx::resume_frame();
#else
  return true;
#endif
}
...
void aurora_set_vi_scanout_mode(bool enabled) {
  aurora::g_viScanoutMode.store(enabled, std::memory_order_release);
  if (!enabled &&
      !aurora::g_viScanoutRepeatMode.load(std::memory_order_acquire)) {
    std::lock_guard lock(aurora::g_viScanout.mutex);
    aurora::g_viScanout.image.reset();
    aurora::g_viScanout.logicalFrame = 0;
    aurora::g_viScanout.imageSerial = 0;
    aurora::g_viScanout.lastScannedSerial = 0;
  }
}
void aurora_set_vi_scanout_repeat_mode(bool enabled) {
  aurora::g_viScanoutRepeatMode.store(enabled, std::memory_order_release);
  if (!enabled && !aurora::g_viScanoutMode.load(std::memory_order_acquire)) {
    std::lock_guard lock(aurora::g_viScanout.mutex);
    aurora::g_viScanout.image.reset();
    aurora::g_viScanout.logicalFrame = 0;
    aurora::g_viScanout.imageSerial = 0;
...
bool aurora_present_vi_scanout(uint64_t presentAtNanos) {
#ifdef AURORA_ENABLE_GX
  const bool viOwnsScanout =
      aurora::g_viScanoutMode.load(std::memory_order_acquire);
  const bool repeatOnly =
      aurora::g_viScanoutRepeatMode.load(std::memory_order_acquire);
  if (!viOwnsScanout && !repeatOnly) {
    return false;
  }

  aurora::PresentationJob job;
  {
    std::lock_guard lock(aurora::g_viScanout.mutex);
  ...
  if (presentAtNanos != 0) {
    job.presentAt = aurora::PresentClock::time_point{
        std::chrono::nanoseconds{static_cast<int64_t>(presentAtNanos)}};
  }

#if defined(__APPLE__)
  return aurora::present_presentation_job(job);
#else
  std::vector<aurora::PresentationJob> jobs;
  jobs.push_back(std::move(job));
  aurora::enqueue_presentations(std::move(jobs));
  return true;
#endif
#else
  (void)presentAtNanos;
...
bool aurora_flush_efb_copies_to_ram() {
  ...
  const auto commandBuffer = encoder.Finish(&commandDescriptor);
  {
    ...
    std::unique_lock submitLock(aurora::g_queueSubmitMutex);
    const auto submitLocked = std::chrono::steady_clock::now();
    aurora::webgpu::g_queue.Submit(1, &commandBuffer);
    const auto submitFinished = std::chrono::steady_clock::now();
    const auto total = std::chrono::duration_cast<std::chrono::nanoseconds>(submitFinished - submitStarted);
    if (total >= std::chrono::milliseconds(50)) {
      aurora::Log.warn("CPU-visible EFB Queue.Submit took {:.1f} ms (mutex {:.1f}, api {:.1f})",
                       std::chrono::duration<double, std::milli>(total).count(),
                       std::chrono::duration<double, std::milli>(submitLocked - submitStarted).count(),
                       std::chrono::duration<double, std::milli>(submitFinished - submitLocked).count());
    }
...
bool aurora_flush_efb_copy_to_ram(void* dest) {
  ...
  const auto commandBuffer = encoder.Finish(&commandDescriptor);
  {
    ...
    const auto total = std::chrono::duration_cast<std::chrono::nanoseconds>(submitFinished - submitStarted);
    if (total >= std::chrono::milliseconds(50)) {
      aurora::Log.warn("Demanded EFB Queue.Submit took {:.1f} ms (mutex {:.1f}, api {:.1f})",
                       std::chrono::duration<double, std::milli>(total).count(),
                       std::chrono::duration<double, std::milli>(submitLocked - submitStarted).count(),
                       std::chrono::duration<double, std::milli>(submitFinished - submitLocked).count());
    }
