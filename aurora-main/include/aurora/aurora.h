...
// The VI owner calls aurora_present_vi_scanout() once per hardware retrace to
// scan the most recently latched image again when no new GX frame exists.
void aurora_set_vi_scanout_mode(bool enabled);
// Normal producer-present mode can still model the hardware VI repeatedly
// scanning the last completed XFB when GX does not seal a new frame. Unlike
// aurora_set_vi_scanout_mode(), this does not transfer presentation ownership
// away from the producer; it only keeps an immutable repeatable latch.
void aurora_set_vi_scanout_repeat_mode(bool enabled);
// Present the currently latched VI image at an absolute steady_clock deadline.
// A zero deadline presents as soon as the presenter can run. Returns false
// until the first scanout image has been latched.
