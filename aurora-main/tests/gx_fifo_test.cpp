...
TEST(GXPipelineConfig, RejectsInvalidDeserializedEnums) {
  ...
  config.shaderConfig.attrs[GX_VA_POS].attrType = GX_DIRECT;
...
TEST(GXPipelineConfig, RejectsCachedPipelineWithoutPosition) {
  aurora::gx::PipelineConfig config{};
  EXPECT_EQ(config.shaderConfig.attrs[GX_VA_POS].attrType, GX_NONE);
  EXPECT_FALSE(aurora::gx::valid_pipeline_config(config));
...
TEST(EfbRamEncoderContract, R8WritesRedChannelInNativeEightByFourTile) {
  std::array<u8, 8 * 4 * 4> rgba{};
  for (u32 i = 0; i < 32; ++i) {
    rgba[i * 4 + 0] = static_cast<u8>(0x20 + i);
    rgba[i * 4 + 1] = static_cast<u8>(0x80 + i);
    rgba[i * 4 + 2] = static_cast<u8>(0xC0 + i);
    rgba[i * 4 + 3] = static_cast<u8>(0xE0 + i);
  }

  std::array<u8, 32> encoded{};
  ASSERT_TRUE(aurora::gfx::efb_ram::encode(
      encoded.data(), encoded.size(), GX_CTF_R8, 8, 4, rgba.data(), 8, 4, 32,
      aurora::gfx::efb_ram::HostPixelOrder::RGBA));
  EXPECT_EQ(aurora::gfx::efb_ram::encoded_size(GX_CTF_R8, 8, 4), encoded.size());
  for (u32 i = 0; i < encoded.size(); ++i) {
    EXPECT_EQ(encoded[i], static_cast<u8>(0x20 + i)) << "R8 texel " << i;
  }
}
...
TEST(FrameInterpolationContract, IndexedPaletteHistoryKeepsAbsoluteVertexSlots) {
  ...
  // The current implementation stages palette uniforms before matching at seal
  // time. A topology mismatch must retain the current bytes, not blend slots.
  ASSERT_NE(changedRanges[0].size, 0u);
  aurora::gx::finalize_frame_interpolation();
  EXPECT_EQ(aurora::gfx::testing::uniform_allocation(0),
            (std::vector<uint8_t>(changedSource.begin(), changedSource.end())));
...
TEST(TevRegisterLivenessContract, PacksOneUniformWhenBothHalvesNeedInitialValue) {
  ...
  // The allocation is padded to 256 bytes; 16 bytes can fit in existing padding.
  EXPECT_GE(info.uniformSize, baselineInfo.uniformSize);
  EXPECT_EQ(info.uniformSize % 256u, 0u);
}
...
TEST_F(GXFifoTest, RawBpWriteFollowsAlreadyQueuedHostBlendState) {
  GXSetBlendMode(GX_BM_BLEND, GX_BL_ONE, GX_BL_ZERO, GX_LO_NOOP);
  ASSERT_GT(aurora::gx::fifo::get_buffer_size(), 0u);

  // The raw register is later in the guest command stream than the host
  // command above. Draining the queue must not resurrect the old blend mode.
  const u32 rawBlend = 1u | (static_cast<u32>(GX_BL_INVSRCALPHA) << 5) |
                       (static_cast<u32>(GX_BL_SRCALPHA) << 8);
  GXApplyBPReg(0x41, rawBlend);
  aurora::gx::fifo::drain();
  EXPECT_EQ(gxState().blendFacSrc, GX_BL_SRCALPHA);
  EXPECT_EQ(gxState().blendFacDst, GX_BL_INVSRCALPHA);
}

TEST_F(GXFifoTest, RawBpWriteConsumesEarlierQueuedMaskInOrder) {
  GXApplyBPReg(0x41, 0u);
  const auto mask = bp_cmd(0xFE, 1u);
  aurora::gx::fifo::write_data(mask.data(), static_cast<u32>(mask.size()));
  GXApplyBPReg(0x41, 0x00ffffffu);
  aurora::gx::fifo::drain();
  EXPECT_EQ(gxState().bpRegCache[0x41] & 0x00ffffffu, 1u);
  EXPECT_EQ(gxState().bpRegCache[0xFE] & 0x00ffffffu, 0x00ffffffu);
}

TEST_F(GXFifoTest, RawBpWriteFollowsDeferredHostGenMode) {
  GXSetNumTevStages(3);
  ASSERT_NE(__gx->dirtyState & 4u, 0u);
  GXApplyBPReg(0x00, 0u); // One TEV stage; raw command comes last.
  __GXSetDirtyState();
  aurora::gx::fifo::drain();
  EXPECT_EQ(gxState().numTevStages, 1u);
}

TEST_F(GXFifoTest, RawTextureBindingFollowsQueuedHostTextureMetadata) {
  alignas(32) u8 first[256]{};
  alignas(32) u8 second[256]{};
  GXTexObj obj{};
  GXInitTexObj(&obj, first, 8, 8, GX_TF_RGBA8, GX_CLAMP, GX_CLAMP, GX_FALSE);
  GXLoadTexObj(&obj, GX_TEXMAP0);
  ASSERT_GT(aurora::gx::fifo::get_buffer_size(), 0u);

  // This is the same boundary used by GX_HLE_ApplyBPReg for a raw texture
  // burst: apply hardware state, then publish its translated source pointer.
  GXApplyBPReg(0x88, 15u | (15u << 10) | (static_cast<u32>(GX_TF_I8) << 20));
  GXApplyBPReg(0x94, 0x1234u);
  GXSetDirectTextureData(GX_TEXMAP0, second);
  aurora::gx::fifo::drain();
  const auto& slot = gxState().loadedTextures[GX_TEXMAP0];
  EXPECT_EQ(slot.data, second);
  EXPECT_EQ(slot.width(), 16u);
  EXPECT_EQ(slot.height(), 16u);
  EXPECT_EQ(slot.format(), static_cast<u32>(GX_TF_I8));
  EXPECT_EQ(slot.texObjId, 0u);
  EXPECT_EQ(slot.texDataVersion, 0u);
}

TEST_F(GXFifoTest, RawTextureBindingClearsObjectIdentityButAllowsValidatedSourceCache) {
  u8 first[32]{};
  u8 second[32]{};
  auto& slot = gxState().loadedTextures[GX_TEXMAP0];
  slot.data = first;
  slot.texObjId = 73;
  slot.texDataVersion = 12;
  slot.set_no_cache(true);
  gxState().stateDirty = false;
  GXSetDirectTextureData(GX_TEXMAP0, first);
  EXPECT_EQ(slot.data, first);
  EXPECT_EQ(slot.texObjId, 0u);
  EXPECT_EQ(slot.texDataVersion, 0u);
  ...
  gxState().stateDirty = false;
  GXSetDirectTextureData(GX_TEXMAP0, first);
  EXPECT_FALSE(gxState().stateDirty);
  GXSetDirectTextureData(GX_TEXMAP0, second);
  EXPECT_EQ(slot.data, second);
  EXPECT_EQ(slot.texObjId, 0u);
  EXPECT_FALSE(slot.no_cache());
...
TEST_F(GXFifoTest, RawBpC8TlutLoadSnapshotsPaletteByTmem) {
  alignas(32) u8 image[256]{};
  alignas(32) u16 palette[256]{};
  for (u32 i = 0; i < 256; ++i) {
    palette[i] = static_cast<u16>(0x8000u | (i & 0x7fffu));
  }

  // Reproduce the hardware shape observed in Tom & Jerry: C8 texture,
  // LOADTLUT destination 0x200, GX_TLUT_256, then TX_SETTLUT RGB5A3.
  GXApplyBPReg(0x88, 255u | (255u << 10) | (static_cast<u32>(GX_TF_C8) << 20));
  GXApplyBPReg(0x94, 0x1234u);
  GXSetDirectTextureData(GX_TEXMAP0, image);
  GXApplyBPReg(0x64, 0xA0D26Au);
  GXApplyBPReg(0x65, 0x200u | (16u << 10));
  GXSetDirectTlutData(0x200u, palette, sizeof(palette));
  GXApplyBPReg(0x98, 0x200u | (static_cast<u32>(GX_TL_RGB5A3) << 10));

  const auto& tex = gxState().loadedTextures[GX_TEXMAP0];
  EXPECT_EQ(tex.rawTlutTmem, 0x200u);
  EXPECT_EQ(tex.rawTlutFormat, GX_TL_RGB5A3);

  const auto rawIt = gxState().rawTluts.find(0x200u);
  ASSERT_NE(rawIt, gxState().rawTluts.end());
  // GXTlutObj_::loadTlut0 stores the complete BP word, including register 0x64,
  // just like GXInitTlutObj does for the SDK path.  The low 24 bits are the
  // LOADTLUT0 payload/source field we care about here.
  EXPECT_EQ(rawIt->second.loadTlut0 >> 24, 0x64u);
  EXPECT_EQ(rawIt->second.loadTlut0 & 0x00ffffffu, 0xA0D26Au);
  EXPECT_EQ(rawIt->second.numEntries, 256u);
  ASSERT_EQ(rawIt->second.data.size(), sizeof(palette));
  EXPECT_EQ(std::memcmp(rawIt->second.data.data(), palette, sizeof(palette)), 0);

  // Reissuing the complete BP RAM->TMEM transaction with the same source and
  // bytes leaves the resulting palette unchanged. BP 0x65 must not clear the
  // committed palette while the embedding runtime is between the register
  // write and GXSetDirectTlutData, and the eventual commit must keep the host
  // cache version stable rather than reconvert/reuploading every glyph draw.
  const u32 firstVersion = rawIt->second.dataVersion;
  gxState().stateDirty = false;
  GXApplyBPReg(0x64, 0xA0D26Au);
  GXApplyBPReg(0x65, 0x200u | (16u << 10));
  ASSERT_EQ(rawIt->second.data.size(), sizeof(palette));
  EXPECT_EQ(std::memcmp(rawIt->second.data.data(), palette, sizeof(palette)), 0);
  EXPECT_EQ(rawIt->second.dataVersion, firstVersion);
  EXPECT_FALSE(gxState().stateDirty);
  GXSetDirectTlutData(0x200u, palette, sizeof(palette));
  EXPECT_EQ(rawIt->second.dataVersion, firstVersion);
  EXPECT_FALSE(gxState().stateDirty);

  // A real palette-byte change is still observable and must invalidate the
  // renderer-side resolved texture.
  palette[1] ^= 1u;
  GXApplyBPReg(0x64, 0xA0D26Au);
  GXApplyBPReg(0x65, 0x200u | (16u << 10));
  GXSetDirectTlutData(0x200u, palette, sizeof(palette));
  EXPECT_EQ(rawIt->second.dataVersion, firstVersion + 1u);
  EXPECT_TRUE(gxState().stateDirty);

  // LOADTLUT is a RAM->TMEM transfer, not a live pointer binding.
  palette[0] = 0;
  EXPECT_NE(std::memcmp(rawIt->second.data.data(), palette, sizeof(palette)), 0);
}
...
TEST_F(GXFifoTest, MaximumQuadCountDoesNotWrapTheTopologyLoop) {
  g_gxState.lastVtxFmt = GX_VTXFMT0;
  g_gxState.lastVtxSize = 1;
  g_gxState.stateDirty = true;
  std::vector<u8> fifo;
  append_test_draw(fifo, GX_QUADS, 65535);
  decode_fifo(fifo);
  const auto& indices = aurora::gfx::testing::last_pushed_indices();
  ASSERT_EQ(indices.size(), 16384u * 6u);
  EXPECT_EQ(indices.back(), 65532u);
}

TEST_F(GXFifoTest, StagingRotationPrecedesFifoAndRawDrawUploads) {
  for (bool raw : {false, true}) {
    for (bool vertexPressure : {false, true}) {
      // Raw submission first drains the SDK encoder. Finish setup before
      // installing this deliberately minimal decoded one-byte test format.
      (void)flush_and_capture();
      aurora::gfx::testing::reset_vertex_push_record();
      g_gxState.lastVtxFmt = GX_VTXFMT0;
      g_gxState.lastVtxSize = 1;
      g_gxState.stateDirty = true;
      aurora::gfx::testing::set_staging_usage(
          vertexPressure ? aurora::gfx::vertex_capacity() - 2 : 0,
          vertexPressure ? 0 : aurora::gfx::index_capacity() - 2, 0);
      if (raw) {
        const u8 vertices[]{0, 1, 2};
        ASSERT_TRUE(aurora::gx::fifo::submit_raw_draw(GX_TRIANGLES, GX_VTXFMT0, vertices, 3, 3));
      } else {
        std::vector<u8> fifo;
        append_test_draw(fifo, GX_TRIANGLES, 3);
        decode_fifo(fifo);
      }
      EXPECT_EQ(aurora::gfx::testing::batch_splits(), 1u);
      EXPECT_EQ(aurora::gfx::testing::last_pushed_vertices(), (std::vector<u8>{0, 1, 2}));
      EXPECT_EQ(aurora::gfx::testing::last_pushed_indices(), (std::vector<u16>{0, 1, 2}));
      EXPECT_EQ(aurora::gfx::current_frame(), 0u);
    }
  }
}
...
TEST_F(GXFifoTest, InvalidateVtxCache_PreventsMergingAcrossArrayReload) {
  g_gxState.lastVtxFmt = GX_VTXFMT0;
  g_gxState.lastVtxSize = 1;
  g_gxState.stateDirty = true;
  aurora::gfx::testing::use_draw_command_tracking(true);

  // The cache invalidation means the next draw must get a fresh array upload
  // and uniform offsets, even when all array bindings and VAT fields match.
  std::vector<u8> fifo;
  append_test_draw(fifo, GX_TRIANGLES, 3);
  fifo.push_back(GX_CMD_INVL_VC);
  append_test_draw(fifo, GX_TRIANGLES, 3);
  decode_fifo(fifo);

  EXPECT_EQ(aurora::gfx::g_mergedDrawCallCount, 0u);
  EXPECT_EQ(aurora::gfx::testing::last_pushed_indices(),
            (std::vector<u16>{0, 1, 2}));
}

class IndexedArrayUploadTest : public GXFifoTest {
protected:
  std::array<u8, 6> positions{0, 0, 8, 0, 0, 8};
  std::array<u8, 6> texcoords{0, 0, 255, 0, 0, 255};
  inline static u64 sourceGeneration = 1;
  AuroraGuestWriteGenerationCallback previousHook = nullptr;
  const std::vector<u8> draw{static_cast<u8>(GX_TRIANGLES), 0, 3, 0, 0, 1, 1, 2, 2};

  void SetUp() override {
    GXFifoTest::SetUp();
    // The fixture installs raw decoded state below. Finish GXInit's queued
    // host defaults first so submit_raw_draw cannot overwrite that setup.
    decode_fifo(flush_and_capture());
    reset_gx_state();
    previousHook = aurora::g_guestWriteGenerationHook;
    sourceGeneration = 1;
    aurora::g_guestWriteGenerationHook = [](const void*, size_t) -> u64 { return sourceGeneration; };
    aurora::gfx::testing::use_storage_upload_tracking(true);
    aurora::gfx::testing::use_draw_command_tracking(true);
    aurora::gfx::testing::use_real_vertex_format_helpers(true);
    auto& state = gxState();
    state.vtxDesc[GX_VA_POS] = GX_INDEX8;
    state.vtxDesc[GX_VA_TEX0] = GX_INDEX8;
    state.vtxFmts[0].attrs[GX_VA_POS] = {.cnt = GX_POS_XY, .type = GX_U8, .frac = 0};
    state.vtxFmts[0].attrs[GX_VA_TEX0] = {.cnt = GX_TEX_ST, .type = GX_U8, .frac = 0};
    state.arrays[GX_VA_POS].data = positions.data();
    state.arrays[GX_VA_POS].size = positions.size();
    state.arrays[GX_VA_POS].stride = 2;
    state.arrays[GX_VA_TEX0].data = texcoords.data();
    state.arrays[GX_VA_TEX0].size = texcoords.size();
    state.arrays[GX_VA_TEX0].stride = 2;
  ...
  void TearDown() override {
    aurora::g_guestWriteGenerationHook = previousHook;
    aurora::gfx::testing::use_storage_upload_tracking(false);
    aurora::gfx::testing::use_draw_command_tracking(false);
  }
};

TEST_F(IndexedArrayUploadTest, ChangedGenerationRefreshesPosAndUvBeforeMerge) {
  decode_fifo(draw);
  ASSERT_EQ(aurora::gfx::testing::storage_uploads().size(), 2u);
  ASSERT_GT(gxState().arrays[GX_VA_POS].cachedRange.size, 0u);
  positions[2] = 16;
  texcoords[2] = 128;
  ++sourceGeneration; // Out-of-FIFO DC publication, no rebind or INVL_VC.
  decode_fifo(draw);
  ASSERT_EQ(aurora::gfx::testing::storage_uploads().size(), 4u);
  EXPECT_EQ(aurora::gfx::testing::storage_uploads()[2], (std::vector<u8>(positions.begin(), positions.end())));
  EXPECT_EQ(aurora::gfx::testing::storage_uploads()[3], (std::vector<u8>(texcoords.begin(), texcoords.end())));
  EXPECT_EQ(aurora::gfx::g_mergedDrawCallCount, 0u);
}

TEST_F(IndexedArrayUploadTest, UnchangedTrackedGenerationReusesUploadAndMerge) {
  decode_fifo(draw);
  decode_fifo(draw);
  EXPECT_EQ(aurora::gfx::testing::storage_uploads().size(), 2u);
  EXPECT_EQ(aurora::gfx::g_mergedDrawCallCount, 1u);
}

TEST_F(IndexedArrayUploadTest, UntrackedSourceIsNeverProvedUnchanged) {
  sourceGeneration = aurora::kGuestWriteUntracked;
  decode_fifo(draw);
  positions[2] = 32;
  decode_fifo(draw);
  ASSERT_EQ(aurora::gfx::testing::storage_uploads().size(), 4u);
...
TEST_F(IndexedArrayUploadTest, ExplicitVertexInvalidationStillForcesFreshUpload) {
  decode_fifo(draw);
  decode_fifo({GX_CMD_INVL_VC});
  decode_fifo(draw);
  EXPECT_EQ(aurora::gfx::testing::storage_uploads().size(), 4u);
  EXPECT_EQ(aurora::gfx::g_mergedDrawCallCount, 0u);
}

TEST_F(IndexedArrayUploadTest, RawBridgeAlsoRefreshesChangedSource) {
  ASSERT_TRUE(aurora::gx::fifo::submit_raw_draw(GX_TRIANGLES, GX_VTXFMT0, draw.data() + 3, 3, 6));
  ASSERT_EQ(aurora::gfx::testing::storage_uploads().size(), 2u);
  positions[2] = 48;
  ++sourceGeneration;
  ASSERT_TRUE(aurora::gx::fifo::submit_raw_draw(GX_TRIANGLES, GX_VTXFMT0, draw.data() + 3, 3, 6));
...
TEST_F(GXFifoTest, DrawMergeDoesNotReusePreviousVatFromOpcode) {
  // Both VATs are installed before either draw. Selecting a different VAT in
  // the command byte is not a CP write, but changes the vertex interpretation.
  g_gxState.vtxDesc[GX_VA_POS] = GX_DIRECT;
  for (int i = 0; i < 2; ++i) {
    auto& position = g_gxState.vtxFmts[i].attrs[GX_VA_POS];
    position.cnt = GX_POS_XY;
    position.type = GX_U8;
    position.frac = static_cast<u8>(i);
  }
  g_gxState.lastVtxFmt = GX_VTXFMT0;
  g_gxState.lastVtxSize = 2;
  g_gxState.stateDirty = true;
  aurora::gfx::testing::use_real_vertex_format_helpers(true);
  aurora::gfx::testing::use_draw_command_tracking(true);

  const std::vector<u8> fifo{
      static_cast<u8>(GX_TRIANGLES | GX_VTXFMT0), 0, 3, 0, 0, 2, 0, 0, 2,
      static_cast<u8>(GX_TRIANGLES | GX_VTXFMT1), 0, 3, 0, 0, 4, 0, 0, 4,
  };
  decode_fifo(fifo);
...
TEST_F(GXFifoTest, DrawMergeDoesNotUseLineExpansionForFollowingTriangle) {
  g_gxState.lastVtxFmt = GX_VTXFMT0;
  g_gxState.lastVtxSize = 1;
  g_gxState.stateDirty = true;
  aurora::gfx::testing::use_draw_command_tracking(true);

  // One line has instanceCount==1, just like one ordinary triangle draw.
  std::vector<u8> fifo;
  append_test_draw(fifo, GX_LINES, 2);
  append_test_draw(fifo, GX_TRIANGLES, 3);
  decode_fifo(fifo);
...
TEST_F(GXFifoTest, DrawMergeSplitsBeforeSixteenBitVertexIndexOverflow) {
  g_gxState.lastVtxFmt = GX_VTXFMT0;
  g_gxState.lastVtxSize = 1;
  g_gxState.stateDirty = true;
  aurora::gfx::testing::use_draw_command_tracking(true);
  ...
  std::vector<u8> fifo;
  append_test_draw(fifo, GX_TRIANGLES, 65535);
  append_test_draw(fifo, GX_TRIANGLES, 3);
  decode_fifo(fifo);
...
TEST_F(GXFifoTest, DrawMergePreservesLargestRepresentableVertexIndex) {
  g_gxState.lastVtxFmt = GX_VTXFMT0;
  g_gxState.lastVtxSize = 1;
  g_gxState.stateDirty = true;
  aurora::gfx::testing::use_draw_command_tracking(true);

  // Index 65535 is still valid. Different triangle-list-compatible primitive
  // opcodes do not themselves require a split; only overflow does.
  std::vector<u8> fifo;
  append_test_draw(fifo, GX_TRIANGLES, 65532);
  append_test_draw(fifo, GX_QUADS, 4);
  decode_fifo(fifo);

  EXPECT_EQ(aurora::gfx::g_mergedDrawCallCount, 1u);
  EXPECT_EQ(aurora::gfx::testing::last_pushed_indices(),
            (std::vector<u16>{65532, 65533, 65534, 65534, 65535, 65532}));
}
...
TEST_F(GXFifoTest, RawTextureInvalidationExecutesEvenWhenTheBpWordRepeats) {
  for (int repetition = 0; repetition < 3; ++repetition) {
    gxState().stateDirty = false;
    GXApplyBPReg(0x66, 0x1000u);
    EXPECT_TRUE(gxState().stateDirty);
    EXPECT_EQ(gxState().bpRegCache[0x66] & 0xffffffu, 0x1000u);
  }
}

TEST_F(GXFifoTest, RawTextureInvalidationHonorsBpMaskAndConsumesIt) {
  GXApplyBPReg(0x66, 0x1000u);
  GXApplyBPReg(0xFE, 0x100u);
  gxState().stateDirty = false;
  GXApplyBPReg(0x66, 0x100u);
  EXPECT_TRUE(gxState().stateDirty);
  EXPECT_EQ(gxState().bpRegCache[0x66] & 0xffffffu, 0x1100u);
  EXPECT_EQ(gxState().bpRegCache[0xFE], 0xffffffu);
}

TEST_F(GXFifoTest, TexObjRawDimensionsDecodeTenBitMaximumAs1024) {
  auto& slot = gxState().loadedTextures[GX_TEXMAP0];
  slot.image0 = (0x3FFu << 0) | (0x3FFu << 10);
  slot.mWidth = 0;
  slot.mHeight = 0;

  // BP stores width-1 and height-1, so the maximum fields encode 1024.
...
TEST_F(GXFifoTest, LoadNrmMtx_RawRowSubCopiesPreserveOffsets) {
  // WOS emits indexed XF normal-matrix loads as three independent 3-word
  // transfers to 0x400, 0x403 and 0x406. They address consecutive XF words;
  // each transfer must land in its own 3-float row rather than overwrite row 0.
  std::vector<u8> bytes;
  const auto append = [&bytes](const std::vector<u8>& cmd) {
    bytes.insert(bytes.end(), cmd.begin(), cmd.end());
  };
  append(xf_cmd(0x400, {0x3F800000u, 0x40000000u, 0x40400000u})); // 1,2,3
  append(xf_cmd(0x403, {0x40800000u, 0x40A00000u, 0x40C00000u})); // 4,5,6
  append(xf_cmd(0x406, {0x40E00000u, 0x41000000u, 0x41100000u})); // 7,8,9

  reset_gx_state();
  decode_fifo(bytes);

  const auto& decoded = g_gxState.pnMtx[0].nrm;
  EXPECT_FLOAT_EQ(decoded.m0[0], 1.0f);
  EXPECT_FLOAT_EQ(decoded.m0[1], 2.0f);
  EXPECT_FLOAT_EQ(decoded.m0[2], 3.0f);
  EXPECT_FLOAT_EQ(decoded.m1[0], 4.0f);
  EXPECT_FLOAT_EQ(decoded.m1[1], 5.0f);
  EXPECT_FLOAT_EQ(decoded.m1[2], 6.0f);
  EXPECT_FLOAT_EQ(decoded.m2[0], 7.0f);
  EXPECT_FLOAT_EQ(decoded.m2[1], 8.0f);
...
// --- GXLoadTexMtxImm 3x4 (XF 0x078-0x0FB, including GX_IDENTITY) ---
...
TEST_F(GXFifoTest, Viewport_Raw64MatchesHardwareScissorOrigin) {
  // Raw retail XF words, independent of the encoder under test.
  // Hardware viewport and scissor share origin bias 342.
  decode_fifo(xf_cmd(0x101A, {
      0x42000000u, 0xC2000000u, 0x4B7FFFFFu,
      0x43BB0000u, 0x43BB0000u, 0x4B7FFFFFu})); // 32,-32,Z,374,374,Z
  decode_fifo(bp_cmd(0x20, 0x156156u));
  decode_fifo(bp_cmd(0x21, 0x195195u));
  EXPECT_FLOAT_EQ(g_gxState.logicalViewport.left, 0.0f);
  EXPECT_FLOAT_EQ(g_gxState.logicalViewport.top, 0.0f);
  EXPECT_FLOAT_EQ(g_gxState.logicalViewport.width, 64.0f);
  EXPECT_FLOAT_EQ(g_gxState.logicalViewport.height, 64.0f);
  EXPECT_EQ(g_gxState.logicalScissor.x, 0);
  EXPECT_EQ(g_gxState.logicalScissor.y, 0);
  EXPECT_EQ(g_gxState.logicalScissor.width, 64);
  EXPECT_EQ(g_gxState.logicalScissor.height, 64);
}

TEST_F(GXFifoTest, Viewport_Raw128MatchesHardwareScissorOrigin) {
  decode_fifo(xf_cmd(0x101A, {
      0x42800000u, 0xC2800000u, 0x4B7FFFFFu,
      0x43CB0000u, 0x43CB0000u, 0x4B7FFFFFu})); // 64,-64,Z,406,406,Z
  decode_fifo(bp_cmd(0x20, 0x156156u));
  decode_fifo(bp_cmd(0x21, 0x1D51D5u));
  EXPECT_FLOAT_EQ(g_gxState.logicalViewport.left, 0.0f);
  EXPECT_FLOAT_EQ(g_gxState.logicalViewport.top, 0.0f);
  EXPECT_FLOAT_EQ(g_gxState.logicalViewport.width, 128.0f);
  EXPECT_FLOAT_EQ(g_gxState.logicalViewport.height, 128.0f);
  EXPECT_EQ(g_gxState.logicalScissor.x, 0);
  EXPECT_EQ(g_gxState.logicalScissor.y, 0);
  EXPECT_EQ(g_gxState.logicalScissor.width, 128);
  EXPECT_EQ(g_gxState.logicalScissor.height, 128);
}

TEST_F(GXFifoTest, Viewport_EncoderUsesHardwareOriginBias) {
  GXSetViewport(0.0f, 0.0f, 64.0f, 64.0f, 0.0f, 1.0f);
  const auto bytes = capture_fifo();
  ASSERT_EQ(bytes.size(), 29u);
  EXPECT_EQ(bytes[0], 0x10u);
  EXPECT_EQ(read_be32_at(bytes, 1), 0x0005101Au);
  EXPECT_EQ(read_be32_at(bytes, 17), 0x43BB0000u); // 342 + 32 = 374
  EXPECT_EQ(read_be32_at(bytes, 21), 0x43BB0000u);
}
...
TEST_F(GXFifoTest, MatrixIndexA_CpReplayRestoresStateAfterXfWrite) {
  const u32 first = GX_PNMTX3 | (GX_TEXMTX3 << 6) | (GX_TEXMTX4 << 12) |
                    (GX_IDENTITY << 18) | (GX_TEXMTX7 << 24);
  const u32 other = GX_PNMTX1 | (GX_IDENTITY << 6) | (GX_TEXMTX0 << 12) |
                    (GX_TEXMTX1 << 18) | (GX_TEXMTX2 << 24);
  decode_fifo(cp_cmd(0x30, first));
  decode_fifo(xf_cmd(0x1018, {other}));
  ASSERT_EQ(g_gxState.currentPnMtx, 1u);
  ASSERT_EQ(g_gxState.tcgs[0].mtx, GX_IDENTITY);
  decode_fifo(cp_cmd(0x30, first));
  EXPECT_EQ(g_gxState.currentPnMtx, 3u);
  EXPECT_EQ(g_gxState.tcgs[0].mtx, GX_TEXMTX3);
  EXPECT_EQ(g_gxState.tcgs[1].mtx, GX_TEXMTX4);
  EXPECT_EQ(g_gxState.tcgs[2].mtx, GX_IDENTITY);
  EXPECT_EQ(g_gxState.tcgs[3].mtx, GX_TEXMTX7);
  // The reverse direction must keep working as well.
  decode_fifo(xf_cmd(0x1018, {other}));
  EXPECT_EQ(g_gxState.currentPnMtx, 1u);
  EXPECT_EQ(g_gxState.tcgs[0].mtx, GX_IDENTITY);
}

TEST_F(GXFifoTest, MatrixIndexB_CpReplayRestoresStateAfterXfWrite) {
  const u32 first = GX_TEXMTX4 | (GX_TEXMTX5 << 6) | (GX_IDENTITY << 12) |
                    (GX_TEXMTX9 << 18);
  const u32 other = GX_IDENTITY | (GX_TEXMTX0 << 6) | (GX_TEXMTX1 << 12) |
                    (GX_TEXMTX2 << 18);
  decode_fifo(cp_cmd(0x40, first));
  decode_fifo(xf_cmd(0x1019, {other}));
  ASSERT_EQ(g_gxState.tcgs[4].mtx, GX_IDENTITY);
  decode_fifo(cp_cmd(0x40, first));
  EXPECT_EQ(g_gxState.tcgs[4].mtx, GX_TEXMTX4);
  EXPECT_EQ(g_gxState.tcgs[5].mtx, GX_TEXMTX5);
  EXPECT_EQ(g_gxState.tcgs[6].mtx, GX_IDENTITY);
  EXPECT_EQ(g_gxState.tcgs[7].mtx, GX_TEXMTX9);
  decode_fifo(xf_cmd(0x1019, {other}));
  EXPECT_EQ(g_gxState.tcgs[4].mtx, GX_IDENTITY);
}

TEST_F(GXFifoTest, NumChans_RawXfChangeInvalidatesPipelineMemo) {
  decode_fifo(xf_cmd(0x1009, {0}));
  const u32 before = g_gxState.pipelineStateGeneration;
  decode_fifo(xf_cmd(0x1009, {1}));
...
TEST_F(GXFifoTest, ChanAmbColor_Color0) {
  ...
  EXPECT_NEAR(state.ambColor[3], 17.f / 255.f, 1.f / 255.f);
}

TEST_F(GXFifoTest, LoadTexMtx3x4_GxIdentityPhysicalSlot) {
  aurora::Mat3x4<float> mtx{};
  mtx.m0[0] = 1.0f;
  mtx.m1[1] = 1.0f;
  mtx.m2[2] = 1.0f;
  ...
  ASSERT_GT(g_gxState.texMtxs.size(), 10u);
  const auto& decoded = g_gxState.texMtxs[10];
  EXPECT_FLOAT_EQ(decoded.m0[0], 1.0f);
  EXPECT_FLOAT_EQ(decoded.m1[1], 1.0f);
  EXPECT_FLOAT_EQ(decoded.m2[2], 1.0f);
  EXPECT_FLOAT_EQ(decoded.m0[3], 0.0f);
...
TEST_F(GXFifoTest, ChanMatColor_Color0) {
  ...
  EXPECT_NEAR(state.matColor[3], 23.f / 255.f, 1.f / 255.f);
}

TEST_F(GXFifoTest, ChanAmbColor_Color1) {
  GXColor initial = {1, 2, 3, 29};
  ...
  EXPECT_NEAR(state.ambColor[3], 29.f / 255.f, 1.f / 255.f);
}

TEST_F(GXFifoTest, ChanMatColor_Color1) {
  GXColor initial = {1, 2, 3, 31};
  ...
  EXPECT_NEAR(state.matColor[3], 31.f / 255.f, 1.f / 255.f);
}
...
TEST_F(GXFifoTest, ChanAmbColor_ComponentWritesPreserveSharedRegister) {
  GXColor initial = {20, 40, 60, 80};
  GXSetChanAmbColor(GX_COLOR0A0, initial);
  GXColor rgbOnly = {100, 120, 140, 1};
  GXSetChanAmbColor(GX_COLOR0, rgbOnly);
  GXColor alphaOnly = {2, 3, 4, 200};
  GXSetChanAmbColor(GX_ALPHA0, alphaOnly);
  auto bytes = capture_fifo();
  ...
  auto& state = g_gxState.colorChannelState[GX_COLOR0];
  EXPECT_NEAR(state.ambColor[0], 100.f / 255.f, 1.f / 255.f);
  EXPECT_NEAR(state.ambColor[1], 120.f / 255.f, 1.f / 255.f);
  EXPECT_NEAR(state.ambColor[2], 140.f / 255.f, 1.f / 255.f);
  EXPECT_NEAR(state.ambColor[3], 200.f / 255.f, 1.f / 255.f);
}

TEST_F(GXFifoTest, ChanMatColor_ComponentWritesPreserveSharedRegister) {
  GXColor initial = {30, 50, 70, 90};
  GXSetChanMatColor(GX_COLOR1A1, initial);
  GXColor alphaOnly = {1, 2, 3, 210};
  GXSetChanMatColor(GX_ALPHA1, alphaOnly);
  GXColor rgbOnly = {110, 130, 150, 4};
  GXSetChanMatColor(GX_COLOR1, rgbOnly);
  auto bytes = capture_fifo();
  ...
  auto& state = g_gxState.colorChannelState[GX_COLOR1];
  EXPECT_NEAR(state.matColor[0], 110.f / 255.f, 1.f / 255.f);
  EXPECT_NEAR(state.matColor[1], 130.f / 255.f, 1.f / 255.f);
  EXPECT_NEAR(state.matColor[2], 150.f / 255.f, 1.f / 255.f);
  EXPECT_NEAR(state.matColor[3], 210.f / 255.f, 1.f / 255.f);
}
...
TEST_F(GXFifoTest, DispCopyState_EncodesBpAndYScale) {
  ...
  EXPECT_EQ(g_gxState.dispCopyDstWidth, 640u);
  EXPECT_NEAR(g_gxState.dispCopyYScale, 1.0f, 0.001f);
}

TEST_F(GXFifoTest, RawDispCopyDstPitchDecodesThirtyTwoByteUnits) {
  // Tom & Jerry programs COPY_DST directly as 0x28.  That is a 1280-byte
  // YUYV row pitch, i.e. exactly 640 pixels, not 655.
  GXApplyBPReg(0x4D, 0x28u);
  EXPECT_EQ(g_gxState.dispCopyDstWidth, 640u);
}
...
TEST_F(GXFifoTest, CopyTrigger_DecodesClampGammaFormatHalfScaleAndFrameMode) {
  // BP stores the EFB copy mode rotated: raw 0xC decodes to real format 0x6 (RGBA8).
  const u32 value = (static_cast<u32>(GX_CLAMP_TOP) << 0) | (0xCu << 3) |
...
TEST_F(GXFifoTest, CopyTrigger_DecodesRawEfbCopyFormatRotation) {
  reset_gx_state();

  // real = (raw >> 1) | ((raw & 1) << 3)
  decode_fifo(bp_cmd(0x52, 0x8u << 3));
  EXPECT_EQ(g_gxState.texCopyFmt, GX_TF_RGB565);

  decode_fifo(bp_cmd(0x52, 0x1u << 3));
  EXPECT_EQ(g_gxState.texCopyFmt, GX_CTF_R8);

  decode_fifo(bp_cmd(0x52, 0xEu << 3));
  EXPECT_EQ(g_gxState.texCopyFmt, GX_CTF_A8);
}

TEST_F(GXFifoTest, CopyTrigger_UsesIntensityOnlyWithAutomaticConversion) {
  reset_gx_state();

  decode_fifo(bp_cmd(0x52, 1u << 15));
  EXPECT_EQ(g_gxState.texCopyFmt, GX_CTF_R4);

  decode_fifo(bp_cmd(0x52, (1u << 15) | (1u << 16)));
  EXPECT_EQ(g_gxState.texCopyFmt, GX_TF_I4);
}

TEST_F(GXFifoTest, XfbCopyTrigger_DoesNotClobberTextureCopyFormat) {
  reset_gx_state();
  g_gxState.texCopyFmt = GX_TF_RGBA8;

  decode_fifo(bp_cmd(0x52, (0x8u << 3) | (1u << 14)));

  EXPECT_EQ(g_gxState.texCopyFmt, GX_TF_RGBA8);
}

TEST_F(GXFifoTest, RawCopyTriggerLatchesTextureRectangleAndImageSize) {
  GXSetTexCopySrc(12, 24, 8, 8);
  GXSetTexCopyDst(8, 8, GX_TF_RGBA8, GX_FALSE);
  GXApplyBPReg(0x49, 4u | (6u << 10));
  GXApplyBPReg(0x4A, 127u | (63u << 10));
  GXApplyBPReg(0x4D, 32u);
  GXApplyBPReg(0x52, (0x8u << 3) | (1u << 16));
  EXPECT_EQ(gxState().texCopySrc, (aurora::gfx::ClipRect{4, 6, 128, 64}));
  EXPECT_FALSE(gxState().texCopySrcRenderSpace);
  EXPECT_EQ(gxState().texCopyDstWidth, 128u);
  EXPECT_EQ(gxState().texCopyDstHeight, 64u);
  EXPECT_EQ(gxState().texCopyFmt, GX_TF_RGB565);

  // Same trigger word, different source: this is not a redundant state write.
  GXApplyBPReg(0x4A, 63u | (31u << 10));
  GXApplyBPReg(0x52, (0x8u << 3) | (1u << 16));
  EXPECT_EQ(gxState().texCopyDstWidth, 64u);
  EXPECT_EQ(gxState().texCopyDstHeight, 32u);
}

TEST_F(GXFifoTest, RawCopyHalfScaleUsesSourceDimensionsNotDestinationPitch) {
  GXApplyBPReg(0x49, 0u);
  GXApplyBPReg(0x4A, 127u | (127u << 10));
  GXApplyBPReg(0x4D, 64u); // Padded row pitch does not resize the source image.
  GXApplyBPReg(0x52, (0x8u << 3) | (1u << 9) | (1u << 16));
  EXPECT_EQ(gxState().texCopySrc, (aurora::gfx::ClipRect{0, 0, 128, 128}));
  EXPECT_EQ(gxState().texCopyDstWidth, 64u);
  EXPECT_EQ(gxState().texCopyDstHeight, 64u);
  EXPECT_TRUE(gxState().texCopyHalfScale);
}

TEST_F(GXFifoTest, XfbTriggerLeavesTheLastTextureCopyRectangleIntact) {
  GXSetTexCopySrc(8, 12, 128, 64);
  GXSetTexCopyDst(128, 64, GX_TF_RGB565, GX_FALSE);
  GXApplyBPReg(0x49, 0u);
  GXApplyBPReg(0x4A, 639u | (447u << 10));
  GXApplyBPReg(0x52, 1u << 14);
  EXPECT_EQ(gxState().texCopySrc, (aurora::gfx::ClipRect{8, 12, 128, 64}));
  EXPECT_EQ(gxState().texCopyDstWidth, 128u);
  EXPECT_EQ(gxState().texCopyDstHeight, 64u);
}

TEST_F(GXFifoTest, SetTexCopyDst_UsesLogicalSdkFormatWithoutBpRotation) {
  GXSetTexCopyDst(8, 4, GX_TF_RGB565, GX_FALSE);
...
TEST_F(GXFifoTest, CopyTexCtfFormatsCanBackCompatibleTextureFormats) {
  ...
  EXPECT_TRUE(aurora::gx::copy_texture_format_compatible(GX_CTF_R8, GX_TF_C8));
  EXPECT_TRUE(aurora::gx::copy_texture_format_compatible(GX_CTF_G8, GX_TF_C8));
  EXPECT_TRUE(aurora::gx::copy_texture_format_compatible(GX_CTF_B8, GX_TF_C8));
  EXPECT_TRUE(aurora::gx::copy_texture_format_compatible(GX_CTF_A8, GX_TF_C8));
...
TEST_F(GXFifoTest, RawPTTexMtx2x4LoadUpdatesEightWordsAndPreservesThirdRow) {
  reset_gx_state();
  auto& before = g_gxState.ptTexMtxs[0];
  before.m2[0] = 90.0f;
  before.m2[1] = 91.0f;
  before.m2[2] = 92.0f;
  before.m2[3] = 93.0f;

  // A game can emit this XF register load directly even though Aurora's
  // high-level GXLoadTexMtxImm wrapper restricts post-transform matrices to
  // GX_MTX3x4.  Eight words cover rows 0 and 1 only.
  const auto bytes = xf_cmd(0x500, {
  ...
  EXPECT_FLOAT_EQ(decoded.m0[1], 2.0f);
  EXPECT_FLOAT_EQ(decoded.m0[2], 3.0f);
  EXPECT_FLOAT_EQ(decoded.m0[3], 4.0f);
  EXPECT_FLOAT_EQ(decoded.m1[0], 5.0f);
  EXPECT_FLOAT_EQ(decoded.m1[1], 6.0f);
  EXPECT_FLOAT_EQ(decoded.m1[2], 7.0f);
  EXPECT_FLOAT_EQ(decoded.m1[3], 8.0f);
  EXPECT_FLOAT_EQ(decoded.m2[0], 90.0f);
  EXPECT_FLOAT_EQ(decoded.m2[1], 91.0f);
  EXPECT_FLOAT_EQ(decoded.m2[2], 92.0f);
  EXPECT_FLOAT_EQ(decoded.m2[3], 93.0f);
}

TEST_F(GXFifoTest, PartialXfMatrixRowsPreserveUnwrittenValues) {
  for (const u32 address : {0u, 0x78u, 0x500u}) {
    reset_gx_state();
    auto& matrix = address == 0 ? g_gxState.pnMtx[0].pos :
        address == 0x78 ? g_gxState.texMtxs[0] : g_gxState.ptTexMtxs[0];
    matrix.m0 = {10.0f, 11.0f, 12.0f, 13.0f};
    matrix.m1 = {20.0f, 21.0f, 22.0f, 23.0f};
    matrix.m2 = {30.0f, 31.0f, 32.0f, 33.0f};
    g_gxState.stateDirty = false;
    decode_fifo(xf_cmd(address + 4, {0x3f800000u, 0x40000000u, 0x40400000u, 0x40800000u}));
    EXPECT_FLOAT_EQ(matrix.m0[0], 10.0f) << address;
    EXPECT_FLOAT_EQ(matrix.m0[3], 13.0f) << address;
    EXPECT_FLOAT_EQ(matrix.m1[0], 1.0f) << address;
    EXPECT_FLOAT_EQ(matrix.m1[3], 4.0f) << address;
    EXPECT_FLOAT_EQ(matrix.m2[0], 30.0f) << address;
    EXPECT_FLOAT_EQ(matrix.m2[3], 33.0f) << address;
    EXPECT_TRUE(g_gxState.stateDirty) << address;
  }
}

TEST_F(GXFifoTest, PartialXfPositionBankAcceptsTexture2x4Load) {
  reset_gx_state();
  g_gxState.pnMtx[0].pos.m2 = {10.0f, 11.0f, 12.0f, 13.0f};
  decode_fifo(xf_cmd(0, {0x3f800000u, 0, 0, 0, 0, 0x3f800000u, 0, 0}));
  EXPECT_FLOAT_EQ(g_gxState.pnMtx[0].pos.m0[0], 1.0f);
  EXPECT_FLOAT_EQ(g_gxState.pnMtx[0].pos.m1[1], 1.0f);
  EXPECT_FLOAT_EQ(g_gxState.pnMtx[0].pos.m2[0], 10.0f);
  EXPECT_FLOAT_EQ(g_gxState.pnMtx[0].pos.m2[3], 13.0f);
}

TEST_F(GXFifoTest, PartialXfMatrixLoadCrossesPositionTextureBoundary) {
  reset_gx_state();
  g_gxState.pnMtx[9].pos.m2[2] = 9.0f;
  g_gxState.texMtxs[0].m0[2] = 8.0f;
  decode_fifo(xf_cmd(0x77, {0x3f800000u, 0x40000000u, 0x40400000u}));
  EXPECT_FLOAT_EQ(g_gxState.pnMtx[9].pos.m2[2], 9.0f);
  EXPECT_FLOAT_EQ(g_gxState.pnMtx[9].pos.m2[3], 1.0f);
  EXPECT_FLOAT_EQ(g_gxState.texMtxs[0].m0[0], 2.0f);
  EXPECT_FLOAT_EQ(g_gxState.texMtxs[0].m0[1], 3.0f);
  EXPECT_FLOAT_EQ(g_gxState.texMtxs[0].m0[2], 8.0f);
}

TEST_F(GXFifoTest, PartialXfPostMatrixLoadCrossesSlotBoundary) {
  reset_gx_state();
  decode_fifo(xf_cmd(0x50b, {0x3f800000u, 0x40000000u}));
  EXPECT_FLOAT_EQ(g_gxState.ptTexMtxs[0].m2[3], 1.0f);
  EXPECT_FLOAT_EQ(g_gxState.ptTexMtxs[1].m0[0], 2.0f);
  EXPECT_FLOAT_EQ(g_gxState.ptTexMtxs[1].m0[1], 0.0f);
}
