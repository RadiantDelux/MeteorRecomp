...
size_t s_vertexBytesUsed = 0, s_indexBytesUsed = 0, s_storageBytesUsed = 0;
uint32_t s_batchSplits = 0;
bool s_trackStorageUploads = false;
std::vector<std::vector<uint8_t>> s_storageUploads;
} // namespace
...
bool split_recording_batch() noexcept {
  ++s_batchSplits;
  s_vertexBytesUsed = s_indexBytesUsed = s_storageBytesUsed = 0;
  s_lastGxDraw.reset();
  gfx::g_drawCallCount = 0;
...
void invalidate_dynamic_palette_source(const void*) noexcept {}
} // namespace aurora::gx
...
// --- Buffer push stubs ---
namespace aurora::gfx {
size_t vertex_bytes_used() noexcept { return s_vertexBytesUsed; }
size_t index_bytes_used() noexcept { return s_indexBytesUsed; }
size_t storage_bytes_used() noexcept { return s_storageBytesUsed; }
size_t storage_allocation_size(size_t length) noexcept { return (length + 255u) & ~size_t(255u); }
Range push_verts(const uint8_t* data, size_t length) {
  CHECK(s_vertexBytesUsed + length <= vertex_capacity(), "test vertex staging overflow");
  s_vertexBytesUsed += length;
  s_lastPushedVertices.assign(data, data + length);
  return {};
}
Range push_indices(const uint8_t* data, size_t length) {
  CHECK(s_indexBytesUsed + length <= index_capacity(), "test index staging overflow");
  s_indexBytesUsed += length;
...
Range push_storage(const uint8_t* data, size_t length) {
  CHECK(s_storageBytesUsed + storage_allocation_size(length) <= storage_capacity(), "test storage staging overflow");
  s_storageBytesUsed += storage_allocation_size(length);
  if (!s_trackStorageUploads) return {};
  s_storageUploads.emplace_back(data, data + length);
  return {static_cast<uint32_t>((s_storageUploads.size() - 1) * 256),
...
void set_staging_usage(size_t vertex, size_t index, size_t storage) noexcept {
  s_vertexBytesUsed = vertex;
  s_indexBytesUsed = index;
  s_storageBytesUsed = storage;
}
uint32_t batch_splits() noexcept { return s_batchSplits; }
void reset_vertex_push_record() noexcept {
  s_vertexBytesUsed = s_indexBytesUsed = s_storageBytesUsed = 0;
  s_batchSplits = 0;
  gx::fifo::set_max_draws_per_batch(0);
  g_drawCallCount = 0;
...
void use_storage_upload_tracking(bool enabled) noexcept {
  s_trackStorageUploads = enabled;
  s_storageUploads.clear();
}
const std::vector<std::vector<uint8_t>>& storage_uploads() noexcept {
  return s_storageUploads;
}
} // namespace aurora::gfx::testing
...
uint64_t calc_texture_size(wgpu::TextureFormat format, uint32_t width, uint32_t height, uint32_t mips) noexcept {
  const uint64_t bytesPerPixel = format == wgpu::TextureFormat::RG8Unorm || format == wgpu::TextureFormat::R16Sint
                                     ? 2u
                                     : format == wgpu::TextureFormat::R8Unorm ? 1u : 4u;
  uint64_t total = 0;
  for (uint32_t mip = 0; mip < std::max(mips, 1u); ++mip) {
    total += static_cast<uint64_t>(std::max(width >> mip, 1u)) * std::max(height >> mip, 1u) * bytesPerPixel;
  }
  return total;
}
