...
#if defined(_WIN32)
thread_local uintptr_t g_lastGuestFaultHostPc = 0;
...
LONG CALLBACK FlatGuestVectoredHandler(EXCEPTION_POINTERS* info) {
    ...
    g_lastGuestFaultHostPc = reinterpret_cast<uintptr_t>(record->ExceptionAddress);
...
void ReportFatalGuestFault(const char* category, uint32_t guestAddress, bool isWrite,
    ...
    std::cerr << "  active func:   0x" << std::hex << std::uppercase
              ...
              << std::endl;
#if defined(_WIN32)
    if (g_lastGuestFaultHostPc != 0) {
        const uintptr_t moduleBase =
            reinterpret_cast<uintptr_t>(GetModuleHandleW(nullptr));
        std::cerr << "  host pc:       0x" << std::hex << std::uppercase
                  << g_lastGuestFaultHostPc;
        if (moduleBase != 0 && g_lastGuestFaultHostPc >= moduleBase) {
            std::cerr << " (image+0x" << (g_lastGuestFaultHostPc - moduleBase) << ')';
        }
...
void ReportUnmappedCommit(uint32_t guestAddress, uint64_t blockBase, bool isWrite,
    ...
    if (g_lastGuestFaultHostPc != 0) {
        ...
        std::cerr << " hostPc=0x" << std::hex << std::uppercase << g_lastGuestFaultHostPc;
        if (moduleBase != 0 && g_lastGuestFaultHostPc >= moduleBase) {
            std::cerr << " image+0x" << (g_lastGuestFaultHostPc - moduleBase);
        }
    }
