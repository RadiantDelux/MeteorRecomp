...
std::array<uint32_t, 4> g_connectCallbacks{};
std::array<bool, 4> g_connectionKnown{};
std::array<bool, 4> g_connected{};

void InvokeConnectCallback(uint32_t chan, bool connected) {
    if (chan >= g_connectCallbacks.size()) return;
    const uint32_t callback = g_connectCallbacks[chan];
    if (callback == 0 || !CanDispatchGuestTarget(callback)) return;
    auto& cpu = GetPersistentCpuContext();
    cpu.gpr[3] = chan;
    cpu.gpr[4] = connected ? 0u : static_cast<uint32_t>(-1);
    InvokeIndirectCpu(callback, &cpu);
}

void UpdateConnectionState(uint32_t chan, bool connected) {
    if (chan >= g_connectionKnown.size()) return;
    if (g_connectionKnown[chan] && g_connected[chan] == connected) return;
    g_connectionKnown[chan] = true;
    g_connected[chan] = connected;
    InvokeConnectCallback(chan, connected);
}
...
int32_t WriteStatus(uint32_t chan, uint32_t addr, const WiiRemoteInput::KpadSample* sample) {
    ...
    // KPAD's DPD position is normalized around screen centre. Preserve the
    // remaining derived pointer fields as neutral for now; UI hit-testing uses
    // pos + dpd_valid_fg, while motion history can be layered on independently.
    WriteZeroFloats(addr + kPos, (kAccVertical + 8 - kPos) / 4);
    if (sample->pointerValid) {
        Memory::WriteFloat32(addr + kPos, sample->pointer[0]);
        Memory::WriteFloat32(addr + kPos + 4, sample->pointer[1]);
        Memory::Write8(addr + kDpdValidFg, 1);
    } else {
        Memory::Write8(addr + kDpdValidFg, 0);
    }
...
extern "C" int32_t KPAD__Read_HLE(uint32_t chan, uint32_t statusPtr, uint32_t count)
{
    ...
    UpdateConnectionState(chan, have);
    try {
...
extern "C" uint32_t KPADSetConnectCallback_HLE(uint32_t chan, uint32_t callback)
{
    if (chan >= g_connectCallbacks.size()) return 0;
    const uint32_t previous = g_connectCallbacks[chan];
    g_connectCallbacks[chan] = callback;

    // KPAD's connect callback is what title code uses to transition its own
    // controller state machine.  Our virtual remote already exists before the
    // title registers the callback, so publish the current state immediately;
    // later disconnect/reconnect transitions are delivered by KPADRead.
    WiiRemoteInput::Poll();
    const bool connected = WiiRemoteInput::IsRemoteChannel(chan);
    g_connectionKnown[chan] = true;
    g_connected[chan] = connected;
    if (callback != 0) InvokeConnectCallback(chan, connected);
    return previous;
}
