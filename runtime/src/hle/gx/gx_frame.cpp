...
void GX_HLE_BeginDisplayListRecordingForFifo(uint32_t listAddr, uint32_t sizeBytes,
    ...
    BeginDisplayListRecordingForFifo(listAddr, sizeBytes, fifoObjAddr);
}

void GX_HLE_EndDisplayListRecordingToFifo(uint32_t fifoObjAddr) {
    EndDisplayListRecordingToFifo(fifoObjAddr);
}

void BeginDisplayListRecording(uint32_t listAddr, uint32_t sizeBytes) {
    BeginDisplayListRecordingForFifo(listAddr, sizeBytes, kDlFifoAddr);
}
...
void EndDisplayListRecordingToFifo(uint32_t fifoObjAddr) {
    ...
    if (s_endLogCount < 16u) {
        RT_LOGF(RT_TAG_GX, "DL recording publish list=0x%08x count=0x%x write=0x%08x fifo=0x%08x\n",
                g_dlRecordState.base, g_dlRecordState.count, g_dlRecordState.writePtr,
                fifoObjAddr);
        ++s_endLogCount;
    }
    g_dlRecordState.active = false;
    try {
        // Revolution SDK GXFifoObj layout: readPtr +0x14, writePtr +0x18,
        // rwDistance/count +0x1C, wrap flag +0x20.  Some title-linked SDKs
        // keep this object at a different address from the generic HLE copy.
        Memory::Write32(fifoObjAddr + 0x18u, g_dlRecordState.writePtr);
        Memory::Write32(fifoObjAddr + 0x1Cu, g_dlRecordState.count);
        Memory::Write8(fifoObjAddr + kDlWrapFlagOffset, g_dlRecordState.wrapped ? 1u : 0u);
    } catch (const Memory::AccessViolation&) {
    }
}
...
void WriteDisplayListData(uint32_t val, uint32_t sizeBytes) {
    ...
    try {
        ...
        if (nextPtr > end) {
            ...
            g_dlRecordState.wrapped = true;
            const uint32_t fifoObjAddr =
                g_dlRecordState.fifoObjAddr != 0 ? g_dlRecordState.fifoObjAddr : kDlFifoAddr;
            Memory::Write8(fifoObjAddr + kDlWrapFlagOffset, 1);
