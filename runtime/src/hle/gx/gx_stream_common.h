...
// Repacks a strided guest array into a tightly packed host copy. Besides making
// strides >255 representable by Aurora, this can compact sparse/padded retail
// vertex records without changing their indices: element N in the packed copy
// is still the bytes read from guest base + N * stride.
...
inline bool ApplyIndexedXfArrayForPacket(uint8_t cmd, uint32_t value) {
    ...
    // Diagnostic only: the binding is already validated and the packet will
    // be forwarded unchanged. Label this observation separately from a raw
    // FIFO load because a display-list prebind may precede actual execution.
    GxMatrixProvenance::ObserveWords(value & 0x0fffu, hostPtr + index * arr.stride,
                                   bytes / 4u, "indexed-bind", arr.base + index * arr.stride);
#endif
    return true;
}
