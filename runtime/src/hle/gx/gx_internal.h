...
// A raw BP display-copy trigger is decoded while the FIFO parser still owns
// the bytes for that command.  Defer sealing/presentation until the caller has
// consumed the complete packet so VI/Aurora cannot re-enter GX on a half-decoded
// command stream.
...
struct HleGxState {
    ...
    // BP 0x4B stores the 32-byte-addressed EFB-copy destination.  Keep it in
    // the same parser state as the rest of the raw FIFO so a later BP 0x52
    // trigger sees exactly the destination programmed earlier in FIFO order.
    uint32_t bpCopyDestPhysical = 0;
    bool bpCopyDestValid = false;
    // Physical CP breakpoint state. GXSetBreakPt programs low16/high14 at
    // 0x3C/0x3E, enables the breakpoint, then re-enables CP reading.
    uint16_t cpControl = 0;
    uint16_t cpBreakpointLow = 0;
    uint16_t cpBreakpointHigh = 0;
    uint32_t cpBreakpointPhysical = 0;
    bool cpBreakpointAddressValid = false;
    bool cpBreakpointHit = false;
    bool cpBreakpointInterruptDispatched = false;
    // CP FIFO latches programmed by GXSetGPFifo. The host decoder consumes
    // gather-pipe commands directly rather than advancing a physical ring, so
    // these software-visible register pairs remain the authoritative values
    // returned to SDK save/query paths such as __GXSaveFifo.
    uint16_t cpRwDistanceLow = 0;
    uint16_t cpRwDistanceHigh = 0;
    uint16_t cpReadPointerLow = 0;
    uint16_t cpReadPointerHigh = 0;

    // Pixel Engine draw-sync state. BP 0x48 (PETOKENINT) latches the token and
    // raises the TOKEN condition; BP 0x47 (PETOKEN) only updates the visible
    // token. The native FIFO decoder can consume several PETOKENINT commands
    // before the translated CPU reaches an interrupt-safe boundary, so preserve
    // those interrupt events in FIFO order instead of collapsing them into the
    // final visible token. PE CSR +0x0A owns interrupt enables and W1C status,
    // while +0x0E remains the latest current token read by GXReadDrawSync.
    // CPU-visible PE poke/configuration registers at 0xCC001000..0xCC001008.
    // These are read-modify-written by the SDK independently of the FIFO BP
    // shadow, so keep their hardware latches explicitly instead of returning a
    // fabricated constant on reads.
    uint16_t peZConfig = 0;
    uint16_t peAlphaConfig = 0;
    uint16_t peDstAlpha = 0;
    uint16_t peAlphaMode = 0;
    uint16_t peAlphaRead = 0;
    uint16_t peControl = 0;
    uint16_t peTokenCurrent = 0;
    std::deque<uint16_t> peTokenInterruptQueue{};
    bool peTokenInterruptDispatched = false;
    bool peFinishInterruptPending = false;
    bool peFinishInterruptDispatched = false;
...
void BeginDisplayListRecordingForFifo(uint32_t listAddr, uint32_t sizeBytes,
...
void EndDisplayListRecordingToFifo(uint32_t fifoObjAddr);
