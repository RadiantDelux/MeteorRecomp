...
GXTexFmt CanonicalizeEfbCopyFormat(uint32_t value) {
    // GX's copy-control BP register carries only the four-bit EFB encoder mode.
    // Modes 7-C are channel-copy formats even though 8-A collide numerically
    // with the palette texture enums. Preserve full GX_CTF_* values in the HLE
    // shadow state so GXGetTexBufferSize agrees with Aurora's copy/readback path.
    switch (value) {
    case 0x7: return GX_CTF_A8;
    case 0x8: return GX_CTF_R8;
    case 0x9: return GX_CTF_G8;
    case 0xA: return GX_CTF_B8;
    case 0xB: return GX_CTF_RG8;
    case 0xC: return GX_CTF_GB8;
    default: return static_cast<GXTexFmt>(value);
    }
}
...
extern "C" void GX__SetTexCopyDst_8016f4dc(uint32_t w, uint32_t h, uint32_t f, uint32_t m) {
    const GXTexFmt copyFormat = CanonicalizeEfbCopyFormat(f);
    GXSetTexCopyDst((u16)w, (u16)h, copyFormat, (GXBool)m);
    g_texCopyState.dstWidth=(u16)w; g_texCopyState.dstHeight=(u16)h;
    g_texCopyState.dstFormat=static_cast<uint32_t>(copyFormat); g_texCopyState.dstMipmap=m;
}
