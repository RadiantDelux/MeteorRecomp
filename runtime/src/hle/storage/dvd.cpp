...
static bool g_dvdDiscIndexInitialized = false;
...
// This guest region is reserved at startup for the published FST (MEM2 on Wii,
// top-of-MEM1 apploader reservation on GameCube).
...
static void InvokeDvdCallback(uint32_t callbackPtr, int32_t result, uint32_t fileInfoPtr) {
    ...
    if (!CanDispatchGuestTarget(callbackPtr)) {
        return;
    }
...
struct GameCubeSystemExtent {
    uint64_t startBytes = 0;
    uint64_t endBytes = 0;
    fs::path hostPath;
};

static std::vector<GameCubeSystemExtent> g_gameCubeSystemExtents;
static std::once_flag g_gameCubeSystemExtentsOnce;

static void EnsureGameCubeSystemExtents() {
    std::call_once(g_gameCubeSystemExtentsOnce, []() {
        const fs::path sysRoot = GetDvdRoot() / "sys";

        auto addExtent = [&](uint64_t startBytes, const char* fileName) {
            const fs::path path = sysRoot / fileName;
            std::error_code ec;
            const uint64_t size = fs::file_size(path, ec);
            if (ec || size == 0) {
                return;
            }
            g_gameCubeSystemExtents.push_back({startBytes, startBytes + size, path});
        };

        // These offsets are fixed by the GameCube disc format.
        addExtent(0x0000ull, "boot.bin");
        addExtent(0x0440ull, "bi2.bin");
        addExtent(0x2440ull, "apploader.img");

        // main.dol and fst.bin are title-specific. boot.bin publishes their
        // physical disc byte offsets at 0x420 and 0x424 respectively.
        const fs::path bootPath = sysRoot / "boot.bin";
        std::vector<uint8_t> bootBytes;
        DvdReadContract::HostReadFailure failure;
        if (DvdReadContract::ReadExact(bootPath, 0, 0x430u, bootBytes, failure)) {
            const uint32_t dolOffset = BigEndian::Read32(bootBytes.data() + 0x420u);
            const uint32_t fstOffset = BigEndian::Read32(bootBytes.data() + 0x424u);
            if (dolOffset != 0) {
                addExtent(dolOffset, "main.dol");
            }
            if (fstOffset != 0) {
                addExtent(fstOffset, "fst.bin");
            }
        }

        std::sort(g_gameCubeSystemExtents.begin(), g_gameCubeSystemExtents.end(),
                  [](const GameCubeSystemExtent& a, const GameCubeSystemExtent& b) {
                      return a.startBytes < b.startBytes;
                  });
    });
}

// Extracted GameCube DATA directories keep boot/apploader/DOL/FST payloads in
// sys/, outside the retail FST tree. Raw DI commands still address those bytes
// at their physical disc offsets. Resolve only requests fully contained in one
// real system payload; alignment gaps between payloads remain unmapped instead
// of being fabricated.
static bool TryReadGameCubeSystemAreaToGuest(uint32_t buffer,
                                             uint32_t length,
                                             uint64_t byteOffset,
                                             bool& handled) {
    handled = false;
    EnsureGameCubeSystemExtents();
    const uint64_t requestEnd = byteOffset + static_cast<uint64_t>(length);

    for (const GameCubeSystemExtent& extent : g_gameCubeSystemExtents) {
        if (byteOffset < extent.startBytes || requestEnd > extent.endBytes) {
            continue;
        }

        handled = true;
        const uint64_t fileOffset = byteOffset - extent.startBytes;
        std::vector<uint8_t> bytes;
        DvdReadContract::HostReadFailure failure;
        if (!DvdReadContract::ReadExact(extent.hostPath, fileOffset, length, bytes, failure)) {
            ReportDvdReadError(HostPathText(extent.hostPath),
        ...
        CopyToGuestAsDma(buffer, bytes.data(), bytes.size());
        return true;
    }
...
#if WIICOMPILED_GUEST_IS_GAMECUBE
        // GameCube FST file offsets are byte addresses on the disc. Wii FST
        // offsets are four-byte word addresses within the decrypted partition.
        const uint64_t startBytes64 = static_cast<uint64_t>(fileOffset);
#else
...
static const PublishedExtent* FindNextPublishedExtent(uint64_t offset) {
    const auto it = std::lower_bound(
        g_publishedExtents.begin(), g_publishedExtents.end(), offset,
        [](const PublishedExtent& extent, uint64_t value) { return extent.startBytes < value; });
    return it != g_publishedExtents.end() ? &(*it) : nullptr;
}

static const PublishedExtent* FindPreviousPublishedExtent(uint64_t offset) {
    if (g_publishedExtents.empty()) {
        return nullptr;
    }
...
static void BuildAndPublishRuntimeFst() {
    ...
    try {
#if WIICOMPILED_GUEST_IS_GAMECUBE
        image = DvdFstContract::BuildImage(registrations, true);
#else
...
#if WIICOMPILED_GUEST_IS_GAMECUBE
    // OSBootInfo::FSTMaxLength is the apploader-reserved capacity, not the
    // number of bytes occupied by the current FST image.  Keep that contract
    // when a GameCube overlay genuinely requires us to republish the tree.
    Memory::Write32(0x8000003Cu, g_dvdFstReservedSize);
#else
    ...
    RT_LOG(RT_TAG_DVD) << "published " << image.entries.size() << " FST entries ("
              ...
              << " inside the boot-time guest reservation" << std::dec << std::endl;
...
#if WIICOMPILED_GUEST_IS_GAMECUBE
// A retail GameCube boot already placed sys/fst.bin at OSBootInfo::FSTLocation
// before the DOL started.  The extracted host's `sys` directory is disc
// metadata, not a directory in that FST, so an unmodified disc must not grow or
// reorder the guest tree merely because the HLE needs a host-side read index.
// Build only the physical extents needed by raw DI reads and leave the guest FST
// byte-for-byte intact.  This also preserves the SDK entry numbering already in
// use by translated DVD filesystem code.
static void BuildGameCubeRetailExtentIndex() {
    g_publishedExtents.clear();
    g_publishedExtents.reserve(g_fstFiles.size());

    size_t mappedFiles = 0;
    for (const FstFileEntry& fstFile : g_fstFiles) {
        const auto mapped = g_pathToEntry.find(NormalizePath(fstFile.dvdPath));
        if (mapped == g_pathToEntry.end() || mapped->second < 0 ||
            mapped->second >= static_cast<int32_t>(g_fileEntries.size())) {
        ...
        DVDFileEntry& entry = g_fileEntries[mapped->second];
        entry.discOffsetWords = fstFile.start / 4u;

        // The physical disc extent is defined by the retail FST.  A host file
        // that is unexpectedly larger must not bleed into the next disc file;
        // a shorter/truncated host file will still fail when ReadExact reaches
        // the missing bytes.
        const uint64_t startBytes = fstFile.start;
        const uint64_t endBytes = startBytes + std::max<uint32_t>(fstFile.size, 1u);
        g_publishedExtents.push_back({startBytes, endBytes, mapped->second});
        ++mappedFiles;
    }
    ...
    RT_LOG(RT_TAG_DVD) << "using retail GameCube FST in place; indexed " << mappedFiles
              << "/" << g_fstFiles.size() << " physical file extent(s)" << std::endl;
}
#endif

void DVD_HLE_EnsureDiscIndex()
{
    if (g_dvdDiscIndexInitialized) {
        return;
    }
...
#if !WIICOMPILED_GUEST_IS_GAMECUBE
    // `sys` is an extracted-image convenience directory.  Wii's legacy HLE
    // index exposes it for its synthetic FST, but on GameCube boot.bin/bi2.bin,
    // apploader, DOL and fst.bin live outside the retail filesystem tree.
...
#if WIICOMPILED_GUEST_IS_GAMECUBE
    if (overlays.empty()) {
        BuildGameCubeRetailExtentIndex();
    } else {
    ...
    BuildAndPublishRuntimeFst();
#endif
    g_dvdDiscIndexInitialized = true;
    initializing = false;
}
...
bool DVD_HLE_ReadWordsToGuest(uint32_t buffer, uint32_t length, uint32_t wordOffset)
{
    ...
    if (!Memory::Contains(buffer, length)) {
        ReportDvdReadError("<invalid DVD destination>", static_cast<int64_t>(wordOffset) * 4,
                           length, "DVD read destination is outside guest memory");
        return false;
    }

    const uint64_t byteOffset64 = static_cast<uint64_t>(wordOffset) * 4ull;
    if (byteOffset64 > 0xFFFFFFFFull) {
        ReportDvdReadError("<unmapped DVD offset>", static_cast<int64_t>(byteOffset64), length,
                           "DVD word offset exceeds the extracted partition address space");
        return false;
    }

#if WIICOMPILED_GUEST_IS_GAMECUBE
    bool handledSystemArea = false;
    const bool systemReadSucceeded =
        TryReadGameCubeSystemAreaToGuest(buffer, length, byteOffset64, handledSystemArea);
    if (handledSystemArea) {
        return systemReadSucceeded;
    }
#endif

    DVD_HLE_EnsureDiscIndex();

    // DI reads address the decrypted partition, not a single FST file. SDK
    // callers commonly round the final transfer up to an alignment boundary,
    // so a request may legitimately cross directly from one file extent into
    // the next. Extracted directories do not preserve the physical padding
    // bytes between FST files. GameCube FST files are four-byte aligned, so
    // reconstruct only that narrow 0..3-byte alignment padding as zeroes; a
    // larger hole remains an unmapped-disc error rather than being fabricated.
    std::vector<uint8_t> staged;
    staged.reserve(length);
    uint64_t currentOffset = byteOffset64;
    uint32_t remaining = length;
    while (remaining != 0) {
        const PublishedExtent* extent = FindPublishedExtentForByteOffset(currentOffset);
        if (!extent) {
            // Extracted files omit disc alignment padding. Reconstruct only the
            // tail to the platform's next boundary, bounded by the next file.
            const PublishedExtent* next = FindNextPublishedExtent(currentOffset);
            const PublishedExtent* previous = FindPreviousPublishedExtent(currentOffset);
            if (previous) {
                const uint32_t padLength = WiiCompiled::Disc::AlignmentPaddingLength(
                    currentOffset, previous->endBytes,
                    next ? next->startBytes : UINT64_MAX, remaining,
                    WIICOMPILED_GUEST_IS_GAMECUBE ? 32u : 4u);
                if (padLength != 0) {
                    staged.insert(staged.end(), padLength, 0u);
                    currentOffset += padLength;
                    remaining -= padLength;
                    continue;
                }
            }
            ReportDvdReadError("<unmapped DVD offset>", static_cast<int64_t>(currentOffset),
                               remaining,
                               "DVD read crosses an unmapped extracted-partition gap");
            return false;
        }
        if (extent->entryIndex < 0 ||
            extent->entryIndex >= static_cast<int32_t>(g_fileEntries.size())) {
            ReportDvdReadError("<unmapped DVD offset>", static_cast<int64_t>(currentOffset),
                               remaining,
                               "DVD read crosses an unmapped extracted-partition gap");
            return false;
        ...
        const DVDFileEntry& entry = g_fileEntries[extent->entryIndex];
        const uint64_t fileOffset64 = currentOffset - extent->startBytes;
        if (fileOffset64 >= entry.size) {
            ReportDvdReadError(HostPathText(entry.hostPath),
                               static_cast<int64_t>(fileOffset64), remaining,
                               "DVD read resolved outside the mapped file payload");
            return false;
        }

        const uint64_t available64 = std::min<uint64_t>(
            extent->endBytes - currentOffset,
            static_cast<uint64_t>(entry.size) - fileOffset64);
        const uint32_t chunkLength = static_cast<uint32_t>(
            std::min<uint64_t>(available64, remaining));
        if (chunkLength == 0) {
            ReportDvdReadError(HostPathText(entry.hostPath),
                               static_cast<int64_t>(fileOffset64), remaining,
                               "DVD read resolved to an empty mapped extent");
            return false;
        }

        std::vector<uint8_t> chunk;
        DvdReadContract::HostReadFailure failure;
        if (!DvdReadContract::ReadExact(entry.hostPath,
                                        static_cast<uint32_t>(fileOffset64),
                                        chunkLength,
                                        chunk,
                                        failure)) {
            ReportDvdReadError(HostPathText(entry.hostPath),
                               static_cast<int64_t>(fileOffset64), chunkLength,
                               DvdReadContract::Describe(failure));
            return false;
        }

        staged.insert(staged.end(), chunk.begin(), chunk.end());
        currentOffset += chunkLength;
        remaining -= chunkLength;
    }

    CopyToGuestAsDma(buffer, staged.data(), staged.size());
    return true;
}
...
extern "C" void DVDInit_8015EA1C()
{
    ...
    // 4. Build the title-neutral extracted-disc index and publish its FST.
    const auto& overlays = RuntimeRiivolution::Overlays();
