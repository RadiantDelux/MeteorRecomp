...
static std::atomic<uint64_t> g_nextNandCallbackId{1};
static thread_local uint64_t g_currentNandCallbackId = 0;

// Callback lifetime tracing is intentionally opt-in.  This path runs for every
// asynchronous IOS completion (including the Bluetooth input stream); building
// an iostream record for each completion can become a material part of the
// frame budget.  Keep the cached flag immutable after startup so the hot path
// only performs one predictable branch when diagnostics are disabled.
static bool MeteorRuntimeTraceEnabled() noexcept {
    static const bool enabled = std::getenv("METEOR_TRACE_RUNTIME") != nullptr;
    return enabled;
}

static bool ShouldTraceMeteorBtCallback(uint32_t callbackPtr, uint32_t commandBlock) noexcept {
    return callbackPtr == 0x8026D0C8u && commandBlock >= 0x97FC1E00u && commandBlock < 0x97FC2400u;
}

static void DispatchNandCallback(CpuContext* cpu, uint64_t id, uint32_t callbackPtr,
                                 int32_t result, uint32_t commandBlock) {
    if (callbackPtr == 0) {
        return;
    }
    ...
    const bool traceLifetime = MeteorRuntimeTraceEnabled() &&
                               ShouldTraceMeteorBtCallback(callbackPtr, commandBlock);
    if (traceLifetime) {
        RT_LOG("ios-callback-lifetime") << "dispatch-start id=" << std::dec << id
                                        << " callback=0x" << std::hex << callbackPtr
                                        << " arg=0x" << commandBlock
                                        << " result=0x" << static_cast<uint32_t>(result)
                                        << std::dec << std::endl;
    }

    const uint64_t previousCallbackId = g_currentNandCallbackId;
    g_currentNandCallbackId = id;
    ...
    g_currentNandCallbackId = previousCallbackId;
    if (traceLifetime) {
        RT_LOG("ios-callback-lifetime") << "dispatch-end id=" << std::dec << id
                                        << " callback=0x" << std::hex << callbackPtr
                                        << " arg=0x" << commandBlock
                                        << std::dec << std::endl;
    }
...
static void InvokeNandCallback(uint32_t callbackPtr, int32_t result, uint32_t commandBlock) {
    ...
    const uint64_t id = g_nextNandCallbackId.fetch_add(1u, std::memory_order_relaxed);
    std::lock_guard<std::mutex> lock(g_pendingNandCallbackMutex);
    g_pendingNandCallbacks.push_back({id, callbackPtr, result, commandBlock});
    if (MeteorRuntimeTraceEnabled() && id <= 256u) {
        RT_LOG("ios-callback-all") << "enqueue id=" << std::dec << id
                                   << " callback=0x" << std::hex << callbackPtr
                                   << " arg=0x" << commandBlock
                                   << " result=0x" << static_cast<uint32_t>(result)
                                   << std::dec << std::endl;
    }
    if (MeteorRuntimeTraceEnabled() && ShouldTraceMeteorBtCallback(callbackPtr, commandBlock)) {
        RT_LOG("ios-callback-lifetime") << "enqueue id=" << std::dec << id
                                        << " callback=0x" << std::hex << callbackPtr
                                        << " arg=0x" << commandBlock
                                        << " result=0x" << static_cast<uint32_t>(result)
                                        << std::dec << std::endl;
...
bool NandIosCallbackDispatchActive() noexcept {
    return g_nandCallbackDrainDepth != 0;
}

uint64_t NandIosCurrentCallbackDispatchId() noexcept {
    return g_currentNandCallbackId;
}
...
bool NandProcessPendingCallbacks(CpuContext* cpu, int maxToProcess) {
    ...
    while (processed < maxCallbacksThisPump) {
        ...
        if (MeteorRuntimeTraceEnabled() && cb.id <= 256u) {
            RT_LOG("ios-callback-all") << "dispatch-start id=" << std::dec << cb.id
                                       << " callback=0x" << std::hex << cb.callbackPtr
                                       << " arg=0x" << cb.commandBlock
                                       << " result=0x" << static_cast<uint32_t>(cb.result)
                                       << std::dec << std::endl;
        }
        DispatchNandCallback(cpu, cb.id, cb.callbackPtr, cb.result, cb.commandBlock);
        if (MeteorRuntimeTraceEnabled() && cb.id <= 256u) {
            RT_LOG("ios-callback-all") << "dispatch-end id=" << std::dec << cb.id
                                       << " callback=0x" << std::hex << cb.callbackPtr
                                       << " arg=0x" << cb.commandBlock
                                       << std::dec << std::endl;
        }
