...
static constexpr int32_t STM_IMMEDIATE_DEV_FD = 5;
static constexpr int32_t STM_EVENTHOOK_DEV_FD = 6;
static constexpr int32_t DI_DEV_FD = 7;
static constexpr uint32_t DI_IOCTL_READ_DISK_ID = 0x12;
static constexpr uint32_t DI_IOCTL_READ = 0x71;
static constexpr uint32_t DI_IOCTL_CLEAR_COVER_INTERRUPT = 0x86;
static constexpr uint32_t DI_DISC_ID_ADDRESS = 0x80000000u;
static constexpr uint32_t DI_DISC_ID_SIZE = 0x20u;
static constexpr uint32_t ES_IOCTL_GETDEVICEID = 0x07;
static constexpr uint32_t ES_IOCTL_GETDATADIR = 0x1D;
...
static void EnsureCurrentTitleDataDirectory() {
    char titleId[9]{};
    char gameId[9]{};
...
extern "C" int32_t NAND_IOS_Open_HLE(uint32_t pathPtr, uint32_t mode) {
    ...
    if (std::strncmp(path, "/dev/", 5) == 0) {
        if (std::strcmp(path, "/dev/fs") == 0) {
            EnsureCurrentTitleDataDirectory();
            return ISFS_DEV_FD;
        }
        ...
        if (std::strcmp(path, "/dev/stm/immediate") == 0) {
            return STM_IMMEDIATE_DEV_FD;
        }
        if (std::strcmp(path, "/dev/stm/eventhook") == 0) {
            return STM_EVENTHOOK_DEV_FD;
        }
...
extern "C" int32_t NAND_IOS_Close_HLE(uint32_t fd) {
    ...
    if (fd == STM_IMMEDIATE_DEV_FD || fd == STM_EVENTHOOK_DEV_FD) {
        return ISFS_OK;
    }
    if (fd == DI_DEV_FD) {
        return ISFS_OK;
    ...
    uint32_t outBufPtr, uint32_t outLen)
{
    ...
    if (fd == STM_IMMEDIATE_DEV_FD || fd == STM_EVENTHOOK_DEV_FD) {
        // STM reset/power/eventhook traffic is host policy in WiiCompiled. The
        // guest only needs IOS submission to succeed; no real host shutdown or
        // reset is triggered by these device ioctls during normal bring-up.
        RT_LOG(RT_TAG_OS) << "STM ioctl fd=" << fd << " cmd=0x" << std::hex << cmd
                          << std::dec << " acknowledged by host HLE" << std::endl;
        return ISFS_OK;
    }
    if (fd == DI_DEV_FD) {
        if (cmd == DI_IOCTL_READ_DISK_ID) {
            if (!outBufPtr || outLen < DI_DISC_ID_SIZE ||
                !Memory::Contains(DI_DISC_ID_ADDRESS, DI_DISC_ID_SIZE) ||
                !Memory::Contains(outBufPtr, DI_DISC_ID_SIZE)) {
                return DvdReadContract::kInterruptDriveError;
            }
            for (uint32_t i = 0; i < DI_DISC_ID_SIZE; ++i) {
                Memory::Write8(outBufPtr + i, Memory::Read8(DI_DISC_ID_ADDRESS + i));
            }
            return DvdReadContract::kInterruptTransferComplete;
        }
        if (cmd == DI_IOCTL_READ) {
            if (!inBufPtr || inLen < 12 || !Memory::Contains(inBufPtr, 12)) {
                return DvdReadContract::kInterruptDriveError;
            }
            const uint32_t requestedLength = Memory::Read32(inBufPtr + 4);
            const uint32_t wordOffset = Memory::Read32(inBufPtr + 8);
            if (requestedLength != outLen || !outBufPtr ||
                !DVD_HLE_ReadWordsToGuest(outBufPtr, requestedLength, wordOffset)) {
                return DvdReadContract::kInterruptDriveError;
            }
            return DvdReadContract::kInterruptTransferComplete;
        }
        if (cmd == DI_IOCTL_CLEAR_COVER_INTERRUPT) {
            // RVL DI command 0x86 = DVDLowClearCoverInterrupt. There is no
            // guest-visible payload; IOS acknowledges successful completion.
            // WiiCompiled does not expose a physical DI cover interrupt latch,
            // so completing the request is the correct host-side equivalent.
            return DvdReadContract::kInterruptTransferComplete;
        }
        LogNandWarning("IOS_Ioctl", "/dev/di unsupported cmd=0x%X", cmd);
        return DvdReadContract::kInterruptDriveError;
    }
...
int32_t ISFS_OpenLib_Initialize(CpuContext* ctx) {
    ...
    // A title launch has a data directory before the RVL NAND library enumerates it.
    EnsureCurrentTitleDataDirectory();
    ...
    uint32_t vectorPtr)
{
    ...
    if (fd == ES_DEV_FD) {
        ...
        switch (cmd) {
            ...
            case ES_IOCTL_GETDATADIR: {
                if (numIn != 1 || numOut != 1) {
                    return ISFS_EINVAL;
                }
                const IosVector in = ReadIosVector(vectorPtr, 0);
                const IosVector out = ReadIosVector(vectorPtr, 1);
                if (in.size < 8 || in.address == 0 || !Memory::Contains(in.address, 8)) {
                    return ISFS_EINVAL;
                }

                const uint32_t titleHi = Memory::Read32(in.address);
                const uint32_t titleLo = Memory::Read32(in.address + 4u);
                char path[64]{};
                const int length = std::snprintf(
                    path, sizeof(path), "/title/%08x/%08x/data", titleHi, titleLo);
                if (length < 0 || static_cast<size_t>(length) + 1u > sizeof(path) ||
                    !WriteGuestBytes(out.address, out.size,
