...
constexpr uint32_t kAxSamplesPerFrame = 160u; // AX GameCube: five milliseconds.
#else
constexpr uint32_t kAxSamplesPerFrame = 96u;
#endif
constexpr uint32_t kAxDspTaskAddr = 0x802F81A0u;
extern uint32_t g_dspInitializedAddr;
extern uint32_t g_dspAssertPendingAddr;
extern uint32_t g_dspAssertTaskAddr;
extern uint32_t g_dspReservedStateAddr;
extern uint32_t g_dspCurrentTaskAddr;
extern uint32_t g_dspFirstTaskAddr;
extern uint32_t g_dspRunningTaskAddr;
...
inline bool UCodeUsesGameCubeAxCommands(uint32_t crc) {
    // Dolphin's AXUCode family.  0x4e8a8b21 is deliberately excluded here:
    // that older variant removes the LPF words from its PB layout, while the
    // GameCube PB bridge below currently models the LPF-bearing layout.  The
    // remaining retail variants share that PB shape and command decoder.
    return crc == 0x3ad3b7acu ||
           crc == 0x3daf59b9u ||
           crc == 0x07f88145u ||
           crc == 0xe2136399u;
}
...
class Accelerator {
public:
    void Setup(AXPBWii* pb, uint16_t* loopCounter = nullptr) {
        ...
        if (m_current == m_end + step - 1) {
            ...
            if (m_pb->audio_addr.looping) {
                ...
                if (m_pb->is_stream != 1) {
                    ...
                    if (m_loopCounter) ++*m_loopCounter;
                    SetYn2(m_yn2);
                }
            } else {
                m_pb->running = 0;
...
private:
    ...
    uint16_t* m_loopCounter = nullptr;
    uint32_t m_start = 0;
    uint32_t m_end = 0;
