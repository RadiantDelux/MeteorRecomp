...
public:
    ...
    void PrepareHostMixState() {
        // Some titles keep DSPInit translated and only HLE the physical first-task
        // boot boundary. In that configuration the guest already owns all DSP
        // scheduler globals and mail sequencing, but the native mixer still needs
        // stable MEM1/MEM2 aliases and the DSP coefficient ROM. Do not call
        // Initialize() here: it resets guest task globals and pushes a synthetic
        // INIT mail that retail code has already accounted for.
        JoinMix();
        RefreshMixMemoryMap();
    ...
    void NotifyTaskCancelled(uint32_t taskPtr) {
        // The real DSP keeps running independently after DSPCancelTask and its
        // next cooperative YIELD lets the SDK ISR turn an asserted task into
        // DONE.  This HLE executes a submitted command list synchronously (or
        // on the mix worker) and therefore has no DSP thread left to produce a
        // later YIELD once the CPU reaches DSPCancelTask.  Join any outstanding
        // mix work, then publish that hardware scheduling boundary through the
        // normal mailbox/IRQ path.  Never manufacture DONE here: the translated
        // SDK ISR owns the flags&2 YIELD->DONE promotion and task callbacks.
        JoinMix();

        // Cancelling a queued/non-current task only sets its flag in the retail
        // SDK.  Only the currently executing DSP task can reach the cooperative
        // YIELD boundary that the HLE must stand in for here.
        uint32_t currentTask = 0;
        if (!Memory::TryRead32(g_dspCurrentTaskAddr, currentTask) || currentTask != taskPtr) {
            return;
        }

        std::lock_guard<std::mutex> lock(m_mutex);
        g_axTaskPtr = taskPtr;
        if (std::find(m_cpuMails.begin(), m_cpuMails.end(), kDspYield) == m_cpuMails.end()) {
            PushMail(kDspYield);
        }
    }
...
private:
    ...
    void PushMail(uint32_t mail) {
        ...
        // Every DSP->CPU mail emitted by this task protocol (INIT/RESUME/YIELD/
        // DONE/SYNC) requests the DSP interrupt independently of mailbox-full.
        // The raw GameCube device bridge only sets hardware state here; guest
        // interrupt dispatch happens later at a normal runtime safe boundary.
    ...
    void HandleMail(uint32_t mail) {
        ...
        case MailState::WaitingForCmdListAddress:
            ...
            DispatchCommandList();
            m_cmdListSize = 0;
            // The raw GameCube path lets the translated DSP ISR consume YIELD
            // and invoke the task callback.  Keep the historical direct callback
            // only on targets that do not use the GameCube hardware IRQ bridge.
            PushMail(kDspYield);
    ...
    void HandleTaskMail(uint32_t mail) {
        ...
        case kMailReset:
#if !WIICOMPILED_GUEST_IS_GAMECUBE
            // Historical Wii path: older host scheduling used RESET as the
            // direct trigger for a synthetic completion mail.
            PushMail(kDspDone);
#endif
            // On GameCube the translated DSP ISR already owns completion.  Its
            // promoted YIELD->DONE path sends RESET after removing the task;
            // answering that RESET with another DONE would duplicate completion.
            m_mailState = MailState::WaitingForCmdListSize;
            break;
        default:
            m_mailState = MailState::WaitingForCmdListSize;
...
#if WIICOMPILED_GUEST_IS_GAMECUBE
        if (UCodeUsesGameCubeAxCommands(m_ucodeCrc)) {
            // Complete GC PB/aux writes before the translated SDK consumes YIELD.
            m_deferredAux = false;
            RefreshMixMemoryMap();
            LoadResamplingCoefficients();
            ExecuteGameCubeCommands();
            return;
        }
#endif
...
#if WIICOMPILED_GUEST_IS_GAMECUBE
#include "ax_gamecube_commands.inl"
#endif
    ...
    void ProcessVoice(AXPBWii& pb, const AXBuffers& buffers, uint32_t count, AXMixControl control,
                      const int16_t* coeffs, bool newFilter, uint16_t* loopCounter = nullptr) {
        if (pb.running != 1) {
            return;
        }
        Accelerator accel;
        accel.Setup(&pb, loopCounter);
    ...
    void OutputSamples(uint32_t lrAddr, uint32_t surroundAddr, uint16_t volume, bool uploadAuxC) {
        ...
        if (traceBlock) {
            RT_LOG(RT_TAG_AUDIO)
                ...
                << " pb=" << m_diagPbCount
                << " running=" << m_diagRunningPbCount
                << " peak=" << diagPeak
                << " lr=0x" << std::hex << lrAddr
                << " volume=0x" << volume
                << " ucode=0x" << m_ucodeCrc
                << std::dec << std::endl;
        }
    ...
    uint64_t m_diagOutputSerial = 0;
    uint32_t m_diagPbCount = 0;
    uint32_t m_diagRunningPbCount = 0;
...
void SetGuestStateLayout(uint32_t initializedFlag,
    ...
    // Configured once during title bring-up, before DSP task/mix worker activity.
    // Zero means "leave the legacy/default location in place", which lets
    // partial title layouts fail closed instead of writing address zero.
    if (initializedFlag != 0) g_dspInitializedAddr = initializedFlag;
    if (assertPending != 0) g_dspAssertPendingAddr = assertPending;
    if (assertTask != 0) g_dspAssertTaskAddr = assertTask;
    if (reservedState != 0) g_dspReservedStateAddr = reservedState;
    if (currentTask != 0) g_dspCurrentTaskAddr = currentTask;
    if (firstTask != 0) g_dspFirstTaskAddr = firstTask;
    if (runningTask != 0) g_dspRunningTaskAddr = runningTask;
}
...
uint32_t CheckInit() {
    return ReadGuestU32OrZero(g_dspInitializedAddr);
}
...
uint32_t AddTask(uint32_t taskPtr) {
    ...
    const uint32_t firstTask = ReadGuestU32OrZero(g_dspFirstTaskAddr);
    const bool bootsFirstTask = firstTask == 0;
    if (firstTask == 0) {
        LinkSingleDspTask(taskPtr);
    } else {
        const uint32_t oldCurrent = ReadGuestU32OrZero(g_dspCurrentTaskAddr);
        Memory::TryWrite32(oldCurrent + 0x38u, taskPtr);
        Memory::TryWrite32(taskPtr + 0x38u, 0);
        Memory::TryWrite32(taskPtr + 0x3cu, oldCurrent);
        Memory::TryWrite32(g_dspCurrentTaskAddr, taskPtr);
    }
    Memory::TryWrite32(taskPtr + 0x00u, 0);
    Memory::TryWrite32(taskPtr + 0x08u, 1);
    if (bootsFirstTask) {
#if WIICOMPILED_GUEST_IS_GAMECUBE
        // The translated SDK boot path normally consumes the DSP ROM handoff
        // mail (0x00543448) before the first task starts. AddTask is the HLE
        // replacement for that whole first-task boot, so retire only that boot
        // handoff here; otherwise the first later task IRQ would consume the
        // stale boot mail and spuriously reassert for the real task mail behind
        // it.
        Memory::CompleteGameCubeDspHleBootHandshake();
#endif
        // Retail DSPAddTask boots the first task.  Hardware then raises the
        // task's init callback once the ucode handshake completes; AX waits on
        // that callback immediately after DSPAddTask.  There is no independent
        // DSP host thread here that could raise the interrupt while guest code
        // busy-waits, so complete the already-emulated boot synchronously at
        // this HLE boundary.  This is the init callback (+0x28), not resume
        // (+0x2C): queuing resume here leaves AX's init flag permanently clear.
        g_axTaskPtr = taskPtr;
        InvokeAxTaskCallback(0x28u, false);
    }
    return taskPtr;
}
...
uint32_t AssertTask(uint32_t taskPtr) {
    if (taskPtr != 0) {
        g_axTaskPtr = taskPtr;
#if WIICOMPILED_GUEST_IS_GAMECUBE
        // GameCube Dolphin SDK DSPCancelTask is guest-visible as task->flags |=
        // 2.  Do not write the request/scheduler globals here: those belong to
        // DSPAssertTask (the priority/request switch operation at the following
        // SDK entry point in this linked build).
        uint32_t flags = 0;
        if (Memory::TryRead32(taskPtr + 0x08u, flags)) {
            Memory::TryWrite32(taskPtr + 0x08u, flags | 2u);
        }
        Instance().NotifyTaskCancelled(taskPtr);
#else
        Memory::TryWrite32(g_dspAssertPendingAddr, 1);
        Memory::TryWrite32(g_dspAssertTaskAddr, taskPtr);
        Memory::TryWrite32(g_dspRunningTaskAddr, taskPtr);
