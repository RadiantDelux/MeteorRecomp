...
std::atomic<bool> g_loggedPostOsInitLowMem{false};

void LogPostOsInitLowMemOnce()
{
    bool expected = false;
    if (!g_loggedPostOsInitLowMem.compare_exchange_strong(expected, true)) {
        return;
    }

    try {
        RT_LOG(RT_TAG_MEMORY)
            << "Post-OSInit lowmem: legacySize=0x" << std::hex << ::Memory::Read32(0x80000028u)
            << " legacyArenaLo=0x" << ::Memory::Read32(0x80000030u)
            << " legacyArenaHi=0x" << ::Memory::Read32(0x80000034u)
            << " simSize=0x" << ::Memory::Read32(0x800000F0u)
            << " bi2=0x" << ::Memory::Read32(0x800000F4u)
            << " physSize=0x" << ::Memory::Read32(0x80003100u)
            << " mem1Size=0x" << ::Memory::Read32(0x80003104u)
            << " mem1End=0x" << ::Memory::Read32(0x80003108u)
            << " arenaLo=0x" << ::Memory::Read32(0x8000310Cu)
            << " arenaHi=0x" << ::Memory::Read32(0x80003110u)
            << std::dec << std::endl;
    } catch (const ::Memory::AccessViolation& e) {
        LogMemoryError(RT_TAG_MEMORY, "Post-OSInit lowmem probe", e);
    }
}
...
extern "C" void OS__SetCurrentContext_801a1e70(uint32_t contextAddr)
{
    try {
        ...
        // Retail OSSetCurrentContext only changes the context's SRR1 exception
        // bit and the processor MSR recovery state.  OSContext+0x1A2 is save/
        // restore bookkeeping consumed by OSLoadContext; it is not MSR[EE].
...
extern "C" void OS__SaveContext_HLE(CpuContext* ctx)
{
    CpuContext* cpu = ctx ? ctx : &GetPersistentCpuContext();
    const uint32_t contextAddr = cpu->gpr[3];
    if (contextAddr == 0) {
        cpu->gpr[3] = 0;
        return;
    }
    ...
    try {
        // RVL OSSaveContext saves the nonvolatile GPR set plus the state needed
        // to resume after the setjmp-style call. This mirrors the RVL SDK
        // OSContext layout used by OSLoadContext below. r3 is stored as 1 so a restored context observes
        // the second return, while the initial call returns 0 to SelectThread.
        ::Memory::Write32(contextAddr + 0x04u, cpu->gpr[1]);
        ::Memory::Write32(contextAddr + 0x08u, cpu->gpr[2]);
        ::Memory::Write32(contextAddr + 0x0Cu, 1u);
        for (uint32_t reg = 13; reg <= 31; ++reg) {
            ::Memory::Write32(contextAddr + reg * 4u, cpu->gpr[reg]);
        }
        ::Memory::Write32(contextAddr + 0x80u, cpu->cr);
        ::Memory::Write32(contextAddr + 0x84u, cpu->lr);
        ::Memory::Write32(contextAddr + 0x88u, cpu->ctr);
        ::Memory::Write32(contextAddr + 0x8Cu, cpu->xer);
        ::Memory::Write32(contextAddr + 0x198u, cpu->lr);
        ::Memory::Write32(contextAddr + 0x19Cu, cpu->msr);
        for (uint32_t gqr = 1; gqr <= 7; ++gqr) {
            ::Memory::Write32(contextAddr + 0x1A4u + gqr * 4u, cpu->gqr[gqr]);
        }
        cpu->gpr[3] = 0;
    } catch (const ::Memory::AccessViolation& e) {
        LogMemoryError(RT_TAG_OS, "OS__SaveContext_HLE", e);
        cpu->gpr[3] = 0;
    }
}
...
extern "C" [[noreturn]] void OS__LoadContext_801a1f58(CpuContext* ctx)
{
    ...
    try {
        ...
        // Retail finishes with rfi: SRR1 becomes the live MSR, including EE.
        // Mirror both the translated register file and the host interrupt latch.
        cpu->msr = cpu->srr1;
        OS_HLE_ApplyInterruptStateFromMsr(cpu->msr);
        cpu->pc = srr0;

        LogPostOsInitLowMemOnce();
