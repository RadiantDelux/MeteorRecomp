...
void OS_HLE_ServiceIdleWait(CpuContext* cpu)
{
    if (!cpu) {
        return;
    }

    // This helper can be called from the middle of a statically translated
    // scheduler function. Hardware/timer callbacks must therefore run on an
    // interrupt copy, not on that function's resident register file. Otherwise
    // a callback's volatile r12/CTR (or even r13) is reloaded into the translated
    // function after the loop checkpoint and corrupts its pending dispatch.
    GuestInterruptCallbackContext interrupt;
    CpuContext* asyncCpu = interrupt.get();

    // Start with VI: the translated SDK idle path observed in RDSPAF is entered
    // immediately after VI configuration and the first hardware event that can
    // make its run queue non-empty is retrace. Other host scheduler pumps remain
    // owned by the native scheduler until a translated wait proves it needs them;
    // invoking unrelated alarm/fiber callbacks here can create nested scheduling
    // that real interrupt return semantics would not permit at this checkpoint.
    VI_HLE_PollRetrace(asyncCpu);
...
{
    // Zero means "leave the built-in MKW value alone" so existing non-generic
    // builds keep their historical layout. Generic-DOL title shims provide every
    // title-specific field they use rather than inheriting an MKW address.
    if (layout.defaultThreadContext != 0) {
        kDefaultThreadContextAddr = layout.defaultThreadContext;
    }
    if (layout.idleThreadContext != 0) {
        kIdleThreadContextAddr = layout.idleThreadContext;
    }
    if (layout.threadQueueArray != 0) {
        kThreadQueueArrayAddr = layout.threadQueueArray;
    }
    if (layout.switchThreadCallback != 0) {
        kSwitchThreadCallbackPtrAddr = layout.switchThreadCallback;
    }
    if (layout.schedulerDisableCount != 0) {
        kSchedulerIdleFlagAddr = layout.schedulerDisableCount;
    }
    if (layout.schedulerReschedFlag != 0) {
        kSchedulerReschedCounterAddr = layout.schedulerReschedFlag;
    }
    if (layout.schedulerPendingMask != 0) {
        kSchedulerPendingFlagAddr = layout.schedulerPendingMask;
    }
}

OsSchedulerGuestStateLayout OS_HLE_GetSchedulerGuestStateLayout()
{
    return OsSchedulerGuestStateLayout{
        kDefaultThreadContextAddr,
        kIdleThreadContextAddr,
        kThreadQueueArrayAddr,
        kSwitchThreadCallbackPtrAddr,
        kSchedulerIdleFlagAddr,
        kSchedulerReschedCounterAddr,
...
static void TryInvokeSwitchCallback(uint32_t oldCtx, uint32_t newCtx, CpuContext* cpu)
{
    try {
        ...
        if (CanDispatchGuestTarget(callbackAddr)) {
...
extern "C" void SelectThread_801a9c08(CpuContext* ctx)
{
    ...
    if (reschedPending == 0) {
        ...
        while (true) {
            ...
            // The retail scheduler executes an EE-setting mtmsr before entering
            // its idle spin and clears EE again before choosing the next thread.
            // The host interrupt helpers above track the global OS latch and the
            // guest OSContext mode flag, but they have no CpuContext parameter,
            // so mirror that architectural EE bit here as well.  Runtime loop
            // options are the generic checkpoint for asynchronous title hardware
            // while native HLE code is spinning with no translated call boundary.
            cpu->msr |= 0x00008000u;

            while (::Memory::Read32(kSchedulerPendingFlagAddr) == 0) {
                ApplyRuntimeLoopOptions(0u, cpu);
