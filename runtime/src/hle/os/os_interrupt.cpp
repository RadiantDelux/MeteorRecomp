...
inline void UpdateAmbientCpuInterruptFlag(bool enabled)
{
    // OSDisable/RestoreInterrupts operate on MSR[EE]. Keep the translated CPU
    // register file coherent with the host-side interrupt latch so an
    // OSSaveContext/OSLoadContext round trip carries the correct EE state.
...
extern "C" int32_t OS__DisableInterrupts_801a65ac()
{
    ...
    UpdateAmbientCpuInterruptFlag(false);
    return previous ? 1 : 0;
}
...
extern "C" int32_t OS__RestoreInterrupts_801a65d4(int32_t level)
{
    ...
    UpdateAmbientCpuInterruptFlag(level != 0);
    return prev ? 1 : 0;
}
...
extern "C" int32_t OS__EnableInterrupts_801a65c0()
{
    ...
    UpdateAmbientCpuInterruptFlag(true);
    return prev ? 1 : 0;
}
...
{
    g_interrupts_enabled.store((msr & kMsrExternalInterruptEnable) != 0,
