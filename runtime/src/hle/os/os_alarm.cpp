...
bool IsLikelyCodeAddress(uint32_t addr)
{
    ...
    if (CanDispatchGuestTarget(addr)) {
        return true;
    }
        ...
        try {
            for (int i = 0; i < maxToProcess; ++i) {
                ...
                if ((periodHi | periodLo) != 0) {
                    ...
                    cpu->gpr[6] = 0;
                    cpu->gpr[7] = handler;
                    InvokeIndirectCpu(kAlarmInsertFunctionAddr, cpu);
                }
...
{
    if (layout.queueOffsetFromR13 != 0) {
        kAlarmQueueOffsetFromR13 = layout.queueOffsetFromR13;
    }
    if (layout.insertAlarmFunction != 0) {
        kAlarmInsertFunctionAddr = layout.insertAlarmFunction;
    }
}
