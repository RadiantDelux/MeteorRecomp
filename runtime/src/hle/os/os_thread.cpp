...
{
    if (config.initContextFunction != 0) {
        kOSInitContextFunctionAddr = config.initContextFunction;
    }
    if (config.exitThreadFunction != 0) {
        kOSExitThreadFunctionAddr = config.exitThreadFunction;
    }
    if (config.schedulerInitFlag != 0) {
        kSchedulerInitFlagAddr = config.schedulerInitFlag;
    }
    if (config.threadAttrSource != 0) {
        kThreadAttrSourceAddr = config.threadAttrSource;
    }
}
...
extern "C" void OSCreateThread_HLE_801a9e84(CpuContext* ctx)
{
    ...
    try {
        ...
        // that follow in the original PPC.
        {
            ...
            InvokeIndirectCpu(kOSInitContextFunctionAddr, cpu); // OSInitContext
        }
        ...
        ::Memory::Write32(threadPtr + 0x84u, kOSExitThreadFunctionAddr); // LR = OSExitThread
        ...
        // CreateGuestFiber is allocated before the translated OSInitContext call
        // above, so seed the host fiber's architectural MSR only after retail has
        // written the initial SRR1 (0x9032, plus the OSCreateThread additions).
        // Otherwise a first switch into a new thread would look like EE=0 even
        // though OSLoadContext/rfi enables external interrupts for that context.
        if (Fiber::GuestFiberManager::IsInitialized()) {
            if (auto* fiber = Fiber::GuestFiberManager::GetFiber(threadPtr)) {
                const uint32_t initialMsr = ::Memory::Read32(threadPtr + 0x19Cu);
                fiber->cpuContext.msr = initialMsr;
                fiber->cpuContext.srr1 = initialMsr;
            }
        }
...
{
    OSCreateThread_HLE_801a9e84(ctx);
}
...
{
    OSResumeThread_HLE_801aa58c(ctx);
}
