...
void DrawGraphicsSettings() {
    ...
    constexpr std::array<const char*, 2> kAspectRatioModes{
        "4:3 (Original)",
        "16:9 (Widescreen)",
    };
    int aspectMode = g_widescreen ? 1 : 0;
    if (ImGui::Combo("Aspect ratio", &aspectMode, kAspectRatioModes.data(),
                     static_cast<int>(kAspectRatioModes.size()))) {
        g_widescreen = aspectMode == 1;
        RuntimeConfigFile::SetWidescreenEnabled(g_widescreen);

        uint32_t surfaceWidth = 0;
        uint32_t surfaceHeight = 0;
        AuroraGetSurfaceSize(&surfaceWidth, &surfaceHeight);
        ConfigureMkwDynamicAspect(g_widescreen, surfaceWidth, surfaceHeight);
    }
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(
            "4:3 preserves the original Wii presentation.\n"
            "16:9 enables the game's widescreen mode and presents it at the correct aspect ratio.\n"
    ...
    if (ImGui::BeginCombo("Internal render resolution", resolutionLabel)) {
    ...
    if (ImGui::IsItemHovered()) {
        ImGui::SetTooltip(
            "Scales the game's internal 3D render target.\n"
            "1x is native GameCube resolution; higher values render the scene more sharply.\n"
            "The setting applies immediately and is saved for the next launch.");
    }
    ImGui::TextDisabled("Higher multipliers use more GPU power and VRAM.");
    ImGui::Separator();
...
bool StartupScreenVisible() noexcept {
    ...
    // The startup cover is tied to Mario Kart's StrapScene::CheckInput hook.
    // Generic translated DOLs do not own that scene/address, so keeping the
    // cover armed would permanently hide otherwise valid guest frames.
    return false;
#else
