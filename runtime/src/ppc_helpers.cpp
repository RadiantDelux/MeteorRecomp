...
extern "C" void PPC_Dcbst(uint32_t address)
{
    DataCacheWriteback::PublishLine(address,
        [](uint32_t line, uint32_t bytes) { return Memory::Contains(line, bytes); },
        [](uint32_t line, uint32_t bytes) { GxNotifyGuestRamDmaWrite(line, bytes); });
}

extern "C" void PPC_Dcbf(uint32_t address)
{
    // There is no separate host copy of the guest's L1 cache to invalidate;
    // its writeback has the same publication boundary as dcbst.
    PPC_Dcbst(address);
}
...
extern "C" uint32_t PPC_Stwcx(uint32_t addr, uint32_t value, uint32_t cr, uint32_t xer)
{
    ...
    // stwcx. always records into CR0.  Make the updated architectural value the
    // helper result instead of mutating the thread-local CpuContext invisibly;
    // generated code can then keep CR resident without relying on aliasing the TLS
    // CpuContext through a helper that does not receive it.
    const uint32_t so = (xer >> 31) & 1u;
    const uint32_t cr0 = (success ? 0x2u : 0u) | so;
    return (cr & 0x0FFFFFFFu) | (cr0 << 28);
}
...
extern "C" uint32_t PPC_ReadSpr(uint32_t spr)
{
    ...
    switch (spr)
    {
        ...
        case LockedCacheDma::kUpperSpr:
        case LockedCacheDma::kLowerSpr:
            return shadowValue();
...
extern "C" void PPC_WriteSpr(uint32_t spr, uint32_t value)
{
    ...
    switch (spr)
    {
        ...
        case LockedCacheDma::kUpperSpr:
            ...
            return;
        case LockedCacheDma::kLowerSpr:
            LockedCacheDma::WriteLower(g_sprShadow[LockedCacheDma::kUpperSpr], g_sprShadow[spr], value,
                [](const LockedCacheDma::Transfer& transfer) {
                    // This also covers inlined LCLoadBlocks/LCStoreBlocks: they
                    // write the SPRs directly and never call an SDK HLE entry.
                    // Keep guest bytes unchanged (DMA does not swap scalars).
                    auto* source = Memory::GetPointer(transfer.source, transfer.bytes);
                    auto* destination = Memory::GetPointer(transfer.destination, transfer.bytes);
                    std::memmove(destination, source, transfer.bytes);
                    GxNotifyGuestRamDmaWrite(transfer.destination, transfer.bytes);
                    static uint32_t loggedTransfers = 0;
                    if (loggedTransfers++ < 8) {
                        RT_LOG(RT_TAG_RUNTIME) << "Locked-cache DMA " << (transfer.load ? "load" : "store")
                            << ": src=0x" << std::hex << transfer.source << " dst=0x" << transfer.destination
                            << " bytes=0x" << transfer.bytes << std::dec << std::endl;
                    }
                });
            return;
