    ...
    for (void* f : toDelete) {
        if (f) {
            if (HostContext::IsCurrent(f)) {
                // A terminated guest can still own the host stack while the
                // scheduler idles. Keep it until a different fiber can reap it.
                std::lock_guard<std::mutex> lock(s_mutex);
                s_fibersPendingDelete.push_back(f);
            } else {
...
uint32_t ThreadQueueArrayAddr() { return OS_HLE_GetSchedulerGuestStateLayout().threadQueueArray; }
uint32_t SchedulerPendingFlagAddr() { return OS_HLE_GetSchedulerGuestStateLayout().schedulerPendingMask; }
uint32_t SchedulerReschedCounterAddr() { return OS_HLE_GetSchedulerGuestStateLayout().schedulerReschedFlag; }
...
{
    const uint32_t threadQueueArrayAddr = ThreadQueueArrayAddr();
    if (queueEntry < threadQueueArrayAddr ||
        ((queueEntry - threadQueueArrayAddr) % 8u) != 0) {
        return;
    }

    const uint32_t priority = (queueEntry - threadQueueArrayAddr) / 8u;
    if (priority >= 32 || Memory::Read32(queueEntry) != 0) {
        return;
    }

    const uint32_t pendingFlagAddr = SchedulerPendingFlagAddr();
    const uint32_t pending = Memory::Read32(pendingFlagAddr);
    Memory::Write32(pendingFlagAddr, pending & ~(1u << (31u - priority)));
}
...
void WakeGuestThreadsOnQueueNoSwitch(uint32_t queueAddr)
{
    ...
    for (int woke = 0; woke < kMaxWake; ++woke) {
        ...
        const uint32_t runQueue = ThreadQueueArrayAddr() + static_cast<uint32_t>(priority) * 8u;
        ...
        const uint32_t pendingFlagAddr = SchedulerPendingFlagAddr();
        const uint32_t pending = Memory::Read32(pendingFlagAddr);
        Memory::Write32(pendingFlagAddr, pending | (1u << (31u - static_cast<uint32_t>(priority))));
        Memory::Write32(SchedulerReschedCounterAddr(), 1);
    ...
    if (existingIt != s_fibers.end()) {
        ...
        if (existingIt->second.fiber && !existingIt->second.isSchedulerFiber) {
            if (HostContext::IsCurrent(existingIt->second.fiber)) {
                // An alarm can reuse a detached OSThread after its entry has
                // returned, while its old host fiber is still in SelectThread.
                // DeleteFiber on that stack terminates the whole Windows thread.
                s_fibersPendingDelete.push_back(existingIt->second.fiber);
                // The replacement has a new context even though its guest
                // address is identical. Do not save the retiring registers over
                // that context in SwitchToThread.
                s_currentGuestThread = 0;
            } else {
...
void GuestFiberManager::SwitchToThread(uint32_t guestThreadAddr, CpuContext* cpu) {
    ...
    if (cpu && haveTargetContext) {
        *cpu = targetContext;
        OS_HLE_ApplyInterruptStateFromMsr(cpu->msr);
...
#endif
{
    ...
    try {
        ...
        // OSInitContext seeds SRR1 with the thread's initial machine state.
        // A real first dispatch reaches the entry through rfi, which copies
        // SRR1 into MSR. Host fibers start from a zeroed CpuContext, so mirror
        // that rfi effect explicitly or new guest threads incorrectly run with
        // external interrupts disabled.
        cpu->srr1 = Memory::Read32(guestThreadAddr + kCtxSrr1Offset);
        cpu->msr = cpu->srr1;
        OS_HLE_ApplyInterruptStateFromMsr(cpu->msr);
    ...
    if (CanDispatchGuestTarget(entryPoint)) {
    ...
    // guest OSThread lifecycle here and then enter SelectThread from the dying
    // fiber.  Merely clearing Current/Running and jumping to the scheduler host
    // fiber strands every READY guest thread: the scheduler fiber is the main
    // guest thread's existing C++ call stack, not a standalone dispatch loop.
    ...
    // Keep OSCurrentContext/OSRunningContext and s_currentGuestThread pointing at
    // this terminated thread until SelectThread chooses its successor.  Because
    // the guest state is already 0/MORIBUND, SelectThread will not requeue it;
    // it can directly consume the pending run queues (or idle until an IRQ wakes
    // one) and SwitchToThread performs the non-local host-fiber transfer.
    cpu->gpr[3] = 1;
    OS_HLE_SelectThread(cpu);

    // A successful switch never returns to a terminated fiber.  Keep the old
    // scheduler jump only as a defensive escape for an unexpected refusal path
    // such as scheduler-disable nesting.
    RT_LOG(RT_TAG_OS) << "Thread return scheduler unexpectedly returned for 0x"
                      << std::hex << guestThreadAddr << std::dec << std::endl;
    try {
