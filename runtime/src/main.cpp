...
template <size_t N>
void WriteFatalBreadcrumbRaw(const char (&message)[N]) noexcept {
#if defined(_WIN32)
    ...
    DWORD written = 0;
    (void)::WriteFile(handle, message, static_cast<DWORD>(N - 1), &written, nullptr);
#else
...
const TranslatedFunctionInfo* ResolveEntry() {
    const auto* entry = TranslatedFunctionRegistry::FindByAddressPtr(RuntimeConfig::ENTRY_POINT);
    if (!entry) {
        std::ostringstream oss;
        oss << "No translated function registered at address 0x" << std::hex << RuntimeConfig::ENTRY_POINT;
...
LONG ReportFatalSehAndExit(EXCEPTION_POINTERS* info) {
    ...
    if (s_inCrashHandler.test_and_set()) {
        WriteFatalBreadcrumbRaw("[runtime] fatal breadcrumb: SEH reporter re-entry -> _Exit(1)\n");
        std::_Exit(EXIT_FAILURE);
    }

    WriteFatalBreadcrumbRaw("[runtime] fatal breadcrumb: SEH reporter first entry\n");
...
void PosixMemoryFaultHandler(int sig, siginfo_t* info, void* ucontextVoid) {
    ...
    static std::atomic_flag s_inCrashHandler = ATOMIC_FLAG_INIT;
    if (s_inCrashHandler.test_and_set()) {
        WriteFatalBreadcrumbRaw("[runtime] fatal breadcrumb: POSIX fault reporter re-entry -> _Exit(1)\n");
        std::_Exit(EXIT_FAILURE);
    }

    WriteFatalBreadcrumbRaw("[runtime] fatal breadcrumb: POSIX fault reporter first entry\n");
...
void AbortSignalHandler(int signum) {
    ...
    if (g_abortSignalHandled.test_and_set()) {
        WriteFatalBreadcrumbRaw("[runtime] fatal breadcrumb: SIGABRT handler re-entry -> _Exit(1)\n");
        std::_Exit(EXIT_FAILURE);
    }


    WriteFatalBreadcrumbRaw("[runtime] fatal breadcrumb: SIGABRT handler first entry\n");
    ...
    if (g_fatalErrorReported.load(std::memory_order_acquire)) {
        WriteFatalBreadcrumbRaw("[runtime] fatal breadcrumb: SIGABRT saw prior fatal report -> _Exit(1)\n");
        std::fflush(stderr);
        std::fflush(stdout);
        std::_Exit(EXIT_FAILURE);
    }
...
static void TerminateHandler() {
    ...
    if (g_fatalErrorReported.load(std::memory_order_acquire)) {
        WriteFatalBreadcrumbRaw("[runtime] fatal breadcrumb: terminate saw prior fatal report -> _Exit(1)\n");
        std::fflush(stderr);
        std::_Exit(EXIT_FAILURE);
    }
    WriteFatalBreadcrumbRaw("[runtime] fatal breadcrumb: terminate handler first entry\n");
...
bool ValidateGameCubeDiscDataBeforeStartup() {
    const std::filesystem::path dvdRoot = RuntimeConfigFile::ResolvedDvdRoot();

    auto isDirectory = [](const std::filesystem::path& path) {
        std::error_code ec;
        return std::filesystem::is_directory(path, ec) && !ec;
    };
    auto isRegularFile = [](const std::filesystem::path& path) {
        std::error_code ec;
        return std::filesystem::is_regular_file(path, ec) && !ec;
    };

    const bool hasRequiredLayout =
        !dvdRoot.empty() &&
        isDirectory(dvdRoot / "files") &&
        isRegularFile(dvdRoot / "sys" / "boot.bin") &&
        isRegularFile(dvdRoot / "sys" / "fst.bin");

    bool discIdMatches = false;
    if (hasRequiredLayout) {
        std::ifstream boot(dvdRoot / "sys" / "boot.bin", std::ios::binary);
        std::array<unsigned char, 6> discId{};
        if (boot.read(reinterpret_cast<char*>(discId.data()),
                      static_cast<std::streamsize>(discId.size()))) {
            const uint32_t gameCode =
                (static_cast<uint32_t>(discId[0]) << 24) |
                (static_cast<uint32_t>(discId[1]) << 16) |
                (static_cast<uint32_t>(discId[2]) << 8) |
                static_cast<uint32_t>(discId[3]);
            const uint16_t makerCode =
                static_cast<uint16_t>((static_cast<uint16_t>(discId[4]) << 8) |
                                      static_cast<uint16_t>(discId[5]));
            discIdMatches = gameCode == RuntimeConfig::GAME_CODE &&
                            makerCode == RuntimeConfig::MAKER_CODE;
        }
    }

    if (hasRequiredLayout && discIdMatches) {
        return true;
    }

    std::array<char, 7> expectedDiscId{{
        static_cast<char>((RuntimeConfig::GAME_CODE >> 24) & 0xFFu),
        static_cast<char>((RuntimeConfig::GAME_CODE >> 16) & 0xFFu),
        static_cast<char>((RuntimeConfig::GAME_CODE >> 8) & 0xFFu),
        static_cast<char>(RuntimeConfig::GAME_CODE & 0xFFu),
        static_cast<char>((RuntimeConfig::MAKER_CODE >> 8) & 0xFFu),
        static_cast<char>(RuntimeConfig::MAKER_CODE & 0xFFu),
        '\0',
    }};

    std::ostringstream details;
    details << "This release needs an extracted copy of the correct GameCube disc.\n\n";
    if (!dvdRoot.empty()) {
        details << "Configured game folder:\n"
                << RuntimeConfigFile::PathToUtf8(dvdRoot) << "\n\n";
    }
    details << "Expected layout:\n"
            << "game\\sys\\boot.bin\n"
            << "game\\sys\\fst.bin\n"
            << "game\\files\\...\n\n"
            << "Expected disc ID: " << expectedDiscId.data();
    if (hasRequiredLayout && !discIdMatches) {
        details << "\n\nThe folder exists, but it is not the disc version this build was compiled for.";
    }

    ShowRuntimeFatalPopup("the required GameCube disc data was not found", details.str());
    MarkFatalErrorReported();
    SetRuntimeExitCodeImpl(EXIT_FAILURE);
    return false;
}
#endif
...
int RuntimeMain(int argc, char** argv) {
    ...
    const auto shutdownAurora = [] {
        // Configuration and guest-memory setup can fail before Aurora exists.
        // Its ImGui/backend teardown requires a successful initialization.
...
#if WIICOMPILED_GUEST_IS_GAMECUBE
        if (!ValidateGameCubeDiscDataBeforeStartup()) {
...
// only vulkan for linux
        ...
        // Opt-in renderer diagnostic: capture Aurora's actual present-source texture
        // at an exact logical frame. This is intentionally environment-gated so
        // normal runtime behavior is unchanged when no capture is requested.
        if (const char* captureFrameText = std::getenv("MKW_CAPTURE_FRAME")) {
            char* end = nullptr;
            const unsigned long requestedFrame = std::strtoul(captureFrameText, &end, 0);
            if (end != captureFrameText && end != nullptr && *end == '\0' &&
                requestedFrame <= std::numeric_limits<uint32_t>::max()) {
                const char* capturePath = std::getenv("MKW_CAPTURE_PATH");
                aurora_request_frame_capture(static_cast<uint32_t>(requestedFrame),
                                             capturePath != nullptr ? capturePath : "frame_capture.bmp");
                RT_LOG(RT_TAG_RUNTIME) << "requested Aurora present-source capture for frame "
                                       << requestedFrame << std::endl;
            }
        }
        ...
        RT_LOG(RT_TAG_CONFIG) << "video.widescreen=" << (configWidescreen ? "true" : "false")
                  ...
                  << " presentAspect="
                  << (g_dynamicAspectRatioEnabled ? "surface (dynamic EGG canvas)"
                                                  : (configWidescreen ? "16:9" : "4:3"))
                  << std::endl;
#if defined(MKW_GENERIC_DOL_BOOT)
        // Generic DOLs own GXInit and therefore touch CP MMIO before any title-
        // specific HLE hook has a chance to reset the host parser. Bring the
        // reusable GX FIFO/CP backend online once Aurora and guest-write hooks are
        // ready, while leaving the established Wii/title boot paths unchanged.
        GX_HLE_ResetFifoParser();
#endif
