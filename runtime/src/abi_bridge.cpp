...
static_assert(sizeof(CpuContext) == 504);
...
const RawDispatchRecord* FindDynamicRawDispatchEntry(uint32_t address) {
...
std::string ValidateGeneratedIndirectDispatchLocked() {
    ...
    for (size_t i = 0; i < table->entryCount; ++i) {
        ...
        // A higher-priority native/title HLE is intentionally kept in the
        // compact dynamic overlay by RebuildIndicesLocked().  The generated
        // translated entry remains structurally valid, but is shadowed and
        // must never be selected while that native winner is installed.
        if (info.kind == FunctionKind::Native) {
            const auto* nativeOverride = FindDynamicRawDispatchEntry(record.address);
            if (!nativeOverride || nativeOverride->entry != info.rawCpuInvoker ||
                nativeOverride->nonvolatileFprWriteMask != NonvolatileFprGuardMaskFor(&info) ||
                nativeOverride->preserveNonvolatileGprs != ShouldPreserveNonvolatileGprsForRawCpuCall(&info)) {
                std::ostringstream message;
                message << "Missing native indirect dispatch overlay at 0x" << std::hex
...
const RawDispatchRecord* TranslatedFunctionRegistry::FindRawByAddressPtrSlow(uint32_t address) {
    ...
    // Native/title HLE winners form a small dynamic overlay over the generated
    // translated table. Check it first so a deliberate higher-priority native
    // replacement also wins indirect calls (bctrl), not only direct registry
    // lookups.
    if (const auto* nativeOverride = FindDynamicRawDispatchEntry(address)) {
