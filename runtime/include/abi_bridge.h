...
#if defined(WIICOMPILED_TITLE_RUNTIME_CALL_OPTIONS_HEADER)
#include WIICOMPILED_TITLE_RUNTIME_CALL_OPTIONS_HEADER
#elif defined(MKW_TITLE_RUNTIME_CALL_OPTIONS_HEADER)
#include MKW_TITLE_RUNTIME_CALL_OPTIONS_HEADER
#endif
...
// Some title-facing runtime options alter arguments or synchronize host-backed
// state at well-defined ABI boundaries. Keep this independent of the dispatch
// mechanism: generated static calls deliberately bypass InvokeDirectCpu for
// performance. New title adapters use the WIICOMPILED_* hook; MKW_* remains a
// compatibility fallback for existing build integrations and out-of-tree titles.
inline void ApplyRuntimeCallOptions(uint32_t target, CpuContext* ctx) {
#ifdef WIICOMPILED_APPLY_TITLE_RUNTIME_CALL_OPTIONS
    WIICOMPILED_APPLY_TITLE_RUNTIME_CALL_OPTIONS(target, ctx);
#elif defined(MKW_APPLY_TITLE_RUNTIME_CALL_OPTIONS)
    MKW_APPLY_TITLE_RUNTIME_CALL_OPTIONS(target, ctx);
#else
    (void)target;
    (void)ctx;
#endif
...
inline void ApplyRuntimeReturnOptions(uint32_t target, CpuContext* ctx) {
#ifdef WIICOMPILED_APPLY_TITLE_RUNTIME_RETURN_OPTIONS
    WIICOMPILED_APPLY_TITLE_RUNTIME_RETURN_OPTIONS(target, ctx);
#else
    (void)target;
    (void)ctx;
#endif
...
// Translated guest code can wait for asynchronous hardware in a loop that has
// no function calls at all.  A call-boundary hook alone cannot service such an
// external interrupt, so the translator also exposes a lightweight checkpoint
// on CFG backedges.  Titles opt into work here explicitly; without a title hook
// this compiles to a no-op and has no runtime cost.
inline void ApplyRuntimeLoopOptions(uint32_t guestPc, CpuContext* ctx) {
#ifdef WIICOMPILED_APPLY_TITLE_RUNTIME_LOOP_OPTIONS
    WIICOMPILED_APPLY_TITLE_RUNTIME_LOOP_OPTIONS(guestPc, ctx);
#elif defined(MKW_APPLY_TITLE_RUNTIME_LOOP_OPTIONS)
    MKW_APPLY_TITLE_RUNTIME_LOOP_OPTIONS(guestPc, ctx);
#else
    (void)guestPc;
    (void)ctx;
#endif
...
inline bool ShouldApplyRuntimeLoopOptions(uint32_t guestPc) {
#ifdef WIICOMPILED_SHOULD_APPLY_TITLE_RUNTIME_LOOP_OPTIONS
    return WIICOMPILED_SHOULD_APPLY_TITLE_RUNTIME_LOOP_OPTIONS(guestPc);
#else
    (void)guestPc;
    return true;
#endif
...
private:
    // Memo miss: the slow path first checks the small dynamic native/title-HLE
    // overlay, then falls through to the generated translated table. This lets
    // a higher-priority native replacement shadow a generated function for
    // indirect calls as well as direct registry lookups.
    static MKW_PPC_NO_INLINE MKW_PPC_COLD const RawDispatchRecord* FindRawByAddressPtrMiss(
        ...
        const RawDispatchRecord* record = FindRawByAddressPtrSlow(address);
...
inline bool TryDispatchDynamicCpuTarget(uint32_t target, CpuContext* cpu) {
    DynamicModule::ResolvedTarget resolved;
    if (!DynamicModule::TryResolveTarget(target, resolved) || !resolved) {
        return false;
    }

    DynamicModule::ScopedExecution dynamicExecution(cpu, resolved);
    PpcNonvolatileGprGuard gprGuard(cpu, resolved.function->preserveNonvolatileGprs);
    PpcNonvolatileFprGuard fprGuard(cpu, resolved.function->nonvolatileFprWriteMask);
    if (TryGetCpuContext() != cpu) {
        CpuContextScope scope(cpu);
        resolved.function->entry(cpu);
    } else {
        resolved.function->entry(cpu);
    }
    return true;
...
inline bool TryDispatchCurrentDynamicModuleFunction(uint32_t functionOffset, CpuContext* cpu) {
    DynamicModule::ResolvedTarget resolved;
    if (!DynamicModule::TryResolveCurrentFunction(cpu, functionOffset, resolved) || !resolved) {
        return false;
    }

    // The current module instance has already been validated by
    // TryResolveCurrentFunction. Nested same-template calls keep the same
    // runtime base/generation while preserving ordinary ABI guard semantics.
    DynamicModule::ScopedExecution dynamicExecution(cpu, resolved);
    PpcNonvolatileGprGuard gprGuard(cpu, resolved.function->preserveNonvolatileGprs);
    PpcNonvolatileFprGuard fprGuard(cpu, resolved.function->nonvolatileFprWriteMask);
    if (TryGetCpuContext() != cpu) {
...
inline void InvokeCurrentDynamicModuleFunction(uint32_t functionOffset, CpuContext* cpu) {
    if (TryDispatchCurrentDynamicModuleFunction(functionOffset, cpu)) {
        return;
    }
    const uint32_t runtimeTarget = DynamicModule::RuntimeAddressFromOffset(cpu, functionOffset);
    ReportMissingCpuTarget(runtimeTarget, cpu);
}

inline bool CanDispatchGuestTarget(uint32_t target) {
    // Fixed translated/native address ownership always wins. Dynamic templates
    // are a separate fallback namespace and cannot override DOL/REL/fixed-mod
    // registrations at the same guest address.
    if (TranslatedFunctionRegistry::FindRawByAddressPtr(target) != nullptr ||
        TranslatedFunctionRegistry::FindByAddressPtr(target) != nullptr) {
        return true;
    }
    return DynamicModule::CanDispatchTarget(target);
}
...
#ifdef WIICOMPILED_FORCE_DYNAMIC_DIRECT_CALL
    // Some title-local replacements must shadow a translated target even when
    // the translator resolved a direct branch statically. Route only those
    // explicitly opted-in addresses through the finalized registry so the
    // normal priority rules (native > translated) still choose the winner.
    if constexpr (WIICOMPILED_FORCE_DYNAMIC_DIRECT_CALL(Target)) {
...
template <uint32_t Target, void (*Entry)(CpuContext*)>
inline void InvokeStaticTranslatedCpu(CpuContext* ctx) {
    static_assert(Target != 0, "InvokeStaticTranslatedCpu cannot target address 0");
    CpuContext* cpu = ctx ? ctx : &GetPersistentCpuContext();
    ApplyRuntimeCallOptions(Target, cpu);
#ifdef WIICOMPILED_FORCE_DYNAMIC_DIRECT_CALL
    if constexpr (WIICOMPILED_FORCE_DYNAMIC_DIRECT_CALL(Target)) {
...
#endif
    Entry(cpu);
    ApplyRuntimeReturnOptions(Target, cpu);
}
...
inline void InvokeIndirectJump(uint32_t target, CpuContext* ctx) {
    ...
    if (TryDispatchDynamicCpuTarget(target, cpu)) {
        return;
    }
...
// Per-title HLE registrations are deliberately separate from the legacy
// Mario-Kart-address table below.  A generic DOL build suppresses the legacy
// table, while source files supplied through MKW_TITLE_NATIVE_DIR can still
// register addresses that belong to that title.
#define REGISTER_TITLE_NATIVE_FUNCTION(address, fn) \
    static AbiTrampoline<decltype(fn)> MKW_DETAIL_MAKE_UNIQUE(_abi_title_native_trampoline_, __COUNTER__)(address, #fn, fn, FunctionKind::Native, false, kPpcAllNonvolatileFprMask, 0, 0, &AbiRawCpuThunk<&fn>::Invoke)

#define REGISTER_TITLE_NATIVE_FUNCTION_AS(address, fn, pretty_name) \
    static AbiTrampoline<decltype(fn)> MKW_DETAIL_MAKE_UNIQUE(_abi_title_native_trampoline_named_, __COUNTER__)(address, pretty_name, fn, FunctionKind::Native, false, kPpcAllNonvolatileFprMask, 0, 0, &AbiRawCpuThunk<&fn>::Invoke)
...
// The only hand-written "translated" registration in runtime/src is also
// pinned to an MKW guest address. Generic titles get translated winners solely
// from the generated bulk-registration shards for their own DOL.
...
#if defined(MKW_GENERIC_DOL_BOOT)
// The hand-written native-address table in runtime/src is currently pinned to
// Mario Kart Wii.  A generic DOL must not install those trampolines: identical
// guest addresses can name completely unrelated functions in another title.
