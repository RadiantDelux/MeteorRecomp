...
struct CpuContext {
    ...
    // Host-only translated-execution metadata for guest modules loaded at a
    // runtime-selected address. This state intentionally travels with the
    // CpuContext so guest fiber save/restore cannot leak one module's base into
    // another fiber sharing the same host thread.
    const void* dynamicModuleTemplate = nullptr;
    uint32_t dynamicModuleRuntimeBase = 0;
    uint32_t dynamicModuleCanonicalBase = 0;
    uint32_t dynamicModuleImageSize = 0;
    uint32_t dynamicModuleRuntimeBssBase = 0;
    uint32_t dynamicModuleCanonicalBssBase = 0;
    uint32_t dynamicModuleBssSize = 0;
    uint64_t dynamicModuleGeneration = 0;
};
