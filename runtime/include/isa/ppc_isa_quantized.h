...
inline SignedType PpcScaleAndClampPsqInline(float value, uint32_t scale)
{
    ...
    // GQR scales are signed six-bit exponents (-32..31). Every factor is
    // exactly representable as a normal float, so construct its exponent
    // directly instead of calling the host libm for every quantized lane.
    const int exponent = scale < 32u ? static_cast<int>(scale) : static_cast<int>(scale) - 64;
    const float factor = PpcBitCastToFloatInline(static_cast<uint32_t>(127 + exponent) << 23);
...
inline float PpcDequantizePsqInline(SignedType value, uint32_t scale)
{
    ...
    const int exponent = scale < 32u ? -static_cast<int>(scale) : 64 - static_cast<int>(scale);
    const float factor = PpcBitCastToFloatInline(static_cast<uint32_t>(127 + exponent) << 23);
