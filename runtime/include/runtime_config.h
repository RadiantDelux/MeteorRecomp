...
struct RuntimeUserConfig {
    ...
    // Host-side emulated Wii Remote + Nunchuk. When enabled, channel 0 is
    // exposed through WPAD/KPAD even without a physical Wii Remote; ordinary
    // SDL gamepads and the keyboard feed the emulated peripheral. A real Wii
    // Remote on the channel always takes precedence.
    std::optional<bool> wiiVirtualRemote;
...
// Resolve once so every user-state path shares the same portable root.
inline const std::optional<std::filesystem::path>& PortableRootDirectory() {
    static const auto root = []() -> std::optional<std::filesystem::path> {
        const auto executableDirectory = ExecutableDirectory();
        return executableDirectory ? RuntimeProjectPaths::FindPortableRoot(*executableDirectory, kPortableSearchDepth)
                                   : std::nullopt;
    }();
    return root;
}
...
inline std::filesystem::path ApplicationDataDirectory() {
    return RuntimeProjectPaths::UserDataDirectory(PortableRootDirectory());
}
...
inline void EnsureConfigFile() {
    ...
    output << "# WiiCompiled user configuration\n"
              "# paths.dvd_root is relative to this Config.toml.\n\n"
              "[video]\n"
              "widescreen = true\n"
              ...
              "[paths]\n"
              << (RuntimeProjectPaths::kStorageNamespace.empty() ? "dvd_root = \"../game\"\n" : "dvd_root = \"game\"\n") <<
              "# nand_root = \"D:\\\\WiiNand\"\n"
...
inline RuntimeUserConfig ParseConfigDocument(const toml::value& document) {
    ...
    config.wiiVirtualRemote = FindConfigValue<bool>(document, "controller", "wii_virtual_remote");
...
inline bool WriteSetting(std::string_view section, std::string_view key, std::string_view value) {
    ...
    if (sectionStart == lines.size()) {
        ...
        if (!replaced) {
            ...
            // the next section. Host configuration writers apply the same rule.
...
inline bool SetWidescreenEnabled(bool value) {
...
// Exposes channel 0 as an emulated Wii Remote + Nunchuk backed by ordinary
// host controls. Disabled by default so projects keep their previous input
// behaviour unless they explicitly opt in.
inline bool WiiVirtualRemoteEnabled(bool fallback = false) {
    return Get().wiiVirtualRemote.value_or(fallback);
}

inline bool SetWiiVirtualRemoteEnabled(bool value) {
    Mutable().wiiVirtualRemote = value;
    return WriteSetting("controller", "wii_virtual_remote", value ? "true" : "false");
}
