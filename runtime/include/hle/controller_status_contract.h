...
public:
    ...
    int32_t GetDataFormat(uint32_t chan) const
    {
        ...
        return m_initialized ? m_dataFormat[chan] : kErrorNotReady;
    }

    int32_t SetDataFormat(uint32_t chan, int32_t format)
    {
        if (chan >= kChannelCount) {
            return kErrorBadChannel;
        }
        ...
        m_dataFormat[chan] = format;
        return 0;
    }

private:
    bool m_initialized = false;
    std::array<int32_t, kChannelCount> m_dataFormat{};
};
