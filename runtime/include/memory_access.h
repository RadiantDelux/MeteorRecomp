...
MKW_MEMORY_NO_INLINE MKW_MEMORY_COLD double ReadFloat32Slow(uint32_t addr);
...
MKW_MEMORY_NO_INLINE MKW_MEMORY_COLD double ReadResolvedFallbackFloat32(uint32_t addr);
...
// Only MMIO reads with an implemented device model should bypass the flat-view fault path.
// Unknown MMIO deliberately remains PAGE_NOACCESS so it still fails loudly without adding a
// policy branch to every guest read.
MKW_MEMORY_FORCE_INLINE bool FlatRead32NeedsPolicy(uint32_t address) {
#if WIICOMPILED_GUEST_IS_GAMECUBE
    return (address >= 0xCC003000u && address <= 0xCC003030u) || // GameCube/Wii PI registers
           (address >= 0xCC002000u && address <= 0xCC002074u) || // Video Interface
           (address >= 0xCC004000u && address <= 0xCC00405Au) || // Memory Interface
           (address >= 0xCC005000u && address <= 0xCC00503Au) || // DSP/ARAM/Audio DMA
           (address >= 0xCC006000u && address <= 0xCC006024u) || // DVD Interface
           (address >= 0xCC006400u && address <= 0xCC0064FCu) || // Serial Interface + buffer
           (address >= 0xCC006800u && address <= 0xCC006838u) || // Expansion Interface
           (address >= 0xCC006C00u && address <= 0xCC006C0Cu) || // Audio Interface
           address == 0xCD0000C0u;   // Hollywood GPIOB_OUT
#else
    return (address >= 0xCC003000u && address <= 0xCC003030u) || // GameCube/Wii PI registers
           (address >= 0xCC004000u && address <= 0xCC00405Au) || // Wii/GC Memory Interface
           (address >= 0xCC005000u && address <= 0xCC00503Au) || // Wii DSP/audio-DMA registers
           (address >= 0xCD006000u && address <= 0xCD006024u) || // Wii Drive Interface registers
           (address >= 0xCD006400u && address <= 0xCD0064FCu) || // Wii Serial Interface registers
           (address >= 0xCD006800u && address <= 0xCD006838u) || // Wii EXI registers
           (address >= 0xCD006C00u && address <= 0xCD006C0Cu) || // Wii Audio Interface registers
           address == 0xCD800180u || // Hollywood HW_COMPAT
           address == 0xCD8001CCu || // Hollywood HW_PLLAI
           address == 0xCD8001D0u || // Hollywood HW_PLLAIEXT
           address == 0xCD8000C0u || // Hollywood GPIOB_OUT trusted mirror
           address == 0xCD8000C4u || // Hollywood GPIOB_DIR trusted mirror
           address == 0xCD8000C8u || // Hollywood GPIOB_IN trusted mirror
           address == 0xCD0000C0u;   // Hollywood GPIOB_OUT PPC window
#endif
}

MKW_MEMORY_FORCE_INLINE bool FlatRead16NeedsPolicy(uint32_t address) {
#if WIICOMPILED_GUEST_IS_GAMECUBE
    return (address >= 0xCC000000u && address < 0xCC000080u) ||  // Command Processor
           (address >= 0xCC003000u && address <= 0xCC003032u) || // PI 16-bit read halves
           (address >= 0xCC002000u && address <= 0xCC002074u) || // Video Interface
           (address >= 0xCC004000u && address <= 0xCC00405Au) || // Memory Interface
           (address >= 0xCC005000u && address <= 0xCC00503Au) || // DSP/ARAM/Audio DMA
           (address >= 0xCC006C00u && address <= 0xCC006C0Eu) || // Audio Interface halves
           (address >= 0xCC001000u && address <= 0xCC00100Au) || // PE poke/config + control
           address == 0xCC00100Eu;   // PE current draw-sync token
#else
    return (address >= 0xCC000000u && address < 0xCC000080u) ||  // Command Processor
           (address >= 0xCC002000u && address <= 0xCC002074u) || // Wii/GC Video Interface
           (address >= 0xCC004000u && address <= 0xCC00405Au) || // Wii/GC Memory Interface
           (address >= 0xCC005000u && address <= 0xCC00503Au) || // Wii DSP/audio-DMA halves
           (address >= 0xCC001000u && address <= 0xCC00100Au) || // PE poke/config + control
           address == 0xCC00100Eu;   // PE current draw-sync token
#endif
}
...
MKW_MEMORY_FORCE_INLINE double ReadResolvedFloat32(uint8_t* r, uint32_t o, uint32_t a) {
    uint32_t bits = 0;
...
// in guest_flat_memory.h). MMIO accesses are the exception: writes must be dispatched before a
// fault because the written value is not recoverable from a fault record, while reads must reach
// the device dispatcher so implemented registers can return values instead of faulting first.
//
...
MKW_MEMORY_FORCE_INLINE uint16_t FlatRead16(uint32_t address) {
    ...
    if (FlatRead16NeedsPolicy(address)) [[unlikely]] return Read16Slow(address);
...
MKW_MEMORY_FORCE_INLINE uint32_t FlatRead32(uint32_t address) {
    ...
    if (FlatRead32NeedsPolicy(address)) [[unlikely]] return Read32Slow(address);
    return FlatLoad<uint32_t>(address);
}

MKW_MEMORY_FORCE_INLINE double FlatReadFloat32(uint32_t address) {
...
inline double Memory::ReadFloat32(uint32_t addr) {
    uint32_t bits = 0;
