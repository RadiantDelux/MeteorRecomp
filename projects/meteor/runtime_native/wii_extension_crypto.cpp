// Wii extension first-party crypto derived from Dolphin Emulator's WiimoteEmu encryption code.
// Dolphin copyright 2008 Dolphin Emulator Project; Hector Martin (marcan).
// SPDX-License-Identifier: GPL-2.0-or-later

#include "wii_extension_crypto.h"

#include <algorithm>
#include <array>
#include <bit>

namespace MeteorWiiExtensionCrypto {
namespace {
using SBox = std::array<uint8_t, 256>;
constexpr SBox keygen_sbox_1st_party = {
    ,
};

constexpr std::array<SBox, 8> sboxes_1st_party = {{
    {
        ,
    },
    {
        ,
    },
    {
        ,
    },
    {
        ,
    },
    {
        ,
    },
    {
        ,
    },
    {
        ,
    },
    {
        ,
    },
}};


uint8_t RotateRight8(uint8_t value, uint8_t count) {
    return std::rotr<uint8_t>(value, static_cast<int>(count));
}

std::array<uint8_t, 6> GenerateKeyData(const std::array<uint8_t, 10>& rand, uint32_t idx) {
    static constexpr uint8_t ans_tbl[7][6] = {
        {0xa8, 0x77, 0xa6, 0xe0, 0xf7, 0x43}, {0x5a, 0x35, 0x85, 0xe2, 0x72, 0x97},
        {0x8f, 0xb7, 0x1a, 0x62, 0x87, 0x38}, {0x0d, 0x67, 0xc7, 0xbe, 0x4f, 0x3e},
        {0x20, 0x76, 0x37, 0x8f, 0x68, 0xb7}, {0xa9, 0x26, 0x3f, 0x2b, 0x10, 0xe3},
        {0x30, 0x7e, 0x90, 0x0e, 0x85, 0x0a},
    };
    const auto& ans = ans_tbl[idx];
    std::array<uint8_t, 10> t0{};
    for (std::size_t i = 0; i < t0.size(); ++i)
        t0[i] = keygen_sbox_1st_party[rand[i]];
    return {
        uint8_t((RotateRight8(uint8_t(ans[0] ^ t0[5]), t0[2]) - t0[9]) ^ t0[4]),
        uint8_t((RotateRight8(uint8_t(ans[1] ^ t0[1]), t0[0]) - t0[5]) ^ t0[7]),
        uint8_t((RotateRight8(uint8_t(ans[2] ^ t0[6]), t0[8]) - t0[2]) ^ t0[0]),
        uint8_t((RotateRight8(uint8_t(ans[3] ^ t0[4]), t0[7]) - t0[3]) ^ t0[2]),
        uint8_t((RotateRight8(uint8_t(ans[4] ^ t0[1]), t0[6]) - t0[3]) ^ t0[4]),
        uint8_t((RotateRight8(uint8_t(ans[5] ^ t0[7]), t0[8]) - t0[5]) ^ t0[9]),
    };
}

KeyTables GenerateTables(const std::array<uint8_t, 10>& rand, const std::array<uint8_t, 6>& key,
                         uint32_t idx) {
    const auto& a = sboxes_1st_party[idx];
    const auto& b = sboxes_1st_party[idx + 1u];
    KeyTables out{};
    out.ft = {uint8_t(a[key[4]] ^ b[rand[3]]), uint8_t(a[key[2]] ^ b[rand[5]]),
              uint8_t(a[key[5]] ^ b[rand[7]]), uint8_t(a[key[0]] ^ b[rand[2]]),
              uint8_t(a[key[1]] ^ b[rand[4]]), uint8_t(a[key[3]] ^ b[rand[9]]),
              uint8_t(a[rand[0]] ^ b[rand[6]]), uint8_t(a[rand[1]] ^ b[rand[8]])};
    out.sb = {uint8_t(a[key[0]] ^ b[rand[1]]), uint8_t(a[key[5]] ^ b[rand[4]]),
              uint8_t(a[key[3]] ^ b[rand[0]]), uint8_t(a[key[2]] ^ b[rand[9]]),
              uint8_t(a[key[4]] ^ b[rand[7]]), uint8_t(a[key[1]] ^ b[rand[8]]),
              uint8_t(a[rand[3]] ^ b[rand[5]]), uint8_t(a[rand[2]] ^ b[rand[6]])};
    return out;
}
} // namespace

bool GenerateFirstPartyKeyTables(const std::array<uint8_t, 16>& extensionKey, KeyTables& out) {
    std::array<uint8_t, 10> rand{};
    std::reverse_copy(extensionKey.begin(), extensionKey.begin() + 10, rand.begin());
    std::array<uint8_t, 6> key{};
    std::reverse_copy(extensionKey.begin() + 10, extensionKey.end(), key.begin());
    for (uint32_t idx = 0; idx < 7u; ++idx) {
        if (GenerateKeyData(rand, idx) == key) {
            out = GenerateTables(rand, key, idx);
            return true;
        }
    }
    out = {};
    return false;
}

void Encrypt(uint8_t* data, uint32_t address, std::size_t size, const KeyTables& tables) {
    for (std::size_t i = 0; i < size; ++i, ++address)
        data[i] = uint8_t(uint8_t(data[i] - tables.ft[address & 7u]) ^ tables.sb[address & 7u]);
}
} // namespace MeteorWiiExtensionCrypto
